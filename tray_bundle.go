...
func trayAppPath() string {
	...
	return filepath.Join(home, "Applications", trayAppName()+".app")
}

// trayAppName keeps PRODUCTION as "Partyline" — an existing install must see no change, and the
// Login Items entry people already recognise must not be renamed out from under them. Any other
// control plane gets its own bundle, because a shared one means the two environments overwrite each
// other's app and only whichever launched last is running.
//
// Hyphen, not "Partyline (staging)": the bundle path is used as a `pgrep -f` pattern, where
// parentheses are regex metacharacters and would silently match nothing.
...
func ensureTrayApp() string {
	...
	// Order matches the %s placeholders above: name, display name, bundle id, then the two versions.
	// A per-environment bundle needs a per-environment IDENTIFIER too — macOS keys notification
	// permission and Login Items off it, so two bundles sharing one id fight over both.
	bundleID := "sh.partyline.tray"
	if sfx := envSuffix(); sfx != "" {
		bundleID += "." + sfx
	}
	plist := fmt.Sprintf(trayInfoPlist, trayAppName(), trayAppName(), bundleID, version, version)
