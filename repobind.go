...
type repoBind struct {
	...
	// Project names the project this repo is a MEMBER of (see project_workspace.go). A project
	// spans many repos — a fleet manager, an integration codebase, the services around them —
	// and its shared memory lives in one place all of them point at, so a teammate who has only
	// one of the repos still reads what the others learned.
...
func writeRepoBind(repo, thread string) error {
	rb := repoBind{Thread: thread}
	// Preserve the project membership: the two live in one file and a thread bind must not
	// silently detach the repo from its project.
	if cur, err := readRepoBindFile(repo); err == nil {
		rb.Project = cur.Project
	}
...
// readRepoBindFile reads the whole bind file (loadRepoBind returns only the thread, which is
// what most callers want; project membership needs the rest).
func readRepoBindFile(repo string) (repoBind, error) {
	var rb repoBind
	b, err := os.ReadFile(repoBindPath(repo))
	if err != nil {
		return rb, err
	}
	return rb, json.Unmarshal(b, &rb)
}

// saveRepoBindFile writes it back whole.
func saveRepoBindFile(repo string, rb repoBind) error {
