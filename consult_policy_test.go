...
func TestConsultPolicyIsIndependentOfLaunchPolicy(t *testing.T) {
	cases := []struct {
		...
		{"default project: registration is the consent — launches AND answers", daemonProject{}, "auto", true},
		...
		{"consults off, launch untouched (still auto)", daemonProject{Consults: "ask"}, "ask", true},
		{"explicit off outranks full Auto", daemonProject{Policy: "auto", Consults: "ask"}, "ask", true},
		{"consults auto does not CHANGE launch (already auto)", daemonProject{Consults: "auto"}, "auto", true},
		{"a garbled consults value cannot silently disable it", daemonProject{Consults: "sometimes"}, "auto", true},
		{"only an explicit ask keeps a human in the loop", daemonProject{Policy: "ask"}, "auto", false},
	}
	for _, c := range cases {
