...
// FILING WITHOUT STARTING IS INVISIBLE, so finalize promotes — and the failure mode that matters is
// promotion failing AFTER the tree is already durable. Losing the plan over a machine being offline
// would be far worse than not starting it.

func TestContainerVsTaskPromotion(t *testing.T) {
	task := api.WorkTreeNode{Kind: "task", Title: "one thing"}
	feature := api.WorkTreeNode{Kind: "feature", Title: "f", Children: []api.WorkTreeNode{task}}
	epic := api.WorkTreeNode{Kind: "epic", Title: "e", Children: []api.WorkTreeNode{feature}}

	if isContainer(task) {
		t.Error("a bare task promoted as a TREE — it would enqueue an empty chain")
	}
	if !isContainer(feature) || !isContainer(epic) {
		t.Error("a container promoted as a single task — the whole decomposition becomes one unbuildable run")
	}
	// Kind alone must NOT decide it: a feature someone filed with no children has no leaves to run,
	// and promote-tree on it would enqueue nothing while reporting success.
	childless := api.WorkTreeNode{Kind: "feature", Title: "f"}
	if isContainer(childless) {
		t.Error("a childless feature promoted as a tree — that enqueues nothing and looks like it worked")
	}
}

// The reasons promotion can fail must be ACTIONABLE, because the user reads them instead of a
// started run. "could not promote" tells them nothing they can do.
func TestPromotionFailureReasonsTellYouWhatToDo(t *testing.T) {
	cases := []struct {
		name    string
		peers   []api.Peer
		mustSay string
	}{
		{"nothing advertises it", nil, "add-project"},
		{"registered but offline",
			[]api.Peer{{DaemonID: "d1", DeviceLabel: "mini", Online: false, Projects: []string{"proj"}}}, "online"},
		{"several online",
			[]api.Peer{
				{DaemonID: "d1", DeviceLabel: "mini", Online: true, Projects: []string{"proj"}},
				{DaemonID: "d2", DeviceLabel: "monolith", Online: true, Projects: []string{"proj"}},
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			id, reason := pickFrom(tt.peers, "", "proj")
			if id != "" {
				t.Fatalf("expected a refusal, got daemon %q", id)
			}
			if !strings.Contains(strings.ToLower(reason), tt.mustSay) {
				t.Errorf("reason %q doesn't tell the user %q", reason, tt.mustSay)
			}
			// Never a bare failure: the user is reading this INSTEAD of a started run.
			if len(reason) < 30 {
				t.Errorf("reason is too terse to act on: %q", reason)
			}
		})
	}
}

// FILING MUST NOT START WORK.
//
// finalize briefly auto-promoted, on the reasoning that a filed-but-unpromoted item renders on no
// page. The reasoning was right and the fix was wrong: "you cannot see it" must not be answered with
// "so I spent your tokens". Starting commits a machine, a worktree, model spend and an open PR, and
// every other path to that — the board's Start, the readiness gate, the daemon console — is a
// deliberate act. This pins the boundary so it cannot drift back.
func TestFilingDoesNotStartWork(t *testing.T) {
	...
	// The finalize handler, up to the next case.
	i := strings.Index(body, `case "planning_finalize":`)
	if i < 0 {
		t.Fatal("planning_finalize handler not found — this test is guarding nothing")
	}
	j := strings.Index(body[i:], `case "promote_work_item":`)
	if j < 0 {
		t.Fatal("could not bound the finalize handler")
	}
	finalize := body[i : i+j]

	// It must not reach any promotion path itself.
	for _, forbidden := range []string{"promoteFiled", "PromoteWorkItem", "pickMachine"} {
		if strings.Contains(finalize, forbidden) {
			t.Errorf("planning_finalize calls %s — filing must not start work", forbidden)
		}
	}
	// And it must SAY so: a user who is not told nothing started will assume it did, which is the
	// same surprise in the other direction.
	if !strings.Contains(finalize, "NOTHING HAS STARTED") {
		t.Error("the finalize result does not tell the user that nothing started")
	}
	if !strings.Contains(finalize, "promote_work_item") {
		t.Error("the finalize result does not say how to start the work")
	}
}
