...
// The moment the interview ends is the moment the user is waiting for, and it used to be the one
// vague instruction in the whole flow: "confirm the plan with the user in your own words" — which
// an agent satisfies by restating the plan and stopping. The user, who has been answering
// questions for ten minutes, is never told the interview is over or that anything is ready.
func TestACompletePlanTellsTheAgentToSaySoAndFile(t *testing.T) {
	d := &planDraft{Thread: "t", Title: "Ship the thing", Kind: "feature"}
	out := planStatus(api.Specificity{OK: true}, d)

	for _, want := range []string{
		"THE PLAN IS COMPLETE",
		"SAY THAT TO THE USER",
		"Do NOT ask permission to file",
		"Report what landed",
	} {
		if !strings.Contains(out, want) {
			t.Errorf("the completion message never says %q:\n%s", want, out)
		}
	}
	// The old wording is the bug. It must not come back.
	if strings.Contains(out, "point of no return for their attention") {
		t.Error("the vague 'confirm in your own words' wording is back")
	}
}
