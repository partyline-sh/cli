...
type fakeControlPlane struct {
	...
	// threadUnreadable makes GET /threads/:id 404, simulating a thread that exists but is private
	// to someone else — the state that made an unverified pin poison a whole repo.
	threadUnreadable bool
...
func newFakeControlPlane(t *testing.T) *fakeControlPlane {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		// GET /threads/:id — the READ-BACK createProjectHere does before pinning. Answering it is
		// part of being a working control plane: a pin is written into a file every teammate
		// resolves through, so the id must be proven readable first. `threadUnreadable` lets a test
		// simulate the failure that made this necessary (a thread that exists but is private).
		case r.Method == "GET" && strings.HasPrefix(r.URL.Path, "/api/v1/threads/") &&
			!strings.HasSuffix(r.URL.Path, "/resolve"):
			if f.threadUnreadable {
				w.WriteHeader(http.StatusNotFound)
				_, _ = w.Write([]byte(`{"error":"thread not found"}`))
				return
			}
			id := strings.TrimPrefix(r.URL.Path, "/api/v1/threads/")
			_, _ = w.Write([]byte(`{"thread":{"id":"` + id + `","title":"t"},"blocks":[]}`))
...
func TestCreateProjectRebindsTheLiveSessionThread(t *testing.T) {
	...
	params, _ := json.Marshal(map[string]any{"name": "setup_project", "arguments": map[string]any{}})
	s.handleCall(json.NewEncoder(&out), rpcReq{ID: json.RawMessage(`1`), Params: params})

	if strings.Contains(out.String(), `"isError":true`) {
		t.Fatalf("setup_project returned an error: %s", out.String())
	}
	if s.thread == "" {
...
func TestSetupProjectToolIsRegistered(t *testing.T) {
	for _, d := range cgToolDefs {
		if d["name"] == "setup_project" {
			return
		}
	}
	t.Fatal("setup_project is not in cgToolDefs")
}

// A thread that cannot be READ must never be pinned. .partyline.json is checked into the repo and
// every teammate resolves through it, so an unreadable id turns one machine's bad state into
// everyone's — which is exactly what happened when repo threads were created private: the id
// resolved for its creator and for nobody else, and every later session got a bare "thread not
// found" with no id and no clue where it came from.
func TestAnUnreadableThreadIsNeverPinned(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	f := newFakeControlPlane(t)
	f.threadUnreadable = true
	signedIn(t)
	repo := gitRepo(t, "git@github.com:acme/widgets.git")
	t.Chdir(repo)
	...
	set, msg, isErr := createProjectHere(api.New(), "", "")
	if !isErr {
		t.Fatalf("accepted a thread it could not read: %v", set)
	}
	if b := loadRepoBind(repo); b != "" {
		t.Errorf("pinned %q anyway — a teammate who pulls now inherits the same broken thread", b)
	}
	// The refusal has to be actionable: the id, why, and where to go next.
	for _, want := range []string{"could not be read back", "NOT pinned", "ptln doctor"} {
