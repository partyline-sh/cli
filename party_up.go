...
func partyUp(args []string) {
	...
	if pf.Party.Team == "" {
		// Single-org model: default to your one org — party.team is optional now.
		slug, err := api.New().DefaultOrgSlug()
		if err != nil || slug == "" {
			fatal(fmt.Errorf("party up: couldn't resolve your org (logged in? `ptln login`)"))
		}
		pf.Party.Team = slug
	}
	if len(pf.Agents) == 0 {
