...
func startThreadCheckup(mx *ptymux.Mux) {
	...
	go func() {
		...
		for {
			...
			bl, err := c.Recall(thread, watermark) // only fetched while you're away
			if err != nil || len(bl) == 0 {
				continue
			}
			n, latest := 0, watermark
			for _, b := range bl {
				if b.ID > latest {
					latest = b.ID
				}
...
func llmsNew(args []string) {
	...
	if thread != "" {
		...
		if bin == "claude" {
			...
			fmt.Fprintf(os.Stderr, "note: Common Ground MCP auto-wiring is claude-only for now — %s gets the thread via env; read/write it with `ptln thread` meanwhile.\n", bin)
		}
	}
