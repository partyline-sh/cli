...
func llmsNew(args []string) {
	...
	// --keep-going N: arm the safe auto-continue (E4.0) — a Stop hook forces the agent to keep
	// working until N continuations are spent OR it prints the done sentinel. claude only (Stop
	// hook); the cap makes a runaway impossible.
	if keepGoing > 0 {
		if bin != "claude" {
			fmt.Fprintf(os.Stderr, "note: --keep-going is claude-only for now (needs the Stop hook) — ignored for %s.\n", bin)
		} else if key, err := armKeepGoing(keepGoing, goal); err == nil {
			argv = append(argv, "--settings", keepGoingSettings(key))
			fmt.Fprintf(os.Stderr, "⏩ keep-going armed: up to %d continuations, or until the agent prints %s\n", keepGoing, keepGoingDone)
		}
	}
