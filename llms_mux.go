...
func inheritRepoBindSpec(spec ptymux.Spec) ptymux.Spec {
	if spec.Thread != "" || len(spec.Argv) == 0 || spec.Dir == "" {
		return spec
	}
	...
	if !isWireableEngine(bin) {
		// gemini/antigravity: no per-launch wiring — the thread still rides the spec → env.
		spec.Thread = bound
		return spec
	}
...
func runLLMSApp(initial []ptymux.Spec) error {
	...
	mx.PeerFn = func() { peerMenu(mx) }                 // ctrl-\ p: ask a teammate's agent (ask_peer) → inject the answer
