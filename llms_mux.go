...
func runLLMSApp(initial []ptymux.Spec) error {
	...
	// Capture each live session's git origin while its dir still exists (retroactive on first run),
	// so a session whose directory later vanishes stays recoverable. One pass; persists once.
	if captureSessionRepos(m.all, m.meta) {
		saveLLMMeta(m.meta)
	}
