...
// FLOWING rainbow wordmark (☎ + PARTYLINE) + a status line, centered. `phase` advances each tick
// to sweep the rainbow. Returned to ptymux via mx.LoadingFrame.
...
func runLLMSApp(initial []ptymux.Spec) error {
	...
	// Capture each live session's git origin in the BACKGROUND — never block launch (the retroactive
	// first-run backfill can be dozens of git reads). Runs off its OWN on-disk copy so it never races
	// the UI's m.meta (concurrent map writes would panic); merges only Repo onto the latest on-disk
	// state so a concurrent pin/rename isn't clobbered. Repos are read at recover time / next launch.
	go func() {
		scan := loadLLMMeta()
		if !captureSessionRepos(all, scan) {
			return
		}
		latest := loadLLMMeta()
		for id, mt := range scan {
			...
			if mt.RepoChecked {
				l.RepoChecked = true // persist "looked once" so later launches never re-fork git for it
			}
			latest[id] = l
		}
		saveLLMMeta(latest)
