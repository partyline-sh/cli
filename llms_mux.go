...
func newSessionSpec(tool, dir, thread, worktree, goal string, withState, noThread bool, keepGoing int, permFlags ...string) (ptymux.Spec, error) {
	...
	// The memory refresh hook rides every claude session in a project (see sessionSettings).
	// Skipped when keep-going already added --settings, which includes it.
	if bin == "claude" && keepGoing <= 0 {
		if _, inProject := projectForDir(dir); inProject {
			argv = append(argv, "--settings", sessionSettings(nil))
		}
	}
...
func (h *llmsHome) HandleKey(b []byte) ptymux.HomeAction {
	...
	return ptymux.HomeAction{Suspend: func() {
		if err := runTmuxApp(specs); err != nil {
			// Into the launcher's own flash line, NOT stderr: a suspend's stderr is repainted
			// over the moment the launcher redraws, which turns every failure on this path into
			// "I pressed enter and nothing happened". The flash is the surface the eye is on.
			h.m.flash = "could not open: " + err.Error()
			fmt.Fprintf(os.Stderr, "ptln: %v\n", err)
		}
	}}
}
