...
// argv (the --mcp-config / --append-system-prompt / codex -c common-ground flags) and clear the
...
func unwireThreadArgv(argv []string) []string {
	...
	for i := 0; i < len(argv); i++ {
		...
		if a == "-c" && i+1 < len(argv) && strings.Contains(argv[i+1], "common-ground") {
			i++ // drop codex's `-c mcp_servers.common-ground.…` pair
			continue
		}
		out = append(out, a)
	}
...
func threadPrimer(threadID string) string {
	...
	primer := "## Shared team context (partyline Common Ground)\n\n" +
		...
		"depend on, immediately record it with the common-ground `remember` tool — one concise " +
