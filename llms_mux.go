...
func newSessionSpec(tool, dir, thread, worktree, goal string, withState, noThread bool, keepGoing int) (ptymux.Spec, error) {
	bin := engineBin[strings.ToLower(tool)]
	if bin == "" {
		return ptymux.Spec{}, fmt.Errorf("unknown tool %q — try: claude, codex, gemini, antigravity", tool)
	}
...
// block5 — a 5-row block-letter font for the boot-splash wordmark. Each glyph is exactly 5 cols
// wide, so a centered "PARTYLINE" banner stands ~5× taller than the old one-line wordmark.
...
// loadingFrame renders one frame of the boot splash shown while a session starts up: a big,
// bright FLOWING-rainbow "PARTYLINE" block banner + a status line, centered. `phase` advances
// each tick to sweep the rainbow. Returned to ptymux via mx.LoadingFrame.
func loadingFrame(phase, cols, rows int) []byte {
	...
	letters := []rune("PARTYLINE")
	const gw = 5                                    // glyph width
	bannerW := len(letters)*gw + (len(letters) - 1) // glyphs + 1-col gaps
	if cols >= bannerW+2 && rows >= 9 {             // big terminals: the multi-row block banner
		top := (rows - 5) / 2
		if top < 1 {
			top = 1
		}
		...
		for gr := 0; gr < 5; gr++ { // one banner row at a time
			f.WriteString(fmt.Sprintf("\x1b[%d;%dH", top+gr, col0))
			colpos := 0
			for li, lr := range letters {
				if li > 0 { // 1-col gap between letters
					f.WriteByte(' ')
					colpos++
				}
				for _, ch := range block5[lr][gr] {
					if ch == ' ' {
						f.WriteByte(' ')
					} else {
						// rainbow flows horizontally (colpos+phase) → vertical hue stripes that sweep.
						fmt.Fprintf(&f, "\x1b[1;38;5;%dm█", rainbowAt(colpos+phase))
					}
					colpos++
				}
			}
		...
		f.WriteString(fmt.Sprintf("\x1b[%d;%dH\x1b[38;5;245m%s\x1b[0m", top+7, center(visWidth(msg)), msg))
		return []byte(f.String())
	}

	// Small terminals: fall back to the single-line flowing wordmark.
...
func runLLMSApp(initial []ptymux.Spec) error {
	...
	m := &aiMenu{all: all, tagline: taglines[time.Now().Minute()%len(taglines)], meta: loadLLMMeta(), presence: &daemonPresence{}, firstRun: firstLauncherRun()}
	...
	mx.WorktreeFn = func() { wtMenu(mx) }               // ctrl-\ w: fork this session into a git worktree
	mx.NewFn = func() { newRunMenu(mx) }                // ctrl-\ n: New/Run — fresh session · work · crank
