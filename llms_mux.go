...
func unwireThreadArgv(argv []string) []string {
	os.Unsetenv("PARTYLINE_THREAD_ID")
	os.Unsetenv("PARTYLINE_ENGINE")
...
// sets the thread env (inherited by the relaunched child + the cg-mcp it spawns). It carries the
// conversation forward (adds --continue for a fresh claude with no resume) and is idempotent —
// skips any wiring flag already present. Used by the ctrl-\ c "attach = relaunch wired" path.
func wireThreadArgv(bin string, argv []string, thread string) []string {
	os.Setenv("PARTYLINE_THREAD_ID", thread)
	engine := bin
	if bin == "agy" {
		engine = "antigravity"
	}
	os.Setenv("PARTYLINE_ENGINE", engine)
