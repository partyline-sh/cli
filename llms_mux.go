...
// unwireThreadArgv is the inverse of wireThreadArgv: strip the Common Ground wiring from a launch
// argv (the --mcp-config / --append-system-prompt / codex -c partyline-context-threads flags) and clear the
// thread env, so a relaunch comes up with NO recall/remember. Keeps --resume/--continue so the
// conversation carries over. Used by the ctrl-\ c "disconnect" action.
func unwireThreadArgv(argv []string) []string {
	// No env mutation here: the disconnected Spec simply carries no Thread/Engine, so the child's
	// per-session env (childEnv) omits them. (Global os.Setenv is what caused cross-contamination.)
	out := make([]string, 0, len(argv))
	for i := 0; i < len(argv); i++ {
		a := argv[i]
		if a == "--mcp-config" || a == "--append-system-prompt" {
			i++ // drop this flag AND its value
			continue
		}
		if a == "-c" && i+1 < len(argv) && (strings.Contains(argv[i+1], "partyline-context-threads") || strings.Contains(argv[i+1], "common-ground")) {
			i++ // drop codex's `-c mcp_servers.<ctx>.…` pair (new name or legacy common-ground)
			continue
		}
		out = append(out, a)
	}
...
// wireThreadArgv augments a child's launch argv with the Common Ground wiring for its engine and
// returns the wired argv + the engine name. The thread/engine are carried on the Spec (→ the
// child's own env via childEnv), NOT set globally — that's the fix for cross-session
// contamination. It carries the conversation forward (adds --continue for a fresh claude with no
// resume) and is idempotent — skips any wiring flag already present. For the ctrl-\ c attach path.
func wireThreadArgv(bin string, argv []string, thread string) ([]string, string) {
	engine := bin
	if bin == "agy" {
		engine = "antigravity"
	}
	...
	switch bin {
	case "claude":
		if !has("--resume") && !has("--continue") {
			argv = append(argv, "--continue") // resume the latest convo in this dir (best-effort)
		}
		if !has("--mcp-config") {
			cfg := fmt.Sprintf(`{"mcpServers":{"partyline-context-threads":{"command":%q,"args":["cg-mcp"]}}}`, selfExe())
	...
	case "codex":
		...
		// gemini / antigravity: the thread rides the env; they use a one-time `ptln thread connect`
		// for the MCP server, so no per-launch argv wiring here.
	}
	return argv, engine
}
...
func threadPrimer(threadID string) string {
	...
	primer := "## Shared team context (partyline Context Threads)\n\n" +
		...
		"fact, kind = decision | constraint | contract. Record only durable, cross-seam facts " +
