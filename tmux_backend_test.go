...
func TestUseTmuxBackend(t *testing.T) {
	...
	if useTmuxBackend() {
		t.Error("tmux mode is OPT-IN — unset env must mean the built-in mux")
	}
	t.Setenv("PARTYLINE_MUX", "tmux")
	if !useTmuxBackend() {
		t.Error("PARTYLINE_MUX=tmux with a usable tmux should turn the backend on")
	}
...
func TestTmuxConfContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"tmux --save-workspace", // detach/close snapshot the workspace for --resume
		"client-detached",
		"window-unlinked",
