...
// odooInt reads an integer that JSON delivered as float64 (every Odoo number does).
func odooInt(v any) int {
	switch n := v.(type) {
	case float64:
		return int(n)
	case int:
...
func (s odooSource) projectsBoard() (*boardData, error) {
	...
	d := &boardData{
		...
		Scope:     "all projects",
		Live:      false,
		KeepOrder: true, // rows arrive in kanban sequence order; re-sorting would un-Odoo them
		ReadAt:    time.Now(),
	}
	for _, c := range cols {
		d.ByColumn[c.Key] = nil
...
var taskFields = []string{
	...
	"date_deadline", "priority", "write_date", "description", "sequence",
}
...
func (s odooSource) cards(pid int, keys map[string]bool) ([]api.BoardCard, error) {
	...
	for offset := 0; offset < odooMaxCards; offset += odooLimit {
		page, err := s.query("project.task", []any{[]any{"project_id", "=", pid}},
			taskFields, "sequence asc, id asc", odooLimit, offset) // kanban order — what Odoo itself displays
		if err != nil {
			return nil, err
		}
		recs = append(recs, page...)
	...
	for _, r := range recs {
		...
		card := api.BoardCard{
			...
			Seq:        odooInt(r["sequence"]),
			Ref:        "#" + id,
			Body:       odooBody(odooStr(r["description"]), chatter[id]),
			Attention:  odooUrgent(odooStr(r["priority"])),
		}
		if urlTmpl != "" {
			card.SourceURL = strings.ReplaceAll(urlTmpl, "{id}", id)
		}
...
// odooPriority names the scale rather than printing its number. The mapping is Odoo's own, from
// the help on project.task.display_name: "! medium, !! high, !!! urgent". A bare "3" on a tile
// tells a reader nothing; "urgent" is the same fact in the words the tracker uses.
//
// "0" is the default nobody ever sets, so it is omitted — showing it implies a decision was made.
func odooPriority(v string) string {
	...
	case "3":
		return "urgent"
	default:
		return v // an installation that extended the scale keeps its own value
	}
}

// odooUrgent reports whether a priority should light the tile up. Only the top of the scale does:
// a board where every card is coloured is a board with no signal in the colour.
func odooUrgent(v string) bool { return strings.TrimSpace(v) == "3" }
