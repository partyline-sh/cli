...
func (m *boardModel) attachSession() bool {
	...
	if insidePtlnTmux() {
		if err := tmuxFocus(target); err != nil {
...
func runSessionTarget(runID string) string {
	...
	// Panes, not windows: a run merged in beside another session shares that window's name, so a
	// window-shaped search would jump to its neighbour. The pane's own label is what identifies it.
	out, err := tmuxCmd("list-panes", "-s", "-t", tmuxSessionName, "-F", "#{pane_id}\t#{window_name}\t#{@ptln_spec}").Output()
	if err != nil {
		return ""
	}
	short := runID
...
//
// IN THE TERMINAL, not the browser. This used to open the PR URL, which made the product's central
// gate — "read what the agent wrote, keep what's right" — happen on GitHub or not at all. The diff
// is now computed from this machine's own checkout and read in an overlay with accept on a key; the
// browser remains one keystroke away (`o`) and is the automatic fallback when this machine has no
// checkout of the project. See board_review_overlay.go.
func (m *boardModel) reviewDiff(c *api.Client) {
	card, ok := m.focused()
	if !ok {
		return
	}
	if card.Foreign {
		m.openURL(card.SourceURL, "item") // a foreign card's review lives in its own tracker
		return
	}
	m.openReviewOverlay(c, *card)
}
...
func (m *boardModel) openPR() {
	...
	if card.Foreign {
		// A foreign card's link is the item in its OWN tracker; it has no PR to open.
		m.openURL(card.SourceURL, "item")
		return
	}
	if card.PRURL == "" {
...
func boardBell(prev, next *boardData) (ring bool, note string) {
	if prev == nil || next == nil {
		return false, ""
	}
	was := map[string]api.BoardColumn{}
	for _, col := range prev.Keys() {
	...
	arrived := 0
	first := ""
	// Only partyline's own columns ring. A foreign board declares its own column names and we have
	// no basis for deciding which of somebody's Jira statuses deserves a bell.
	for _, col := range []api.BoardColumn{api.ColReview, api.ColBlocked} {
		for _, c := range next.Column(col) {
			...
			if old, seen := was[c.ID]; !seen || old != col {
				arrived++
				if first == "" {
					first = next.Title(col) + ": " + cardTitle(c)
				}
			}
		}
	}
