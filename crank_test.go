...
func TestTaskBranchName(t *testing.T) {
	...
	// The slug comes from the ORIGINAL task, never the retry preamble.
	if strings.Contains(a, "previous-attempt") || !strings.Contains(a, "Fix-the-flaky-login") {
		t.Fatalf("slug built from the wrapper, not the task: %q", a)
	}
	// No run id (manual crank) → the legacy shape, unchanged.
	if got := taskBranchName("", 0, "Fix the flaky login test"); got != "crank-01-Fix-the-flaky-login" {
