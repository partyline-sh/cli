...
func editSite(c installConfig, ops installOps) (installConfig, bool, string) {
	...
	answer, changed := ask(ops, "site (or 'w' to watch for the record)", c.site)
	if !changed {
		return c, false, ""
	}
	if strings.EqualFold(answer, "w") {
		// DNS propagation is measured in minutes; a one-shot "does not resolve" made the
		// operator re-open this screen to find out whether their record had landed.
		return c, false, watchSiteDNS(c, ops)
	}
	...
	if err := rejectLoopbackSite(site); err != nil {
		return c, false, firstLineOf(err.Error())
	}
	c.site = site
	return c, true, ""
}

// firstLineOf keeps a multi-line refusal usable as a one-line box status.
...
func editDir(c installConfig, ops installOps) (installConfig, bool, string) {
...
func editBind(c installConfig, ops installOps) (installConfig, bool, string) {
...
func editPortField(role string) func(installConfig, installOps) (installConfig, bool, string) {
	return func(c installConfig, ops installOps) (installConfig, bool, string) {
		cur := portFor(c, role)
		...
		if err != nil || n < 1 || n > 65535 {
			return c, false, fmt.Sprintf("%q is not a port number (1-65535).", answer)
		}
		if ops.portBusy(c.bind, n) {
			return c, false, fmt.Sprintf("%d is also in use — the stack would fail to start.", n)
		}
		switch role {
		case "http":
			c.httpPort = n
...
func editDNS(c installConfig, ops installOps) (installConfig, bool, string) {
	...
	if net.ParseIP(host) == nil {
		return c, false, fmt.Sprintf("%q is not an address — a resolver is an IP, optionally with :port.", answer)
	}
	c.dns = answer
	return c, true, ""
}

func editTLS(c installConfig, ops installOps) (installConfig, bool, string) {
	...
	fmt.Fprintf(ops.out, "  5  tunnel    Tailscale/Cloudflare terminates TLS in front (off + bind 127.0.0.1)\n")
	...
	case "4", "off":
		c.tls = tlsOff
		// Off MEANS plain HTTP, so the site follows — otherwise a bare hostname (completed to
		// https:// on entry) left the operator on an https site with TLS off, which is the
		// TUNNEL shape, and the report started talking about tailscale to someone on a LAN.
		if rest, ok := strings.CutPrefix(c.site, "https://"); ok {
			c.site = "http://" + rest
			return c, true, "plain HTTP — site is now " + c.site
		}
	case "5", "tunnel":
		// The tunnel preset is TLS off plus a loopback bind, because plain HTTP on 0.0.0.0
		// behind a tunnel is a LAN-wide bypass of it. The bind moves only if the operator has
		// not chosen one themselves.
		c.tls = tlsOff
		// A tunnel serves https at its own edge, so the PUBLIC site is https by definition.
		if rest, ok := strings.CutPrefix(c.site, "http://"); ok {
			c.site = "https://" + rest
		}
		if c.bind == "0.0.0.0" && !c.explicit["BIND_ADDR"] {
			c.bind = "127.0.0.1"
			c.explicit["BIND_ADDR"] = true
		}
		return c, true, "tunnel mode — the report prints the tailscale/cloudflared command, port filled in"
	default:
		return c, false, "Pick 1-5."
	}
	return c, true, ""
}

func editStorage(c installConfig, ops installOps) (installConfig, bool, string) {
	fmt.Fprintf(ops.out, "\n  Object storage holds attachments. Off means the feature is dark — every\n")
	fmt.Fprintf(ops.out, "  upload says 'object storage is not configured' — and the MinIO pair of\n")
	fmt.Fprintf(ops.out, "  containers never runs. External S3/R2 is a .env change, not a menu one.\n")
	cur := "off"
	if c.minio {
		cur = "minio"
	}
	answer, changed := ask(ops, "storage (minio/off)", cur)
	if !changed {
		return c, false, ""
	}
	switch strings.ToLower(answer) {
	...
	case "off", "none", "no":
		c.minio = false
	default:
		return c, false, "Enter minio or off."
	}
	c.explicit["MINIO_REPLICAS"] = true
	return c, true, ""
}

func editEdge(c installConfig, ops installOps) (installConfig, bool, string) {
	...
	default:
		return c, false, "Enter caddy or none."
	}
	return c, true, ""
}
