...
func isolateInstallRecords(t *testing.T, home string) {
	t.Helper()
	prev := installDirRecords
	installDirRecords = func() []string { return []string{filepath.Join(home, ".partyline", "server-dir")} }
	t.Cleanup(func() { installDirRecords = prev })
}
...
// An upgrade rewrites the Caddyfile when it is still the one we wrote (the "re-apply the current
// settings" step), and `compose up -d` does NOT restart a container whose service definition is
// unchanged. The Caddyfile is a mounted FILE, so without an explicit reload Caddy keeps serving
// the OLD config: the new route sits correct on disk and unreachable in the running edge.
//
// The install path already reloads for exactly this reason. This is the same failure on the
// upgrade path, and it is silent — the upgrade reports success and the new route 404s.
func TestUpgradeReloadsTheEdgeAfterRewritingItsConfig(t *testing.T) {
	dir := writeInstall(t, "SITE_URL=https://box.local\n")
	var out bytes.Buffer
	var ran []string
	ops := installOps{
		...
		httpOK: func(string) (int, error) { return 200, nil },
	}
	if !serverUpgrade(dir, ops) {
		t.Fatalf("upgrade failed:\n%s", out.String())
	}
	joined := strings.Join(ran, " | ")
	up := strings.Index(joined, "compose up")
	reload := strings.Index(joined, "restart caddy")
	if reload < 0 {
		t.Fatalf("upgrade never reloaded the edge, so a rewritten Caddyfile is not served: %s", joined)
	}
	if reload < up {
		t.Errorf("the edge was reloaded before the stack came up: %s", joined)
	}
}
