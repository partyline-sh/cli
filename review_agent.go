...
func runReviewJob(d daemonDevice, ev api.RunEvent) error {
	...
	log(fmt.Sprintf("reviewing %d change(s)…", reviewed))

	grade, summary, issues, err := runGradedReview(dir, headString(b.String(), maxDiffBytes), reviewTimeout)
	if err != nil {
		return fail("review", err)
	}
	if err := api.RecordReview(d.Base, d.Token, targetID, grade, summary, "claude", issues); err != nil {
...
// runGradedReview runs the fresh, tool-less reviewer engine (verifier ≠ producer, no thread wiring —
// same independence as T2b) and parses its graded JSON. Errors on timeout, engine failure, or an
// unparseable/invalid reply so the review run fails loudly (retryable) rather than recording a
// misleading grade.
func runGradedReview(dir, tasksAndDiffs string, timeout time.Duration) (string, string, []api.ReviewIssue, error) {
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	cmd := exec.CommandContext(ctx, "claude", "-p", reviewGradePrompt(tasksAndDiffs), "--output-format", "json", "--allowedTools", "")
	cmd.Dir = dir
	...
	if err != nil {
		return "", "", nil, fmt.Errorf("couldn't run the reviewer engine")
	}
	reply, _, _, ok := parseWorkerOutput(out)
	if !ok {
		reply = string(out)
	}
	return parseGradedReview(reply)
}
