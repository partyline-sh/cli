...
func branchDiff(repo, branch string) string {
	...
	if out, err2 := exec.Command("git", "-C", repo, "show", "--format=", branch).Output(); err2 == nil && len(out) > 0 {
		return string(out)
	}
	// Not local: fetch the pushed ref and diff FETCH_HEAD (no local branch created, nothing mutated).
	if err := exec.Command("git", "-C", repo, "fetch", "origin", "refs/heads/"+branch).Run(); err != nil {
		return ""
	}
	out, err = exec.Command("git", "-C", repo, "diff", "FETCH_HEAD^", "FETCH_HEAD").Output()
	if err != nil {
		out, _ = exec.Command("git", "-C", repo, "show", "--format=", "FETCH_HEAD").Output()
	}
	return string(out)
}
