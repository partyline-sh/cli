...
func runReviewJob(d daemonDevice, ev api.RunEvent) error {
	...
	targetID, tasks, criteria, canon, err := api.ReviewTarget(d.Base, d.Token, ev.RunID)
	if err != nil {
	...
	grade, summary, issues, usage, err := runGradedReview(reviewDir, engineName, headString(b.String(), maxDiffBytes), ev.Model, treeNote, criteria, canon, reviewTimeout, logln)
	if err != nil {
		return fail("review", err)
	}
...
func reviewGradePrompt(tasksAndDiffs string, criteria []api.ReviewCriterion, canon []string, treeNote string) string {
	var b strings.Builder
	...
	if len(canon) > 0 {
		// Project CANON — the team's recorded constraints/contracts. Given to the reviewer as things
		// to CHECK the diff against (not as the producer's context to adopt), so it stays independent
		// but catches violations of established decisions. A clear violation is a finding.
		b.WriteString("PROJECT CANON — established team constraints/contracts. These are NOT the author's reasoning; treat them as rules to check the diff against. If the change plausibly VIOLATES one, that's a finding (severity by impact). Do not assume a violation you can't see in the diff:\n")
		for _, c := range canon {
			b.WriteString("  - " + strings.TrimSpace(c) + "\n")
		}
		b.WriteString("\n")
	}
	// Shared judging rules. This persona already carried its own verification-honesty rule (forced by
	// a real fabricated-finding incident); reviewCore adds reason-before-deciding and the calibration
	// examples that fix what a grade MEANS, which six-word grade definitions cannot.
	b.WriteString(reviewCore())
	b.WriteString("\n\nTASKS AND THEIR DIFFS:\n")
...
func runGradedReview(dir, engineName, tasksAndDiffs, model, treeNote string, criteria []api.ReviewCriterion, canon []string, timeout time.Duration, logf func(string)) (string, string, []api.ReviewIssue, reviewUsage, error) {
	...
	argv, stdinPrompt, err := graderOneShot(spec, reviewGradePrompt(tasksAndDiffs, criteria, canon, treeNote), model, logf)
	if err != nil {
