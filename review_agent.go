...
func runReviewJob(d daemonDevice, ev api.RunEvent) error {
	...
	grade, summary, issues, err := runGradedReview(reviewDir, engineName, headString(b.String(), maxDiffBytes), ev.Model, treeNote, criteria, reviewTimeout, logln)
	if err != nil {
		return fail("review", err)
	}
...
func runGradedReview(dir, engineName, tasksAndDiffs, model, treeNote string, criteria []api.ReviewCriterion, timeout time.Duration, logf func(string)) (string, string, []api.ReviewIssue, error) {
