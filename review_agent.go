...
func runReviewJob(d daemonDevice, ev api.RunEvent) error {
	...
	grade, summary, issues, err := runGradedReview(dir, headString(b.String(), maxDiffBytes), ev.Model, reviewTimeout)
	if err != nil {
		return fail("review", err)
	}
...
// misleading grade.
func runGradedReview(dir, tasksAndDiffs, model string, timeout time.Duration) (string, string, []api.ReviewIssue, error) {
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	cargs := []string{"-p", reviewGradePrompt(tasksAndDiffs), "--output-format", "json", "--allowedTools", ""}
