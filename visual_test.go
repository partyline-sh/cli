...
func TestRunVisualReviewFailClosed(t *testing.T) {
	...
	// Gate off.
	if vr := runVisualReview(t.TempDir(), t.TempDir(), "task", timeout); vr.ran {
	...
	if vr := runVisualReview(noUI, noUI, "task", timeout); vr.ran {
	...
	vr := runVisualReview(empty, empty, "task", timeout)
	...
	vr = runVisualReview(broken, broken, "task", timeout)
