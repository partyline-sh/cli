...
// The bug this guards: envGapsFor reads refs/remotes/origin/*, and NOTHING in the delta path used to
// refresh them. A merge that landed on the remote minutes ago was invisible, so the daemon reported
// "0 commits waiting" — which the UI renders as "everything built is live". A confidently wrong
// answer, and the worst possible failure for a widget whose entire job is telling you what is NOT
// live yet. Observed in production: staging was one commit ahead of main and In flight said in-sync.
func TestEnvGapsSeeRemoteMovesOnlyAfterFetch(t *testing.T) {
	origin := envRepo(t) // main, plus staging one commit ahead

	// A clone is what a daemon actually has: remote-tracking refs frozen at clone time.
	clone := t.TempDir() + "/clone"
	if out, err := exec.Command("git", "clone", "-q", origin, clone).CombinedOutput(); err != nil {
		t.Fatalf("clone: %v\n%s", err, out)
	}

	q := api.EnvQuestion{Environments: []api.EnvStep{
		{Name: "staging", Branch: "staging"},
		{Name: "production", Branch: "main"},
	}}

	gaps := envGapsFor(clone, q)
	if len(gaps) != 1 || gaps[0].CommitCount != 2 {
		t.Fatalf("fresh clone should see the existing gap, got %+v", gaps)
	}

	// Someone merges to staging on the remote. The clone knows nothing about it yet.
	run := func(args ...string) {
		t.Helper()
		cmd := exec.Command("git", args...)
		cmd.Dir = origin
		cmd.Env = append(os.Environ(),
			...
			"GIT_COMMITTER_NAME=Bo", "GIT_COMMITTER_EMAIL=bo@example.com")
		if out, err := cmd.CombinedOutput(); err != nil {
			t.Fatalf("git %v: %v\n%s", args, err, out)
		}
	}
	run("checkout", "-q", "staging")
	if err := os.WriteFile(filepath.Join(origin, "late.txt"), []byte("late"), 0o644); err != nil {
		t.Fatal(err)
	}
	run("add", ".")
	run("commit", "-m", "landed after the clone")
	run("checkout", "-q", "main")

	// Without a fetch the clone still reports the OLD number — confidently, with no error.
	if got := envGapsFor(clone, q)[0].CommitCount; got != 2 {
		t.Fatalf("precondition: stale clone should still say 2, got %d", got)
	}

	fetchForDelta(clone)

	if got := envGapsFor(clone, q)[0].CommitCount; got != 3 {
		t.Fatalf("after fetch the new commit must be visible: want 3, got %d", got)
	}
}
