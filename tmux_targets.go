...
// paneForKey maps an llms session id to its window's active pane.
func paneForKey(key string) (pane, label string, ok bool) {
	out, err := tmuxCmd("list-windows", "-t", tmuxSessionName, "-F", "#{@ptln_key}\t#{pane_id}\t#{window_name}").Output()
	if err != nil {
		return "", "", false
	}
...
func (tmuxTargets) LiveSessions() []ptymux.LiveSession {
	out, err := tmuxCmd("list-windows", "-t", tmuxSessionName, "-F", "#{@ptln_key}\t#{window_name}").Output()
	if err != nil {
		return nil
	}
	var live []ptymux.LiveSession
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		if key, label, ok := strings.Cut(line, "\t"); ok && key != "" && key != tmuxLauncherKey {
			live = append(live, ptymux.LiveSession{Key: key, Label: label})
		}
	}
	return live
}
