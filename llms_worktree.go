...
func (s aiSession) projKey() string {
	...
	// ID-shaped launch dirs (an agent framework's per-session workspaces) group under their
	// first human-named ancestor instead of each becoming a ULID-labeled "project".
	return collapseGeneratedKey(s.Cwd)
}
