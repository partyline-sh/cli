...
//
//	m   move the focused card to another column
//	e   edit its title
//	c   add a comment
//
// A PARTYLINE card's legal moves are its existing transitions: `m` offers exactly the subset of
// its `a` actions that land it in a different column, labelled by DESTINATION ("→ Backlog — To
// backlog"). Nothing new is invented server-side, so a move can never do something the action
// menu could not — and every guard those transitions carry (confirms on destructive ones, the
// review gate on Accept) still applies, because the pick simply runs the action.
//
...
// moveCard is the `m` key: pick a destination, then perform the move.
func (m *boardModel) moveCard(c *api.Client) bool {
	card, ok := m.focused()
	if !ok {
		return false
	}
	...
	if card.Foreign {
		...
		for _, col := range m.data.Columns {
			...
			items = append(items, pickerItem{
				Label: "→ " + col.Title,
				Note:  "writes the change to " + m.data.Source,
				Value: string(col.Key),
			})
		}
		if len(items) == 0 {
			m.setToast("this board has no other column to move to", false)
			return false
		}
		scope, id, title := m.scope, card.ID, cardTitle(*card)
		m.openOverlay(&pickerOverlay{
			heading: "move " + clipVis(title, 40),
			items:   items,
			onPick: func(m *boardModel, c *api.Client, v pickerItem) bool {
				if err := mover.MoveCard(scope, id, api.BoardColumn(v.Value)); err != nil {
					m.setToast("move failed: "+err.Error(), true)
					return false
				}
				m.setToast("moved — "+m.data.Source+" has it", false)
				return true // refresh, so the card renders in its new column
			},
		})
		return false
	}

	items := partylineMoves(*card)
	if len(items) == 0 {
		m.setToast("this card has no column moves right now — `a` shows everything it can do", false)
		return false
	}
	cardCopy := *card
	m.openOverlay(&pickerOverlay{
		heading: "move " + clipVis(cardTitle(*card), 40),
		items:   items,
		onPick: func(m *boardModel, c *api.Client, v pickerItem) bool {
			for _, act := range boardActions(cardCopy) {
				if act.Key == v.Value {
					return m.runAction(c, cardCopy, act) // confirms and gates apply exactly as in `a`
				}
			}
			return false
		},
