...
// odooSeqArgs is the payload for a row-order write: the task, its new kanban sequence, and —
// when the drop crossed a priority band — the priority that makes the position real.
func odooSeqArgs(taskID string, seq int, pri string) (map[string]any, error) {
	id, err := strconv.Atoi(taskID)
	if err != nil {
		return nil, fmt.Errorf("%q is not an Odoo task id", taskID)
	}
	args := map[string]any{"id": id, "sequence": seq}
	if pri != "" {
		args["priority"] = pri
	}
	return args, nil
...
// odooPriLabel names a band the way the tool's own enum does.
func odooPriLabel(pri string) string {
	switch pri {
	case "0":
		return "Low"
	case "1":
...
func (s odooSource) SetCardSeq(scope, cardID string, seq int, pri string) error {
	if err := requireTaskScope(scope); err != nil {
		return err
	}
	args, err := odooSeqArgs(cardID, seq, pri)
	if err != nil {
		return err
	}
	_, err = s.p.callTool(s.cfg.Opt("update_tool", "update_project_task"), args)
