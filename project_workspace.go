...
func projectForDir(dir string) (*projectWorkspace, bool) {
	...
	// 1. The repo declares its project. Checked in, so a teammate who clones inherits the link
	//    without configuring anything. This is the designed path and always wins.
	if b, err := os.ReadFile(repoBindPath(repo)); err == nil {
		var rb repoBind
		if json.Unmarshal(b, &rb) == nil && strings.TrimSpace(rb.Project) != "" {
			if ws, err := loadProjectWorkspace(rb.Project); err == nil {
				return ws, true
			}
		}
	}
	// 2. Failing that, THIS MACHINE knows: the project file lists its members by canonical
	//    remote, so a repo can belong to a project without anything being written into it.
	//    Cached, because this runs on the status bar's redraw path — see project_bind_remote.go.
	if label := cachedProjectFor(repo); label != "" {
		if ws, err := loadProjectWorkspace(label); err == nil {
			// Still a member? A repo dropped from the project must stop resolving to it.
			if ws.repoNameFor(dir) != "" {
				return ws, true
			}
		}
		forgetProjectFor(repo)
	}
	if label := projectByRemote(repo); label != "" {
		if ws, err := loadProjectWorkspace(label); err == nil {
			rememberProjectFor(repo, label)
			return ws, true
		}
	}
	return nil, false
