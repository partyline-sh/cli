...
func (d *planDraft) asTree() api.WorkTreeNode {
	...
	return api.WorkTreeNode{
		...
		Children:           withFiledReadiness(d.Children),
	}
}
...
// withFiledReadiness stamps the same score through a decomposition. A child that already carries one
// keeps it — an explicit score is a judgement, and only an UNSET one (0) is the omission this fixes.
func withFiledReadiness(nodes []api.WorkTreeNode) []api.WorkTreeNode {
	out := make([]api.WorkTreeNode, 0, len(nodes))
	for _, n := range nodes {
		if n.Readiness == 0 {
			n.Readiness = planFiledReadiness
		}
		n.Children = withFiledReadiness(n.Children)
		out = append(out, n)
	}
	return out
}
