...
type planDraft struct {
	...
	// RepoLabel is which partyline project this plan is about. Carried so the criteria can be PROVED
	// in the right repository: the MCP server inherits the SESSION's working directory, which is
	// often not the repo the plan targets, and probing there proves something about the wrong tree.
	...
	// NO Children. A PLAN FILES ONE CARD.
	//
	// The tree existed so an epic became several cards, each running in its own worktree forked
	// from the base branch — sequential in time, parallel in code. They could not see each
	// other's work, re-derived their own approaches, drifted apart, and collided on merge; the
	// reviewers then graded fragments written against a codebase none of them shared.
	//
	// Decomposition did not go away, it moved INSIDE the run, where subagents share one worktree
	// and can read what the others just wrote. That is partyline's job to enable, not to model.
...
func (d *planDraft) asTree() api.WorkTreeNode {
	...
	return api.WorkTreeNode{
		...
		Readiness:          planFiledReadiness,
	}
}

// planFiledReadiness is what a FINALIZED draft is worth, and it is not a guess.
//
// planning_finalize refuses while any required slot is unmet OR any open question is unanswered. So
// by the time a draft becomes a tree it has, by construction, nothing left to ask a human — which is
// the definition readiness measures: "an agent could build this with zero questions".
//
// THIS FIELD IS NOT ADVISORY, whatever its documentation says. web/src/lib/api/work-view.ts sets
// MIN_START_READINESS = 4 and startBlockedByReadiness() DISABLES the board card's Start button below
// it. Leaving it unset filed every plan at 0, so every item this door produced was un-startable from
// the board and there was no control anywhere to raise it. A planning session in the web files items
// that start; this path must be indistinguishable from it.
const planFiledReadiness = 5
...
func planStatus(spec api.Specificity, d *planDraft) string {
	...
	if spec.OK {
		// THE MOMENT THE USER IS WAITING FOR, and the one place this flow used to go vague.
		//
		// Every other step here gives a loud, single instruction — ask THIS, record the answer,
		// do not skip ahead. This one said "confirm the plan with the user in your own words",
		// which an agent satisfies by restating the plan and stopping. The user has been
		// answering questions for ten minutes and is never told the interview ended: they sit
		// looking at a summary with no idea anything is expected of them, or that the plan is
		// ready to file. That is the hang, and it is the same string in the web app and the CLI.
		b.WriteString("\nTHE PLAN IS COMPLETE — every required slot is filled and every question is answered.\n\n")
		b.WriteString("SAY THAT TO THE USER, EXPLICITLY. They have been answering questions and have no way " +
			"to know the interview is over. Then, without waiting for permission:\n\n")
		b.WriteString("  1. Summarize what you are about to file — ONE card, by title and what it " +
			"covers — so the user can object before it lands.\n")
		b.WriteString("  2. Call planning_finalize. Do NOT ask permission to file: filing starts nothing " +
			"(promote_work_item does that, separately), the gate has already refused anything unbuildable, " +
			"and the user asked for this plan in the first place.\n")
		b.WriteString("  3. Report what landed — each item by title, where to read it, and that it is not " +
			"running yet.\n")
		return b.String()
	}
