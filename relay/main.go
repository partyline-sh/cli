...
func main() {
	...
	go serveBus(bus.NewHub()) // the session bus: a separate port, its own auth, no shared state
	for {
		c, err := ln.Accept()
		if err != nil {
			continue
...
func handle(c net.Conn, reg *registry) {
	...
	// NOTE: no per-IP rate limiter here — the Caddy/CF front does not preserve
