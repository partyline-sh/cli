...
type projectSetup struct {
	...
	Private     bool // created private (creator-only) rather than team-wide
	MadeThread  bool
	WrotePin    bool
...
//
// visibility applies ONLY when a NEW project is created: "" or "team" → team-wide (the default —
// teammates see it and their machines auto-adopt it), "private" → creator-only. An adopted existing
// project keeps whatever visibility its creator chose; silently flipping it here would change what
// the whole team sees as a side effect of one machine's setup.
func createProjectHere(c *api.Client, want, visibility string) (*projectSetup, string, bool) {
	cwd, err := os.Getwd()
	if err != nil {
...
func (s *cgServer) handleCreateProject(enc *json.Encoder, req rpcReq) {
	var pp struct {
		Args struct {
			...
			// "team" (default) or "private" — the same choice `ptln project setup --private` makes.
			// Anything else falls through to the server, which rejects it by name.
			Visibility string `json:"visibility"`
		} `json:"arguments"`
	}
	_ = json.Unmarshal(req.Params, &pp)

	set, msg, isErr := createProjectHere(s.c, pp.Args.Label, pp.Args.Visibility)
	if isErr || set == nil {
...
func (p *projectSetup) summary() string {
	...
	if p.MadeProject {
		...
		if p.Private {
			verb = "Created PRIVATE project"
		}
	}
	fmt.Fprintf(&b, "%s %q for %s.\n\n", verb, p.Label, p.Path)
	if p.MadeProject {
		if p.Private {
			b.WriteString("• Private: only you can see it. Share it later with `ptln project visibility " + p.Label + " team`.\n")
		} else {
			b.WriteString("• Team-wide: your teammates can see it, and their machines that have this repo adopt it automatically.\n")
		}
	}
