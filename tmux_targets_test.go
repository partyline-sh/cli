...
// Regression: with the launcher fixture present, opening a NEW session must succeed and land
// LEFT of the launcher. The groups change made tmuxMerge's scan return PANE ids, and the
// insert-before branch handed one to `new-window -b -t` (a window target) — tmux answered
// "can't specify pane here" and every open from the launcher failed.
func TestTmuxMergeOpensLeftOfLauncher(t *testing.T) {
	if _, err := exec.LookPath("tmux"); err != nil {
		t.Skip("tmux not installed")
	}
	t.Setenv("PARTYLINE_TMUX_SOCKET", "ptln-test-launcher-target")
	t.Cleanup(func() { _ = tmuxCmd("kill-server").Run() })
	_ = tmuxCmd("kill-server").Run()

	if _, err := tmuxMerge([]ptymux.Spec{{Label: "w1", Key: "k1", Argv: []string{"sleep", "60"}, Dir: "/tmp"}}); err != nil {
		t.Fatalf("first merge: %v", err)
	}
	if ensureLauncherWindow() == "" {
		t.Fatal("no launcher window")
	}
	// the failing case: launcher exists, a fresh session opens
	if _, err := tmuxMerge([]ptymux.Spec{{Label: "w2", Key: "k2", Argv: []string{"sleep", "60"}, Dir: "/tmp"}}); err != nil {
		t.Fatalf("open with launcher present: %v", err)
	}
	out, _ := tmuxCmd("list-windows", "-t", tmuxSessionName, "-F", "#{window_name}").Output()
	names := strings.Split(strings.TrimSpace(string(out)), "\n")
	if len(names) != 3 || !strings.Contains(names[len(names)-1], "launcher") {
		t.Fatalf("launcher must stay far right, got %v", names)
	}
}
