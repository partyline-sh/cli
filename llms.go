...
type aiSession struct {
	Tool       string    // claude | codex | gemini | llm
...
func llmsMain(args []string) {
	...
	case "resume", "open":
		...
		aiResume(args[1])
	case "-h", "--help", "help":
		fmt.Println("ptln llms — browse + resume your AI CLI sessions (local)")
		fmt.Println("  ptln llms              interactive session menu (arrow keys; ⏎ resume here, o new tab)")
		fmt.Println("  ptln llms ls [--all]   flat list, newest first (--all shows agent/automated)")
		fmt.Println("  ptln llms resume <id>  jump back into one (id or unique prefix)")
	default:
		// Bare `ptln llms <id>` is a friendly shortcut for `resume <id>`.
		aiResume(sub)
	}
}
...
func aiList(args []string) {
	...
	for _, s := range view {
		...
		fmt.Printf("%-7s  %-6s  %-9s  %-18s  %s%s\n",
			s.Tool, humanAge(s.LastActive), short(s.ID, 8), trunc(proj, 18), trunc(s.Title, 56), tag)
	}
	if hidden > 0 {
...
func claudeSessions(home string) []aiSession {
	...
	for _, f := range files {
		...
		out = append(out, aiSession{
			Tool: "claude", ID: id, Cwd: cwd, Title: title, LastActive: fi.ModTime(),
			storePath: f, resumeDir: cwd, resumeArgv: []string{"claude", "--resume", id},
		})
	}
	return out
}
...
func scanClaude(path string) (cwd, title string) {
	...
	sc.Buffer(make([]byte, 1<<20), 1<<24) // lines can embed file contents — allow up to 16MB
...
func codexSessions(home string) []aiSession {
	...
	_ = filepath.WalkDir(root, func(p string, d os.DirEntry, err error) error {
		...
		out = append(out, aiSession{
			Tool: "codex", ID: id, Cwd: cwd, Title: title, LastActive: fi.ModTime(),
			storePath: p, resumeDir: cwd, resumeArgv: []string{"codex", "resume", id},
		})
		return nil
	})
	return out
