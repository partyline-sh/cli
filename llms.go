...
type aiSession struct {
	...
	// Worktree attribution, filled by attributeProjects (llms_worktree.go).
	ProjDir string // grouping dir: the PARENT repo when Cwd is a git worktree, else Cwd
	WtName  string // worktree basename when Cwd is (or was) a worktree; "" otherwise
	DeadWt  bool   // worktree session whose dir no longer exists — hidden by default, prunable
	// root is the HOME whose store this session was found under. "" or the process's own home =
	// the primary root. Anything else is an ADOPTED root, and resuming it needs HOME pointed
	// there (see resumeEnv) or the engine looks in the wrong place and reports it missing.
	root string
}

func llmsMain(args []string) {
	// Opening the session manager is when the tray earns its keep — it's the moment there are
	// sessions whose "waiting on you" state is worth showing while this terminal is buried. Wakes the
	// SHARED tray (never a second icon); a no-op when one is already running or none is installed.
	wakeTray()
	...
	case "ls", "list":
		aiList(args[1:])
	case "prune":
		// Drop sessions whose git worktree is gone from the LIST only (never from disk).
		llmsPrune(args[1:])
	case "--look-in", "--lookin":
		// Where else to look for sessions. The capability existed; the only door was a modal that
		// appears when you have zero sessions, so anyone with one of their own could never reach it.
		llmsLookIn(args[1:])
	case "--roots", "roots":
		llmsRoots()
	case "--resume", "--restore":
	...
	case "-h", "--help", "help":
		...
		fmt.Println("  ptln llms prune          report sessions whose git worktree is gone (--apply to drop)")
...
func collectSessions() []aiSession {
	...
	var all []aiSession
	// Every configured root, not just this process's home. A session started under a different
	// $HOME (a migration, a service account, a second login) is otherwise invisible rather than
	// merely hard to find — see llms_roots.go.
	for _, r := range loadSessionRoots() {
		home := r.Home
		var mine []aiSession
		mine = append(mine, claudeSessions(home)...)
		mine = append(mine, codexSessions(home)...)
		mine = append(mine, geminiSessions(home, hashToPath(home, mine))...)
		mine = append(mine, antigravitySessions(home)...)
		if !r.Primary {
			for i := range mine {
				mine[i].root = home
			}
	...
	// Provisioned / daemon runs (docs/plans/provisioned-workers.md, P3) execute engines inside
	// partyline-managed dirs (~/.partyline/daemon/repos + their crank worktrees), which deposit real
	// sessions in the engines' own global stores. Those are MACHINE work, not user sessions — hide them
	// from the launcher by cwd PATH prefix. A path signature is immune to the title heuristic
	// (isAgentSession), which already MISSES skill-injected crank sessions once the skill manifest
	// prepends the prompt — so this also closes that pre-existing launcher leak. Applied AFTER cwd
	// overrides so a relocated session is judged by its effective path.
	out := filterManagedSessions(dedup)
	// Attribute git-worktree sessions to their parent repo (and flag dead worktrees) so the
	// switchboard nests them instead of showing one top-level project per worktree. Cached
	// per unique cwd — see llms_worktree.go.
	attributeProjects(out)
	return out
}

// filterManagedSessions drops sessions whose working dir lives under partyline's own state dir
// (~/.partyline/) — daemon/crank engine runs the human never opened and shouldn't see in the mux.
// User sessions (anywhere else, e.g. ~/dev/*) are untouched.
func filterManagedSessions(sessions []aiSession) []aiSession {
	managed := stateDir() + string(os.PathSeparator) // ~/.partyline/…
	out := make([]aiSession, 0, len(sessions))
	for _, s := range sessions {
		if s.Cwd != "" && strings.HasPrefix(s.Cwd, managed) {
			continue
		}
		out = append(out, s)
	}
...
func applyCwdOverrides(sessions []aiSession, meta map[string]sessMeta) {
	for i := range sessions {
		...
		if fi, err := os.Stat(ov); err == nil && fi.IsDir() {
			sessions[i].Cwd = ov
			// resumeDir is what a resume actually chdir's into (aiResume + the mux). The store was
			// migrated to the new cwd (see migrateSessionStore), so point resume there too —
			// otherwise a normal (non-recover) resume would cd back to the gone original.
			if sessions[i].resumeArgv != nil {
				sessions[i].resumeDir = ov
			}
		}
	}
}
...
func aiList(args []string) {
	...
	for _, s := range ss {
		...
		// A session whose git worktree was deleted can't be resumed — hidden by default,
		// see `ptln llms prune`.
		if !showAll && s.DeadWt {
			dead++
			continue
	...
	if dead > 0 {
		fmt.Printf("  %d session(s) in deleted git worktrees hidden — `ptln llms prune` to review\n", dead)
	}
...
func resumeSession(s aiSession) {
	...
	// An adopted root resumes with HOME pointed at it — without this the engine resolves its store
	// from the ambient HOME, finds nothing, and reports a session that plainly exists as missing.
	if err := syscall.Exec(bin, s.resumeArgv, resumeEnv(s.root)); err != nil {
...
func scanClaude(path string) (cwd, title string) {
	...
	defer f.Close()
	// cwd lives in the early lines, so read a bounded PREFIX and skip oversized lines: a huge
	// session file (100s of MB of embedded tool output) must not make this crawl, because
	// `ptln llms` scans EVERY session.
	...
	for _, line := range bytes.Split(buf[:n], []byte{'\n'}) {
		...
		if firstTitle == "" && ev.Type == "user" {
			firstTitle = firstUserText(ev.Message)
		}
		if cwd != "" && firstTitle != "" {
			break
		}
	}
	// Title from the LAST thing you said, not the first. In a long-lived session the opening
	// message is months stale ("read the memory files for this project"), and in a COMPACTED one
	// it isn't yours at all — it's the "This session is being continued…" summary, which renders
	// every compacted session identically. That is how a session becomes unfindable in a list of
	// eight. What you actually remember is the last thing you asked for.
	if t := lastUserTitle(f); t != "" {
		return cwd, t
	}
	return cwd, firstTitle
}

// lastUserTitle reads backward from EOF for the most recent real user message. Bounded like the
// prefix scan; "" when the tail holds only tool results and command echoes (cleanTitle rejects
// those), which leaves the caller on its first-message fallback.
func lastUserTitle(f *os.File) string {
	const tail = 512 << 10
	fi, err := f.Stat()
	if err != nil {
		return ""
	...
	if start > 0 && len(lines) > 0 {
		lines = lines[1:] // the window opened mid-line; that fragment isn't parseable JSON
	}
...
// boilerplateTitlePrefixes are openings that belong to the HARNESS, not to you. The compaction
// summary is the dangerous one: every compacted session starts with the same sentence, so a list
// of long-running sessions renders as identical rows and the only way to tell them apart is to
// open each in turn. That is not a cosmetic problem — it is how someone concludes their work is
// gone when the transcript is sitting on disk, intact.
var boilerplateTitlePrefixes = []string{
	"this session is being continued from a previous conversation",
	"caveat: the messages below were generated by the user",
	"analysis:", // some harnesses replay their own scratchpad as the opening turn
	// Agent-harness injected preambles (source tool unidentified; strings quoted from a real screen):
	// which made whole groups of sessions render as identical walls of policy text.
	"workspace boundary",
	"[structured-output-enforce]",
}

func isBoilerplateTitle(s string) bool {
	low := strings.ToLower(s)
	for _, p := range boilerplateTitlePrefixes {
