...
// repoDirForProof is where the probes run: the session's working directory, which is the repo the
// plan is about. Empty (or unreadable) means we cannot prove anything and must say so rather than
// file on an unproven claim.
func repoDirForProof() string {
	wd, err := os.Getwd()
	if err != nil {
		return ""
	}
