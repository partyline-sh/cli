...
func (f *fakeMux) SessionByKey(key string) (*ptysess.Session, string, string, bool) {
...
func (f *fakeMux) paste(_ *ptysess.Session, block string, submit bool, confirm func() string) bool {
...
func TestDeliverPolicyIsIndependentOfTheOtherTwo(t *testing.T) {
	...
	// And opting into submit must not grant either of the others.
	registerProject(t, dir, "submit")
	p := loadDaemonRegistry().Projects[0]
	if p.launchPolicy() == "auto" {
		t.Fatal("deliver=submit must not grant auto-launch")
	}
