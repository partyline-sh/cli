...
func TestChainThatFinishesReleasesTheCursor(t *testing.T) {
	...
	m.data = pboard(&api.Board{Backlog: []api.BoardCard{card("other")}})
	m.restoreFocus()
...
func TestDetailPaneRereadsItsCard(t *testing.T) {
	...
	m := newBoardModel()
	m.data = pboard(&api.Board{Building: []api.BoardCard{running}})
	m.rememberFocus()
	...
	m.data = pboard(&api.Board{Blocked: []api.BoardCard{failed}})
	m.refreshDetail(nil)
...
func TestReorderRefusesOutsideTheBacklog(t *testing.T) {
	m := newBoardModel()
	m.data = pboard(&api.Board{Review: []api.BoardCard{card("r", withStatus("done"))}})
	m.col = 3
	if m.reorder(nil, -1) {
