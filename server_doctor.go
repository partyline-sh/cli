...
func serverMain(args []string) {
	...
	case "announce": // LAN discovery: advertise this box's instance (installed as a unit by install/update)
		serverAnnounceMain(args[1:])
	case "upgrade", "update": // "update" alias: pull the latest and re-apply the current settings
