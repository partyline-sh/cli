...
// loadTheme restores the saved theme by name (default Midnight). Called once from main, so every
// subcommand — not just the launcher — renders in the chosen theme.
func loadTheme() {
...
// applyThemeToBrand tells internal/brand whether a theme is in play. The wordmark's gradient is
// truecolor by default, which themed() cannot rewrite — so under any theme but the identity one
// it has to fall back to indexed colour for the remap to reach it.
func applyThemeToBrand() { brand.UseIndexedColor(len(theme.remap) > 0) }
...
// accentIdx is the ANSI-256 index every partyline surface uses for "a key you can press", and
// the one the theme tables all remap. 215 is the hint bar's key colour, so a menu built on it
// matches the bar under it in every theme.
const accentIdx = "215"

// ThemedIndex maps a source ANSI-256 index through the active theme.
//
// For surfaces that cannot be re-skinned after rendering: tmux draws its own status ribbon and
// popup borders from the generated conf, long after any Go frame has been written, so those
// colours must be resolved when the conf is written rather than remapped on the way out.
func ThemedIndex(src string) string {
	if r, ok := theme.remap[src]; ok {
		return r
	}
	return src
}

// themeAccent is the accent as an SGR, already themed — for a surface that prints straight to the
// terminal without going through themed().
func themeAccent() string { return "\x1b[38;5;" + ThemedIndex(accentIdx) + "m" }
