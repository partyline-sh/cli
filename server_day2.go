...
func findInstallDir(stat func(string) (os.FileInfo, error)) (string, error) {
	...
	// The installer records where it installed (a custom --dir is invisible to the guesses
	// below — a real box at /mnt/data/partyline made every day-2 command miss). Both records
	// are consulted: the system one survives sudo, the home one needs no root.
	for _, rec := range installDirRecords() {
...
func serverUpgrade(dir string, ops installOps) bool {
	...
	steps := []struct {
		...
		// "installs with the current settings": the same reconcile a re-run of the installer
		// does — env overrides re-applied (site derivations, CA trust), the Caddyfile follows
		// the config when untouched — so an upgrade carries config fixes, not only new images.
		{"re-apply the current settings", func() error {
			cfg := prefillFromExisting(installConfig{dir: dir, bind: "0.0.0.0", httpPort: 80, httpsPort: 443, relayPort: 2222, minio: true, explicit: map[string]bool{}}, ops)
			if cfg.site == "" {
				return nil // nothing recorded yet — a fresh dir; the installer owns this case
			}
			if err := writeInstallEnvOverrides(cfg, ops); err != nil {
				return err
			}
			// An upgrade may add routes; it must NEVER change how TLS is terminated. The mode
			// lives only in the Caddyfile, so re-deriving it from the site URL turned a box
			// behind Cloudflare into one that redirects HTTP to HTTPS on the origin — which the
			// proxy in front follows straight back, serving an infinite loop.
			return reconcileCaddyfile(keepExistingTLSMode(cfg, ops), ops)
		}},
		...
		{"restart the stack", func() error {
			...
			// `up -d` does not restart a container whose service definition is unchanged, and the
			// Caddyfile is a mounted FILE — so the rewrite two steps above lands on disk while
			// Caddy keeps serving the old config. The install path has reloaded for this reason
			// for a while; the upgrade path did not, which made every routing change an upgrade
			// claimed to apply silently absent from the running edge.
			//
			// Non-fatal: the stack is up and migrated, and the health probe below reports the
			// edge. An install with no bundled edge has no caddy service, and restarting a
			// service that is not there is not a reason to fail an otherwise clean upgrade.
			if _, err := ops.run(dir, "docker", "compose", "restart", "caddy"); err != nil {
				fmt.Fprintf(ops.out, "(could not reload the edge) ")
			}
			return nil
		}},
		...
		{"point the sign-in client at the site", func() error {
			// Non-fatal by design: an upgrade that pulled, restarted and migrated cleanly
			// must not report failure because the sign-in readback raced Keycloak's boot.
			// The message names the exact re-run.
			cfg := prefillFromExisting(installConfig{dir: dir, explicit: map[string]bool{}}, ops)
			if cfg.site == "" {
				return nil
			}
			if err := kcadmSyncSignIn(dir, cfg.site, ops); err != nil {
				fmt.Fprintf(ops.out, "\n  ⚠ %v — sign-in may still point at an old address; re-run: ptln server update\n  ", err)
			}
			return nil
		}},
	}
...
func probeHealth(dir string, ops installOps) string {
	...
	code, err := pollHTTP(ops, url, 45*time.Second)
	if err == nil && code == http.StatusOK {
		return ""
	}
	// A tunnel-fronted install (Cloudflare/Tailscale terminates TLS; Caddy serves plain HTTP)
	// has NOTHING on the HTTPS port — probing only there reported a healthy stack as broken
	// after every upgrade. The question is "is the stack serving", and the plain door answers it.
	httpPort := readEnvValue(env, "HTTP_PORT")
	if httpPort == "" {
		httpPort = "80"
	}
	plainURL := "http://127.0.0.1:" + httpPort + "/api/health"
	if code2, err2 := pollHTTP(ops, plainURL, 45*time.Second); err2 == nil && code2 == http.StatusOK {
		return ""
	}
	if err != nil {
		return fmt.Sprintf("the stack restarted but neither %s nor %s responded: %v\nLogs: cd %s && docker compose logs --tail=50", url, plainURL, err, dir)
	}
	return fmt.Sprintf("%s answered %d, want 200 (and %s did not answer 200 either)\nLogs: cd %s && docker compose logs --tail=50", url, code, plainURL, dir)
}
...
// installDirRecords: where the installer notes its --dir, in the order day-2 commands read
// them. /etc survives `sudo ptln server install` (root's $HOME is not the operator's); the
// stateDir record covers user-mode installs that cannot write /etc. Both are best-effort.
var installDirRecords = defaultInstallDirRecords

func defaultInstallDirRecords() []string {
	recs := []string{"/etc/partyline/install-dir"}
	recs = append(recs, filepath.Join(stateDir(), "server-dir"))
	// Under sudo, the operator's OWN record is the one their unprivileged commands will read
	// — write/read it too, not only root's.
	if su := strings.TrimSpace(os.Getenv("SUDO_USER")); su != "" {
		if uh, err := userHomeOf(su); err == nil {
			recs = append(recs, filepath.Join(uh, ".partyline", "server-dir"))
		}
	}
	return recs
}

// recordInstallDir writes the install's directory into every record location it can.
func recordInstallDir(dir string) {
	_ = os.MkdirAll("/etc/partyline", 0o755)
	for _, rec := range installDirRecords() {
		_ = os.MkdirAll(filepath.Dir(rec), 0o755)
		_ = os.WriteFile(rec, []byte(dir+"\n"), 0o644)
	}
}

// userHomeOf resolves a username's home without shelling out to a login shell: the passwd
// database via os/user.
func userHomeOf(name string) (string, error) {
...
// localInstanceSite: the SITE_URL of an install hosted on THIS box, "" when there is none.
// Read via the same discovery day-2 commands use, so a custom --dir is found through its
// record. The value is a public URL, never a secret.
func localInstanceSite() string {
	dir, err := findInstallDir(os.Stat)
	if err != nil {
		return ""
	}
	...
	return strings.TrimRight(envValue(string(b), "SITE_URL"), "/")
}
