...
// defaultInstallDir is where partyline.sh's own boxes keep the stack. It is a DEFAULT, not an
// assumption: a self-hoster puts the install where they want it, and PARTYLINE_DIR says where.
// scripts/env-bootstrap.sh reads the same variable, so the two agree about the same box.
const defaultInstallDir = "/opt/partyline"

// installPath is where the stack lives on THIS box. Every path in the printed plan is under it.
func installPath(p bootstrapProbe) string {
	if p.installDir == nil {
		return defaultInstallDir
	}
	if d := strings.TrimRight(strings.TrimSpace(p.installDir()), "/"); d != "" {
		return d
	}
	return defaultInstallDir
}

// liveInstallDir reads PARTYLINE_DIR, the same variable scripts/env-bootstrap.sh reads, so the
// plan this command prints and the script that writes .env agree about which directory is the
// install. Unset means the path partyline.sh's own boxes use.
func liveInstallDir() string { return os.Getenv("PARTYLINE_DIR") }

// stackPorts is what the stack will try to publish on THIS box, which is a question about its
// configuration and not a constant. It used to be `[]int{80, 443, 2222}` — three literals — so on
// a box already running something on :80 the report said "change the port in docker-compose.yml"
// and there was no port to change: 80 and 443 were hardcoded in the compose file. Both are
// variables now, and this reads them.
//
// Read from THIS PROCESS's environment, not from the box's .env — the env file reader deliberately
// returns names and never values. `set -a; . <dir>/.env; set +a` first, which is what the docs
// already say to do before `ptln server doctor`.
//
// These ARE printed, which is safe only because every one of them is parsed as a port number and
// anything else falls back to the default — a value that is not 1..65535 cannot reach the output.
func stackPorts(p bootstrapProbe) []int {
	port := func(name string, def int) int {
		v := strings.TrimSpace(p.look(name))
		if v == "" {
			return def
		}
	...
	// CADDY_REPLICAS=0 is the no-reverse-proxy mode (docker-compose.direct.yml): Caddy publishes
	// nothing and the app and Keycloak publish their own ports instead. Checking 80 and 443 there
	// would warn about ports this box is never going to bind.
	if strings.TrimSpace(p.look("CADDY_REPLICAS")) == "0" {
		return []int{port("WEB_PORT", 8080), port("KEYCLOAK_PORT", 8081), port("RELAY_PORT", 2222)}
	}
	return []int{port("HTTP_PORT", 80), port("HTTPS_PORT", 443), port("RELAY_PORT", 2222)}
}
...
type bootstrapProbe struct {
	...
	// installDir is where the operator wants the stack. Its own probe rather than a `look` of
	// PARTYLINE_DIR, because this one is PRINTED — every path in the plan is under it — and the
	// invariant `look` protects is that nothing it returns reaches the output.
...
func renderServerBootstrap(w io.Writer, p bootstrapProbe, asJSON bool) bool {
	...
	plan := bootstrapPlan(installPath(p), stack, missing)
	...
	fmt.Fprintln(w)
	fmt.Fprintln(w, "PLAN — run these in order, on the box, as a user who can write "+installPath(p)+":")
	fmt.Fprintln(w)
	n := 0
...
func checkDocker(p bootstrapProbe, report reportFn) {
	...
	// THE DAEMON'S VERSION, NOT THE CLIENT'S. `docker --version` reports the CLI, and the two are
	// routinely far apart — a 29.x client against a 20.10 daemon reports "29.2" here and then fails
	// every real call with "client version is too new. Maximum supported API version is 1.41". The
	// number that decides whether this box can run the stack is the server's, so ask the server and
	// fall back to the client only when the daemon cannot be reached (which the next check reports).
	out, ran := p.run("docker", "version", "--format", "{{.Server.Version}}")
	if !ran || strings.TrimSpace(out) == "" {
...
func checkPorts(p bootstrapProbe, report reportFn) {
	for _, port := range stackPorts(p) {
		...
		report(ckWarn, what, "something is already listening",
			fmt.Sprintf("find it with `sudo lsof -i :%d` and stop it, or move partyline: set HTTP_PORT / HTTPS_PORT / RELAY_PORT in %s/.env (nothing hardcodes a port)", port, installPath(p)))
	}
}

func checkDisk(p bootstrapProbe, report reportFn) {
	installDir := installPath(p)
	// df, not syscall.Statfs: one read-only command that works the same on every box the stack
	// runs on, and no build tags.
	out, ran := p.run("df", "-Pk", diskProbeDir(installDir))
	if !ran {
	...
	if freeGB < minDiskGB {
		report(ckFail, "free disk", fmt.Sprintf("%d GB free on %s — the images and the database want at least %d GB", freeGB, diskProbeDir(installDir), minDiskGB),
			"free space, or attach a larger volume, before pulling the images")
		return
	}
	report(ckPass, "free disk", fmt.Sprintf("%d GB on %s", freeGB, diskProbeDir(installDir)), "")
}
...
var stackFiles = []string{"docker-compose.yml", "env.example", "apply-migrations.sh", "env-bootstrap.sh"}
...
var notOnTheBox = map[string]string{
	// The listen knobs. Every one of them has a default that IS today's behaviour, so demanding
	// them would turn a working box into a failing report on the strength of a variable nobody
	// needs to set.
	"BIND_ADDR":                     "compose defaults it to 0.0.0.0; set it only to bind one interface",
	"CADDY_IMAGE":                   "compose defaults it to caddy:2-alpine; set it only for a build with DNS provider modules",
	"NODE_EXTRA_CA_CERTS":           "compose defaults it to empty; the installer writes it only in tls-internal mode",
	"CADDY_REPLICAS":                "compose defaults it to 1; set 0 only for the no-reverse-proxy mode",
	"HTTP_PORT":                     "compose defaults it to 80; set it only when something else owns that port",
	"HTTPS_PORT":                    "compose defaults it to 443; set it only when something else owns that port",
	"KEYCLOAK_PORT":                 "only read by docker-compose.direct.yml, where compose defaults it",
	"OIDC_PUBLIC_URL":               "compose defaults it to SITE_URL/auth; env-bootstrap.sh writes it when they differ",
	"RELAY_PORT":                    "compose defaults it to 2222; set it only to move the relay",
	"WEB_PORT":                      "only read by docker-compose.direct.yml, where compose defaults it",
	"NEXT_PUBLIC_SUPABASE_ANON_KEY": "the legacy fallback name; env-bootstrap.sh writes PGRST_ANON_KEY, which is the one to set",
	"NEXT_PUBLIC_SUPABASE_URL":      "the legacy fallback name; env-bootstrap.sh writes PGRST_URL, which is the one to set",
	// Same two reasons as the entries above and below, applied to the siblings that were missed.
	// `ptln server install` found them: its .env assertion demanded all six on a box where
	// env-bootstrap.sh had just written a complete, working .env, so bootstrap had been telling
	// every self-hoster to go and set six variables that either default or have no effect.
	"NEXT_PUBLIC_PARTYLINE_ENV": "baked into the web image at build time; compose also sets it on the service",
	"NEXT_PUBLIC_SITE_URL":      "baked into the web image at build time on the CI runner",
	"MINIO_REPLICAS":            "compose defaults it to 1; set 0 only when S3_* points at R2 or S3 instead",
	"RELAY_IMAGE":               "compose defaults it; set it only to pin a digest or use your own mirror",
	"RELAY_TAG":                 "compose defaults it; set it only to pin or roll back an image",
	"WEB_IMAGE":                 "compose defaults it; set it only to pin a digest or use your own mirror",
...
func bootstrapPlan(installDir, stack string, missing []string) []string {
	plan := []string{"sudo mkdir -p " + installDir + " && sudo chown $USER " + installDir}
	if stack == "" {
		plan = append(plan,
			// The stack IS published now — this used to say it was not, and sent people to scp a
			// directory off another machine instead of downloading it.
			// Named, not fetched: this command only ever names artifacts it has checked exist on
			// this machine, so it tells you WHERE the bundle lives and stops. The stack IS
			// published now — this used to say it was not, and sent people to scp it off another box.
			"get the self-host bundle onto this box — it is published at partyline.sh/self-host",
			"    (docker-compose.yml, env-bootstrap.sh, apply-migrations.sh, 00-bootstrap.sh, migrations.tar.gz)",
			"    then run `ptln server bootstrap` again from beside them: the next run finds the files",
			"    and prints the exact commands, which this run cannot name honestly.")
	} else {
		plan = append(plan,
			"cp "+stack+"/docker-compose.yml "+installDir+"/docker-compose.yml",
			"cp "+stack+"/apply-migrations.sh "+installDir+"/apply-migrations.sh",
			// NOT `cp env.example .env`. env.example says, in its own header, DO NOT COPY THIS FILE
			// TO .env — because env-bootstrap.sh only ever ADDS what is missing, so every line
			// copied from the reference counts as already set and is left blank forever. This plan
			// used to instruct exactly that, which silently disabled secret generation on a fresh
			// box and produced a stack that came up with empty credentials.
			"cp "+stack+"/env-bootstrap.sh "+installDir+"/env-bootstrap.sh",
			"cd "+installDir+" && ./env-bootstrap.sh https://your-hostname   # writes .env: generates every secret and MINTS the PostgREST JWTs")
		// env-bootstrap.sh installs into the directory it is sitting in, so the `cd` above is
		// what makes it agree with this plan. It is also the port/TLS seam: --no-proxy writes a
		// no-Caddy, plain-HTTP configuration for a private network.
		plan = append(plan,
			"    (a URL with a port — https://host:8443 — moves Caddy's ports; add --no-proxy for a",
			"     no-TLS install on a private network, then bring it up with BOTH compose files:",
			"     docker compose -f docker-compose.yml -f docker-compose.direct.yml up -d)")
	}
	// env-bootstrap.sh WRITES the .env (secrets generated, JWTs minted); what is left for a person
	// is the handful of real credentials it cannot know. A box with no stack files has not run it yet.
	envFile := installDir + "/.env"
	if stack == "" {
		envFile = installDir + "/.env (once env-bootstrap.sh has written it)"
	}
	if len(missing) == 0 {
	...
	return append(plan,
		...
		"./apply-migrations.sh            # halts on the first failure; nothing else starts until it passes",
...
func diskProbeDir(installDir string) string {
	dir := installDir
	for dir != "/" && dir != "." {
