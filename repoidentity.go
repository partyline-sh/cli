...
var (
	...
	// The user@ is OPTIONAL: `github.com:owner/repo.git` is a valid remote and names the same
	// repository as `git@github.com:owner/repo.git`. Requiring the prefix normalised the two to
	// "github.com:owner/repo" and "github.com/owner/repo" — a colon against a slash — so one
	// teammate's clone did not match another's, and facts about that repo failed to line up.
	// Guarded by the "://" check at the call site so a URL scheme is not read as a host.
...
func normalizeRemote(raw string) string {
	...
	if m := scpRemoteRe.FindStringSubmatch(s); m != nil && !strings.Contains(s, "://") {
		s = m[1] + "/" + m[2]
	} else {
