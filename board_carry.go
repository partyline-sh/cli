...
func (m *boardModel) carryCommit(c *api.Client) bool {
	...
	if card.Foreign {
		...
		m.focusID = card.ID
		scope := m.scope
		m.asyncWrite("reordered — "+m.data.Source+" has it", func() error {
			for _, w := range writes {
				if err := orderer.SetCardSeq(scope, w.id, w.seq); err != nil {
					return err
				}
			}
			return nil
...
// foreign board: write one card's row-order value. Odoo implements it with the kanban
// `sequence` through the same curated update tool as stage moves.
type boardOrderer interface {
	SetCardSeq(scope, cardID string, seq int) error
}
...
func reorderWrites(ordered []api.BoardCard, movedID string) []seqWrite {
	...
	switch {
	case at == 0:
		return []seqWrite{{movedID, ordered[1].Seq - 10}}
	case at == len(ordered)-1:
		return []seqWrite{{movedID, ordered[at-1].Seq + 10}}
	default:
		prev, next := ordered[at-1].Seq, ordered[at+1].Seq
		if next-prev >= 2 {
			return []seqWrite{{movedID, prev + (next-prev)/2}}
		}
	}
	// No gap: renumber the whole column at 10-step spacing, writing only what changes.
	var out []seqWrite
	for i, card := range ordered {
		want := (i + 1) * 10
		if card.Seq != want {
			out = append(out, seqWrite{card.ID, want})
		}
	}
	return out
}
