...
func (m *boardModel) carryCommit(c *api.Client) bool {
	...
	// Within the column: a foreign board reorders through its own order field when the
	// source can write one (Odoo: kanban sequence); without that capability the old truth
	// stands and the toast says so.
	if card.Foreign {
		orderer, can := m.sources[m.src].(boardOrderer)
		if !can {
			m.setToast(m.data.Source+" owns its task order — stages move, rows don't", false)
			return false
		}
		var ordered []api.BoardCard
		for _, r := range rows { // rows was captured carry-aware: this IS the preview order
			if r.Card != nil {
				ordered = append(ordered, *r.Card)
			}
		}
		writes := reorderWrites(ordered, card.ID)
		if len(writes) == 0 {
			m.setToast("put back — nothing changed", false)
			return false
		}
		m.applyLocalOrder(target, ordered, writes)
		m.focusID = card.ID
		scope := m.scope
		label := "reordered — " + m.data.Source + " has it"
		for _, w := range writes {
			if w.id == card.ID && w.pri != "" {
				// Crossing a band writes priority — say so, always: a drag that silently
				// changes a business field is worse than one that refuses.
				label = "reordered — priority set to " + odooPriLabel(w.pri) + " to hold the spot"
			}
		}
		m.asyncWrite(label, func() error {
			for _, w := range writes {
				if err := orderer.SetCardSeq(scope, w.id, w.seq, w.pri); err != nil {
					return err
				}
			}
			return nil
...
// boardOrderer is the optional provider capability behind within-column reordering of a
// foreign board: write one card's row-order value — and, when the drop crossed a priority
// band, the priority that makes the position real (pri == "" leaves it untouched). Odoo
// orders kanban by priority FIRST, sequence second (verified live), so sequence alone can
// never lift a card past a higher band.
type boardOrderer interface {
	SetCardSeq(scope, cardID string, seq int, pri string) error
}

// seqWrite is one row-order write the drop decided on.
type seqWrite struct {
	id  string
	seq int
	pri string // non-empty: also set the priority band (the drop crossed one)
}

// reorderWrites turns a previewed order into the fewest sequence writes that make it real.
// `ordered` is the column exactly as previewed (top first); Seq values are the CURRENT ones.
// One write when the moved card fits a gap between its new neighbours; a 10-step renumber of
// every out-of-place card when there is no gap — which is the common case on a fresh Odoo,
// where every task ships with sequence 10 and "between 10 and 10" does not exist.
func reorderWrites(ordered []api.BoardCard, movedID string) []seqWrite {
	at := -1
	for i := range ordered {
	...
	if at < 0 || len(ordered) < 2 {
		return nil
	}

	// The destination band. A band boundary is one visual slot with two names — bottom of
	// the upper band and top of the lower — so the card's OWN band wins whenever it matches
	// either neighbour: no silent promotion or demotion for a drop that already displays
	// where it landed. Only a slot whose neighbours both disagree with the card forces a
	// priority write (announced in the toast), because the source ranks bands above
	// sequence and the previewed position would otherwise be a lie.
	moved := ordered[at]
	abovePri, belowPri := "", ""
	hasAbove, hasBelow := at > 0, at+1 < len(ordered)
	if hasAbove {
		abovePri = ordered[at-1].Pri
	}
	if hasBelow {
		belowPri = ordered[at+1].Pri
	}
	band := moved.Pri
	switch {
	case hasAbove && moved.Pri == abovePri, hasBelow && moved.Pri == belowPri:
		// own band touches the slot — keep it
	case hasAbove:
		band = abovePri
	case hasBelow:
		band = belowPri
	...
	// Sequence neighbours are the nearest band-mates: a band boundary is not a sequence
	// neighbour, whatever its Seq happens to be.
	prev, next := -1, -1 // indexes into ordered
	for i := at - 1; i >= 0; i-- {
		if ordered[i].Pri == band {
			prev = i
			break
		}
	}
	for i := at + 1; i < len(ordered); i++ {
		if ordered[i].Pri == band {
			next = i
			break
		}
	...
	switch {
	case prev < 0 && next < 0: // alone in its band — any sequence displays the same
		if pri == "" {
			return nil
		}
		return []seqWrite{{movedID, moved.Seq, pri}}
	case prev < 0:
		return []seqWrite{{movedID, ordered[next].Seq - 10, pri}}
	case next < 0:
		return []seqWrite{{movedID, ordered[prev].Seq + 10, pri}}
	default:
		if gap := ordered[next].Seq - ordered[prev].Seq; gap >= 2 {
			return []seqWrite{{movedID, ordered[prev].Seq + gap/2, pri}}
		}
	}
	// No gap: renumber the BAND at 10-step spacing, writing only what changes. (The common
	// case on a fresh Odoo, where every task ships with sequence 10.)
	var out []seqWrite
	n := 0
	for i, card := range ordered {
		if card.Pri != band && i != at {
			continue
		}
		n++
		want := n * 10
		w := seqWrite{card.ID, want, ""}
		if i == at {
			w.pri = pri
			if card.Seq == want && pri == "" {
				continue
			}
		} else if card.Seq == want {
			continue
		}
		out = append(out, w)
	}
...
// applyLocalOrder makes the previewed order the displayed order before the writes land.
func (m *boardModel) applyLocalOrder(col api.BoardColumn, ordered []api.BoardCard, writes []seqWrite) {
	if m.data == nil {
		return
	}
	seqs := map[string]int{}
	...
	pris := map[string]string{}
	for _, w := range writes {
		if w.pri != "" {
			pris[w.id] = w.pri
		}
	}
	out := make([]api.BoardCard, len(ordered))
	for i, card := range ordered {
		if s, ok := seqs[card.ID]; ok {
			card.Seq = s
		}
		if p, ok := pris[card.ID]; ok {
			card.Pri = p
		}
		out[i] = card
	}
	m.data.ByColumn[col] = out
}
