...
type machineState struct {
	...
	// Finished are runs that ENDED on this machine recently, so the tray can say so. Read from the
	// daemon's local record, never fetched — same network-free contract as Invites below. Omitted
	// when empty, so an older tray and a quiet machine look identical, which is correct.
	Finished []finishedRun `json:"finished,omitempty"`
	// Invites are live sessions a teammate opened and asked you into. Read from the daemon's cache
	// file, never fetched here — `state` runs every 4s from the tray and must stay network-free.
	// Omitted entirely when empty, so an older tray (and any other reader) is unaffected.
	Invites []inviteRow `json:"invites,omitempty"`
	...
	// Projects are the canonical projects this machine's owner can see, read from the daemon's
	// cache file (autoadopt.go) — never fetched here, same network-free contract as Invites.
	// Names and flags only, no paths. Omitted when the daemon has never written the cache, so an
	// older daemon and an empty team look identical to the tray.
	Projects []canonicalProject `json:"projects,omitempty"`
}
...
func currentMachineState() machineState {
	...
	ms.Invites = readInviteCache(time.Now())
	ms.Finished = recentFinishedRuns()
	if c, ok := readCanonicalCache(); ok {
		ms.Projects = c.Projects
	}
