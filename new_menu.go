...
// a terminal. Three doors, each opens a NEW mux tab (SetPendingOpen):
...
//  2. an autonomous task — spawns `ptln work …` as a tab you watch.
//  3. crank a backlog — spawns `ptln crank --file …` as a tab you watch.
//
// Uses the cg_menu palette. (2) and (3) run our own binary as a child, so no nested mux.
func newRunMenu(mx *ptymux.Mux) {
	...
	cgItem("2", "autonomous task", "one task, sandboxed worktree (ptln work)")
	cgItem("3", "crank a backlog", "drain a task file one at a time (ptln crank)")
