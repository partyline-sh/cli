...
func newRunMenu(mx sessionMenuTarget) {
	in := stdin()
	dir, _ := os.Getwd()
	...
	cgItem("5", "health check", "verify daemon · engines · MCP wiring (ptln daemon doctor)")
	cgHintPrint("NEW")
...
func newShellFlow(mx sessionMenuTarget, in *bufio.Reader, dir string) {
...
func newSessionFlow(mx sessionMenuTarget, in *bufio.Reader, dir string) {
	...
	// Permissions, from the same table the share picker uses. The safe default is first and
	// Enter keeps it; the danger mode says what it costs on the line where it is chosen.
	var permFlags []string
	if modes := permissionModes(tool); modes != nil {
		fmt.Println("\n  permissions:")
		pn, ok := Pick("number", modes, func(pm permMode) string {
			if pm.danger {
				return pm.label + "  (runs with NO prompts at all)"
			}
			return pm.label
		})
		if !ok {
		...
		switch {
		case modes[pn].custom:
			raw, ok := Input("flags", "")
			if !ok {
				return
			}
			permFlags = strings.Fields(raw)
		default:
			permFlags = modes[pn].flags
		}
	}
	...
	spec, err := newSessionSpec(tool, dir, thread, worktree, goal, false, thread == "", keepGoing, permFlags...)
	if err != nil {
		fmt.Printf("\n  %s✗ %v%s\n", cgBad, err, cgOff)
		pause(in)
		return
...
// doctorFlow (E9 slice 3): the health check, one door deep instead of one flag deep. Doctor's
// report is the answer to "why isn't this working" — surfacing it here means the person at the
// launcher with a broken setup finds it without knowing the subcommand exists. The tab holds
// after the report (doctor itself exits in about a second, and a tab that vanishes in under 5s
// reads as a failed launch with the report unread).
func doctorFlow(mx sessionMenuTarget) {
	// The exe path travels as $0, not interpolated into the script — a home directory with a
	// space in it must not break the health check meant to diagnose broken setups.
	mx.SetPendingOpen(ptymux.Spec{
		Label: "✚ doctor",
		Key:   fmt.Sprintf("doctor-%d", time.Now().UnixNano()),
		Argv:  []string{"/bin/sh", "-c", `"$0" daemon doctor; printf '\n  press enter to close '; read -r _`, selfExe()},
	})
}

func newWorkFlow(mx sessionMenuTarget, in *bufio.Reader, dir string) {
	fmt.Println()
	task, ok := Input("task", "")
	if !ok {
		return
...
func keepGoingToggleMenu(mx sessionMenuTarget) {
	in := stdin()
...
func newCrankFlow(mx sessionMenuTarget, in *bufio.Reader, dir string) {
	fmt.Println()
