...
type pickerOverlay struct {
	...
	// filter narrows the list as you type, entered with "/". Arrowing through seventy-nine Odoo
	// projects to reach the one you want is not navigation, it is a chore — and that is the real
	// size of a tracker's project list, not a hypothetical one.
	filter    string
	filtering bool
}
...
func (o *pickerOverlay) footer() string {
	if o.filtering {
		return boardCol + "/" + o.filter + "▏" + reset + boardDim + "  ↑↓ move · ⏎ choose · esc cancel" + reset
	}
	return boardDim + "↑↓ move · / filter · ⏎ choose · esc cancel" + reset
}

// visible is the list after filtering: a case-insensitive substring match on the label, which is
// what people actually remember about a project.
func (o *pickerOverlay) visible() []pickerItem {
...
// clamp keeps the cursor inside the filtered list as it shrinks under the typing.
func (o *pickerOverlay) clamp() {
	n := len(o.visible())
	if o.sel >= n {
		o.sel = max(0, n-1)
	}
...
func (o *pickerOverlay) lines(m *boardModel, w, h int) []string {
	...
	if len(items) == 0 {
		if o.filter != "" {
			return []string{boardDim + "  nothing matches " + o.filter + reset}
		}
	...
	if o.top > max(0, len(items)-h) {
		o.top = max(0, len(items)-h)
	}
	end := min(len(items), o.top+h)

	// The label column is sized from the BOX, not a constant. It was a flat 24 columns whatever the
	// terminal, which is fine for "monolith" and cuts a real Odoo or Jira project name in half —
	// and the note beside it ("48 open") never needed the other fifty. Notes are short by nature, so
	// give them what they use and let the name have the rest.
	noteW := 0
	for _, it := range items {
		if n := visWidth(it.Note); n > noteW {
			noteW = n
		}
	}
	if noteW > maxPickerNote {
		noteW = maxPickerNote
	}
	labelW := w - 2 - noteW // marker, then the note's column
	if noteW > 0 {
		labelW -= 2 // a gap, only when there is something to separate from
	}
	if labelW < minPickerLabel {
		labelW = minPickerLabel
	}

	var out []string
	for i, it := range items[o.top:end] {
		i += o.top
		...
		row := marker + padVis(label, labelW)
		if it.Note != "" {
			row += "  " + boardDim + clipVis(it.Note, noteW) + reset
		}
		out = append(out, row)
	}
	return out
...
// What a note is allowed to take, and what a name keeps even in a cramped box. A note is a count or
// a parent directory; a name is the thing being chosen, so it wins ties.
const (
	maxPickerNote  = 22
	minPickerLabel = 16
)
...
func (o *pickerOverlay) key(b []byte, m *boardModel, c *api.Client) (bool, bool) {
	...
	// While filtering, letters are the filter — j and k included, or "jira" could never be typed.
	if o.filtering {
		switch {
		case b[0] == 0x7f || b[0] == 0x08: // backspace
			if o.filter == "" {
				o.filtering = false
			} else {
				r := []rune(o.filter)
			...
			return false, false
		case b[0] == '\r' || b[0] == '\n':
			if o.sel < 0 || o.sel >= len(items) {
				return true, false
			}
			return true, o.onPick(m, c, items[o.sel])
...
func (m *boardModel) helpOverlay() {
	m.openOverlay(&noticeOverlay{heading: "keys", body: []string{
		...
		"  m              grab the card — arrows carry it, ⏎ drops it, esc puts it back",
		"                 (Backlog: ↑↓ reorders priority · other columns: a real transition)",
		"  e              edit its title",
		"  c              comment — an internal note on a foreign card's thread",
