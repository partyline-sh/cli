...
func threadSetVis(c *api.Client, args []string, vis string) {
	...
	fmt.Println("  \x1b[38;5;245mtip: `ptln thread share <id> --team <slug>` to share with a specific team\x1b[0m")
	fmt.Println("  \x1b[38;5;245m(a personal thread shared this way is still visible only to you — pick a team)\x1b[0m")
}
...
// threadDisconnect is the inverse of threadConnect: it removes the persistent common-ground MCP
...
func threadDisconnect(args []string) {
	...
	switch engine {
	case "claude":
		cerr = run("claude", "mcp", "remove", "-s", "user", "common-ground")
	case "codex":
		cerr = run("codex", "mcp", "remove", "common-ground")
	case "gemini":
		cerr = run("gemini", "mcp", "remove", "common-ground")
	case "antigravity", "agy":
		if cerr = run("claude", "mcp", "remove", "-s", "user", "common-ground"); cerr == nil {
	...
	fmt.Printf("✓ %s disconnected from Common Ground (recall/remember removed).\n", engine)
}
