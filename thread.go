...
func threadUsage() {
	...
	fmt.Println("  ptln thread remember <id> <kind> \"<fact>\" [--supersedes <#>]")
	...
	fmt.Println("  ptln thread graduate <id> <#> <project>      promote a fact into the project canon")
...
func threadRemember(c *api.Client, args []string) {
	...
	for i := 0; i < len(args); i++ {
		if args[i] == "--supersedes" || args[i] == "--supersede" {
			if i++; i < len(args) {
	...
	if supersedes > 0 {
		fmt.Printf("✓ remembered (%s, #%d) — supersedes #%d\n", b.Kind, b.ID, supersedes)
	} else {
...
func threadGraduate(c *api.Client, args []string) {
	...
	if len(pos) < 3 {
		fatal(fmt.Errorf("usage: ptln thread graduate <thread-id> <#> <project-id> [--supersedes <#>]"))
	}
	...
	fmt.Printf("✓ graduated to canon (%s, #%d) — the thread block is now marked graduated\n", b.Kind, b.ID)
}
