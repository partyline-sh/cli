...
func threadAttach(c *api.Client, args []string) {
	...
	// Say what this did and — just as important — what it did NOT do. Attaching governs canon
	// INHERITANCE; it does not change which thread the project RESOLVES to. Reading "✓ attached"
	// as "the project now uses this thread" is a natural and wrong inference, and it cost real time.
	fmt.Println("✓ attached — this thread now inherits the project's facts.")
	fmt.Println("  This does NOT change which thread the project resolves to.")
	fmt.Println("  For that: ptln thread use " + args[0] + " " + args[1])
}

// threadUse points a PROJECT at a thread (threads.project_id) — the link that decides what a
// checkout resolves to. Separate verb from `attach` because they are separate links: `attach` grants
// canon inheritance, `use` decides identity.
//
//	ptln thread use <thread-id> <project-id>
func threadUse(c *api.Client, args []string) {
	if len(args) < 2 {
		fatal(fmt.Errorf("usage: ptln thread use <thread-id> <project-id>\n" +
			"  makes the project RESOLVE to this thread (`ptln project show <project-id>` names both)"))
	}
	if err := c.SetThreadProject(args[0], args[1]); err != nil {
		fatal(err)
	}
	fmt.Println("✓ the project now resolves to this thread — re-run `ptln doctor` in the repo to confirm")
}
