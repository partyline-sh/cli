...
func restartService() error {
	...
	case "darwin":
		...
		return nil
	case "linux":
		if err := exec.Command("systemctl", "--user", "restart", "partyline-daemon").Run(); err != nil {
			return fmt.Errorf("systemctl --user restart partyline-daemon failed: %w", err)
		}
		return nil
	}
...
func stopService() error {
	...
	case "darwin":
		...
		return nil
	case "linux":
		if err := exec.Command("systemctl", "--user", "stop", "partyline-daemon").Run(); err != nil {
			return fmt.Errorf("systemctl --user stop partyline-daemon failed: %w", err)
		}
		return nil
	}
