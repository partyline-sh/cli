...
// doctorMCPWiring prints the per-engine wiring rows. Only engines actually installed on this
// machine are checked — reporting an engine the user doesn't have is noise.
func doctorMCPWiring() {
	...
	for _, p := range probes {
		...
		status.line(p.name, detail, p.fix)
	}
