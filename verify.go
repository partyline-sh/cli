...
// runReviewer runs the independent reviewer gate. ran=false when the gate is off (no
// .partyline/review). Otherwise it always yields a verdict — fail-closed on empty diff, engine
// error, timeout, or an unparseable reply.
func runReviewer(baseRepo, wtPath, task string, timeout time.Duration) verifyResult {
	...
	// A fresh claude with NO tools and NO thread — it only reasons over the text we give it, so it
	// can't edit the branch and shares no context with the producer. (A different engine would be
	// stronger; claude-fresh is the available default.)
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	cmd := exec.CommandContext(ctx, "claude", "-p", reviewerPrompt(task, diff, rubric), "--output-format", "json", "--allowedTools", "")
	cmd.Dir = wtPath
	cmd.Env = append(os.Environ(), "PARTYLINE=1") // no thread wiring — independence from the producer
	out, err := cmd.Output()
	if ctx.Err() == context.DeadlineExceeded {
	...
	reply, _, ok := parseWorkerOutput(out)
	if !ok {
		reply = string(out)
	}
...
// verifyTask runs the full two-layer gate for a committed task: the objective executable checks
// (T2a) first, then the independent reviewer (T2b). Either failing quarantines the task (returns a
// failed result); if neither is enabled the result is a no-op (ran=false → merge proceeds). The
// objective layer runs first so a broken build never spends reviewer tokens.
func verifyTask(baseRepo, wtPath, task string, timeout time.Duration) verifyResult {
	if checks := runChecks(wtPath, readChecks(baseRepo), timeout); checks.ran && !checks.ok {
		return checks
	} else if rev := runReviewer(baseRepo, wtPath, task, timeout); rev.ran && !rev.ok {
		return rev
	} else {
		return verifyResult{ran: checks.ran || rev.ran, ok: true}
	}
}
