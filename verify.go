...
// G.4: checks arrive RESOLVED — repo-declared commands with the project's policy already applied,
// so this function never consults settings and could not run a server-supplied command if one
// existed. Two policy outcomes change what happens here rather than upstream, because both must be
// RECORDED and a dropped check is a silence:
//
//   - Skipped (a path glob the task's changes did not touch) → not run, reported as a warn.
//   - Advisory (Blocking false) → RUN, and its failure is a warn instead of a quarantine. This is
//     the check that cannot pass yet: watched, never gating.
func runChecks(baseRepo, wtPath string, checks []resolvedCheck, timeout time.Duration) verifyResult {
	if len(checks) == 0 {
	...
	for _, c := range checks {
		if c.Skipped != "" {
			warns = append(warns, fmt.Sprintf("check %q not run: %s", c.Name, c.Skipped))
			continue
		}
		ran = true
		cmd := c.Cmd
		...
		if timedOut {
			if !c.Blocking {
				warns = append(warns, fmt.Sprintf("advisory check %q timed out (>%s): %s", c.Name, timeout, cmd))
				continue
			}
		...
		if !c.Blocking {
			warns = append(warns, fmt.Sprintf("advisory check %q failed (not blocking): %s\n%s", c.Name, cmd, tailString(out, 600)))
			continue
		}
	...
	// Every check skipped by policy is NOT a gate that ran. Saying otherwise would let a project
	// glob its way to a green "checks passed" having executed nothing.
	return verifyResult{ran: ran, ok: true, name: "checks", code: checksCode(warns), warn: strings.Join(warns, "; ")}
}
...
func reviewerPrompt(task, diff, rubric, baselineNotes string) string {
	...
	// Shared judging rules: reason before deciding, calibrated by example, and never claim a check
	// you did not run. This gate had the verdict FORMAT specified precisely and the STANDARD not at
	// all — which is how a snap FAIL on a correct-but-unfamiliar change costs a whole re-run.
	b.WriteString("\n" + reviewCore() + "\n")
...
func runReviewer(baseRepo, wtPath, base, task, engineName string, timeout time.Duration, baselineNotes string, lanes []reviewLane) verifyResult {
	// No configured lanes = one reviewer on the build engine, which is what every project got
	// before G.5 and what every project that never opts in still gets. The second lane is real
	// money (2x reviewer tokens on the same diff), so it is never a default.
	if len(lanes) == 0 {
		lanes = []reviewLane{{ID: "primary", Engine: engineName}}
	}
	return runReviewLanes(baseRepo, wtPath, base, task, timeout, baselineNotes, lanes)
}
...
// pipelineCfg is everything the CONTROL PLANE gets to say about how one task is verified — the
// editable half of the pipeline, assembled from project settings and handed down on the run event.
//
// Every field is DATA. A toggle, a name, a glob, an engine identifier, a model string. There is no
// command and no path in here, because the daemon executes this and a server-supplied command is
// remote code execution on every machine in the fleet (#143). The repo stays authoritative about
// what a check IS and what the reviewer READS; this only says which of them run, and how hard.
//
// The zero value is exactly the pre-G.4 behaviour: every repo check blocking and always-run, one
// reviewer lane on the build engine, visual verify off. A project that never opens the settings
// page therefore sees no change at all.
type pipelineCfg struct {
	visual visualCfg
	checks []checkPolicy
	lanes  []reviewLane
	// changed is the task's changed-file list, used only to resolve path globs. Derived on the
	// machine from git, never supplied by the server.
	changed []string
	// step emits a LIVE progress line into the run's step stream (run_logs), so the board card and
	// the run detail page can say what the gate is doing RIGHT NOW.
	//
	// The gate was the longest silent stretch in a run: checks build the project, then a reviewer
	// reads the whole diff — minutes each, with nothing streamed. A card whose last line is "▶ task
	// name" from eight minutes ago is indistinguishable from a hung one, which is exactly how a
	// genuinely-dead run went unnoticed for 3.6 days. nil when there is nowhere to post (a hand-run
	// crank), so every call goes through pc.emit.
	step func(string)
}

// emit is the nil-safe way to report progress. Never let telemetry break a gate: a missing sink is
// silence, not a panic.
func (pc pipelineCfg) emit(format string, args ...any) {
	if pc.step == nil {
		return
	}
	pc.step(fmt.Sprintf(format, args...))
}

func verifyTask(baseRepo, wtPath, base, task, engineName string, timeout time.Duration, pc pipelineCfg) verifyResult {
	vc := pc.visual
	...
	resolved := applyPolicy(readChecks(baseRepo), pc.checks, pc.changed)
	if n := len(resolved); n > 0 {
		pc.emit("🔍 verify — running %d acceptance %s", n, plural(n, "check", "checks"))
	}
	checks = runChecks(baseRepo, wtPath, resolved, timeout)
	if checks.ran && !checks.ok {
		pc.emit("✗ verify — %s", firstReason(checks.reasons))
		return finish(checks)
	}
	if checks.ran {
		pc.emit("✓ verify — acceptance checks pass")
	}
	...
	// The single longest silent step in a run: a fresh engine reads the entire diff.
	pc.emit("👁 review — %s reading the diff against the task", reviewerLabel(engineName, pc.lanes))
	rev = runReviewer(baseRepo, wtPath, base, task, engineName, timeout, checks.warn, pc.lanes)
	if rev.ran && !rev.ok {
		pc.emit("✗ review — %s", firstReason(rev.reasons))
		return finish(rev)
	}
	if rev.ran {
		pc.emit("✓ review — reviewer passed the diff")
	}
	if vc.on {
		pc.emit("🖼 visual — rendering the changed routes")
	}
	vis = runVisualReview(baseRepo, wtPath, task, timeout, vc)
	if vis.ran && !vis.ok {
		pc.emit("✗ visual — %s", firstReason(vis.reasons))
		return finish(vis)
	}
...
// Small and pure so the phase narration stays readable at the call site, and so the strings a human
// reads on a card are testable without running a gate.

// firstReason is the ONE line that goes on the card. A card shows a single step line, so a
// multi-reason failure has to lead with something specific — "verify failed" tells a reader nothing
// they did not already know from the colour. Bounded because a reason can be a whole compiler dump.
func firstReason(reasons string) string {
	...
	if r == "" {
		return "rejected without a stated reason"
	}
	if len(r) > 120 {
		r = r[:117] + "…"
	}
...
// reviewerLabel names WHO is judging, because "review running" is the state people most often
// mistake for a hang — knowing it is a second engine reading a diff makes the wait legible. Mirrors
// runReviewer's own defaulting: no configured lanes means one reviewer on the build engine.
func reviewerLabel(engineName string, lanes []reviewLane) string {
	if len(lanes) == 0 {
		if engineName == "" {
			return "an independent reviewer"
		}
		return engineName
	}
	names := make([]string, 0, len(lanes))
	for _, l := range lanes {
		if e := strings.TrimSpace(l.Engine); e != "" {
			names = append(names, e)
		}
	}
	if len(names) == 0 {
		return fmt.Sprintf("%d reviewers", len(lanes))
	}
