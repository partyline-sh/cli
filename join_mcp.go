...
func joinMCPMain(args []string) {
	// `status` is the read-only inventory of what join-mcp has already registered on this
	// machine — a different job from joining, so it takes its own parser.
	if len(args) > 0 && args[0] == "status" {
		joinMCPStatusMain(args[1:])
		return
	}
	...
	env := [][2]string{
		...
		// This door is the one a PERSON walks through, so their posts go out under their own
		// login (`user:<display name>`, exactly as from the web) instead of the party token.
		// The runner does not set this, so agents are unaffected. See party_identity.go.
		{"PARTYLINE_PARTY_HUMAN", "1"},
	}
