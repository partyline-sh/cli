...
func tmuxMerge(specs []ptymux.Spec) (string, error) {
	...
	launcher := "" // the launcher's WINDOW id — new-window -b takes a window target, and
	// handing it the pane id (which the groups change made this scan return) fails with
	// tmux's "can't specify pane here", killing every open while the fixture exists
	if out, err := tmuxCmd("list-panes", "-s", "-t", tmuxSessionName, "-F", "#{pane_id}\t#{window_id}\t#{@ptln_key}").Output(); err == nil {
