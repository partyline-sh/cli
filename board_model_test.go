...
func TestRestoreFocusFollowsCardAcrossColumns(t *testing.T) {
	m := newBoardModel()
	m.board = &api.Board{Backlog: []api.BoardCard{card("a"), card("b")}}
	m.col = 0
...
func TestRestoreFocusForgetsCardThatLeftTheBoard(t *testing.T) {
	m := newBoardModel()
	m.board = &api.Board{Backlog: []api.BoardCard{card("a"), card("gone")}}
	m.cursor[api.ColBacklog] = 1
	m.rememberFocus()

	m.board = &api.Board{Backlog: []api.BoardCard{card("a")}}
	m.restoreFocus()
