...
// writeRunContext materializes the run's ANCHORED CONTEXT — what the thread records about the files
// this run's tasks name — as a file crank reads via --context-file. Same posture as writeRunGlobals:
// the filename derives only from the already-validated run id, and the content is pure DATA that
// never becomes an argument or a path.
func writeRunContext(runID, anchored string) (string, error) {
	dir := filepath.Join(daemonDir(), "context")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return "", err
	}
	path := filepath.Join(dir, runID+".md")
	if err := os.WriteFile(path, []byte(anchored), 0o600); err != nil {
		return "", err
	}
	return path, nil
}

// writeRunAcceptance materializes the run's runnable criteria as JSON crank reads via
// --acceptance-file. Same posture as writeRunGlobals: the filename derives only from the
// already-validated run id, and the content is DATA the worker runs in its own worktree.
func writeRunAcceptance(runID string, checks []api.RunAcceptanceCheck) (string, error) {
	dir := filepath.Join(daemonDir(), "acceptance")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return "", err
	}
