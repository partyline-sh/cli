...
func TestWatchPeerMessageFilesTheAnswerAndBanners(t *testing.T) {
	...
	if got.Status != taskCompleted || got.Answer != "yes, with jitter" {
...
func TestWatchPeerMessageRecordsATimeoutAtItsDeadline(t *testing.T) {
	...
	defer cancel()
	watchPeerMessage(ctx, p, peerMessage{ID: "c2", Peer: "air", Status: taskSubmitted, AskedAt: time.Now()},
		5*time.Millisecond, func(m peerMessage) { got = m }, b)
	if got.Status != taskFailed {
		t.Fatalf("status = %q, want the A2A failed state", got.Status)
	}
...
func TestWatchPeerMessageTreatsPollErrorsAsTransient(t *testing.T) {
	...
	defer cancel()
	watchPeerMessage(ctx, p, peerMessage{ID: "c3", Peer: "air", Status: taskSubmitted, AskedAt: time.Now()},
		5*time.Millisecond, func(m peerMessage) { got = m }, nil)
	if got.Status != taskFailed {
		t.Fatalf("status = %q, want the A2A failed state after the deadline", got.Status)
	}
	if p.count() < 2 {
...
func TestWatchPeerMessageCarriesTheDeclineDetail(t *testing.T) {
	...
	defer cancel()
	watchPeerMessage(ctx, p, peerMessage{ID: "c4", Peer: "air", Status: taskSubmitted, AskedAt: time.Now()},
		time.Millisecond, func(m peerMessage) { got = m }, nil)
	if got.Status != taskRejected || got.Answer != "that project moved" {
...
func TestPeerBannerNamesTheStatus(t *testing.T) {
	for status, want := range map[string]string{
		taskCompleted: "answered your question", taskRejected: "declined your question",
		taskFailed: "never answered", taskCanceled: "cancelled", "weird": "replied",
	} {
