...
func TestResolveHandle(t *testing.T) {
	...
	if got := resolveLocalRepoHandleIn([]string{home}, "", localRepoHandle(want)); got != want {
		t.Errorf("known handle resolved to %q, want %q", got, want)
	}
	if got := resolveLocalRepoHandleIn([]string{home}, "", "not-a-real-handle"); got != "" {
		t.Errorf("unknown handle resolved to %q, want empty", got)
	}
	if got := resolveLocalRepoHandleIn([]string{home}, "", ""); got != "" {
		t.Errorf("empty handle resolved to %q, want empty", got)
	}
	if got := resolveLocalRepoHandleIn(nil, "", localRepoHandle(want)); got != "" {
		t.Errorf("no roots resolved to %q, want empty", got)
	}
}

// A repo under an extra scan root (a data mount, outside home) must be offered AND resolvable —
// the monolith case: zero repos under $HOME, everything on /mnt/data.
func TestResolveHandleFromScanRoot(t *testing.T) {
	home := t.TempDir()
	mount := t.TempDir() // stands in for /mnt/data
	mkRepo(t, filepath.Join(mount, "acr-odoo"))
	want := filepath.Join(mount, "acr-odoo")

	roots := []string{home, mount}
	if got := resolveLocalRepoHandleIn(roots, "", localRepoHandle(want)); got != want {
		t.Errorf("scan-root handle resolved to %q, want %q", got, want)
	}
	// Without the root registered, the same handle must refuse — a root the owner removed
	// stops granting anything.
	if got := resolveLocalRepoHandleIn([]string{home}, "", localRepoHandle(want)); got != "" {
		t.Errorf("unregistered root's handle resolved to %q, want empty", got)
	}
}
