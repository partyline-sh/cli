...
// tls-internal mode must hand the web container Caddy's CA, or server-side OIDC dies with
// "fetch failed" while the browser side looks fine (the monolith sign-in incident).
func TestInternalTLSWritesCaddyCATrust(t *testing.T) {
	dir := t.TempDir()
	cfg := installConfig{
		dir: dir, site: "https://192.168.1.170:8443", tls: tlsInternal,
		bind: "0.0.0.0", httpPort: 8080, httpsPort: 8443, relayPort: 2222,
		explicit: map[string]bool{},
	}
	...
	if !strings.Contains(string(env), "NODE_EXTRA_CA_CERTS="+caddyCAPath) {
		t.Error("tls-internal install did not point Node at Caddy's CA")
	}
	// acme mode: no CA override — the public chain is the trust
	dir2 := t.TempDir()
	cfg.dir, cfg.tls, cfg.site = dir2, "", "https://box.example.com"
	cfg.httpPort, cfg.httpsPort = 80, 443
	if err := writeInstallEnvOverrides(cfg, installOps{writeFile: os.WriteFile}); err != nil {
		t.Fatal(err)
	}
	env2, _ := os.ReadFile(filepath.Join(dir2, ".env"))
	if strings.Contains(string(env2), "NODE_EXTRA_CA_CERTS") {
		t.Error("public-cert install must not override Node's trust store")
	}
}

// The first sign-in credentials are printed ONCE by env-bootstrap. The install step used to
// swallow them — a working box nobody could sign in to. The identity block must reach the
// operator's terminal verbatim.
func TestInstallSurfacesFirstSignInIdentity(t *testing.T) {
	dir := t.TempDir()
	var out strings.Builder
	cfg := installConfig{
		dir: dir, site: "https://box.local", bind: "0.0.0.0",
		httpPort: 80, httpsPort: 443, relayPort: 2222, explicit: map[string]bool{},
	}
	ops := installOps{
		out: &out, writeFile: os.WriteFile,
		run: func(_ string, name string, _ ...string) (string, error) {
			if name == "./env-bootstrap.sh" {
				return "wrote .env\nIDENTITY — written to keycloak/realm-partyline.json, imported on first start.\n  Sign in at https://box.local with:\n    admin@box.local\n    s3cret-once\n", nil
			}
			return "", nil
		},
	}
	...
	for _, s := range steps {
		if s.what == "fill in .env (secrets are generated here, never printed)" {
	...
	got := out.String()
	if !strings.Contains(got, "IDENTITY") || !strings.Contains(got, "admin@box.local") || !strings.Contains(got, "s3cret-once") {
		t.Errorf("the one-time credentials did not reach the operator:\n%s", got)
	}
}
