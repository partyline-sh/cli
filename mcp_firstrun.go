...
func maybeOfferMCPConnect() {
	...
	for _, e := range pending {
		if err := connectEngineQuiet(e.key); err != nil {
			// Not fatal, and recorded as unanswered so the next run tries again: a transient failure
			// (engine mid-upgrade, config locked) should not permanently mark this engine as done.
