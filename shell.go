...
func shellMain() {
	...
	invite := fs.String("invite", "", "comma-separated emails and/or teammate @handles to invite when the session opens")
	...
	relayAddr := fs.String("relay", "", "relay host:port for ssh <code>@<relay> joining (default: the relay your instance registers; '' to disable)")
	...
	fmt.Printf("   commands: ctrl-\\ ? for the menu · /pwho · /phud [on|off] · /pgrant [name] · /pinvite <email|@handle> · /phelp\n")
	...
	// No compiled-in relay any more. An explicit --relay wins; otherwise PARTYLINE_RELAY; otherwise
	// the control plane assigns the relay YOUR instance registered, on register below.
	useRelay := *relayAddr
	if useRelay == "" {
		useRelay = relayFromEnv()
	}
	apic := api.New()
	...
	if apic.Token != "" {
		...
		if useRelay != "" { // relay first — the universal, code-auth join path
			rh, rp, _ := strings.Cut(useRelay, ":")
			rport := 22
			if rp != "" {
		...
		if err == nil {
			...
			link := fmt.Sprintf("%s/j/%s#k=%s&r=%s", api.Base(), reg.JoinCode, keyB64, useRelay)
			fmt.Printf("   ☎ end-to-end encrypted — share this link:\n     %s\n", link)
			fmt.Printf("   joiners run:  ptln join '%s'\n", link)
			sess.OnInvite = func(target string) string {
				// @teamname → invite that team's members; otherwise the control plane
				// resolves the target as an email address or a teammate's @handle.
				if strings.HasPrefix(target, "@") {
					...
					// No team by that name — fall through: it may be a teammate's handle.
				}
				res, err := apic.InviteSession(sessionID, []string{target})
				if err != nil {
					// A target that resolved to nobody comes back as an error whose text
					// already names it ("couldn't reach @x — not a teammate, or …").
					return "invite to " + target + " failed: " + err.Error()
				}
				if res.N() == 0 {
					return "couldn't invite " + target + " — nothing was delivered"
				}
			...
			if *invite != "" {
				...
				if res, err := apic.InviteSession(sessionID, targets); err == nil {
					// Report the server's OWN accounting of who it reached. Deriving it by
					// subtracting the failures from what we submitted is what made the
					// original bug: a target trimmed by a cap, collapsed by a dedupe, or
					// dropped by a failed send would be printed as sent.
					sent, missed, quiet := inviteOutcome(targets, res.Accepted, res.Unresolved)
					fmt.Printf("   ✉️  invites sent: %d%s\n", res.N(), joined(" — ", sent))
					if len(missed) > 0 {
						fmt.Printf("   ⚠  couldn't reach %s — not a teammate, or no such handle\n", strings.Join(missed, ", "))
					}
					if len(quiet) > 0 {
						fmt.Printf("   ⚠  NOT delivered: %s — try again, or invite them from the web\n", strings.Join(quiet, ", "))
					}
				} else {
...
// inviteOutcome turns one invite response into three honest lists, keyed off what the
// SERVER said it did rather than off what we asked for:
//
//   - sent   — targets the server reported it actually reached (or armed).
//   - missed — targets it resolved to nobody.
//   - quiet  — everything else we submitted: trimmed by a server-side cap, collapsed into
//     another target by dedupe, or handed to a send that failed. These used to be counted
//     as sent by subtraction, which is exactly the "reports success, delivers nothing" bug.
//
// Every name printed is drawn from the host's OWN list (the server's strings are only
// matched against it, never echoed), so no server response can write to the host terminal.
func inviteOutcome(targets, accepted, unresolved []string) (sent, missed, quiet []string) {
	...
	ok, bad := set(accepted), set(unresolved)
	...
	return sent, missed, quiet
}

// joined renders "<sep>a, b" for a non-empty list and "" for an empty one, so a count line
// can carry its names without printing a bare separator when there are none.
func joined(sep string, items []string) string {
	if len(items) == 0 {
		return ""
	}
