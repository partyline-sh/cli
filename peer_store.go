...
const (
	...
	taskCanceled     = "canceled"      // terminal, user-initiated (A2A's US spelling). No producer yet:
	// esc on a wait hands the ask to the watcher rather than cancelling it, so nothing withdraws a
	// consult today. Named here so the state machine is complete when something does.
)
