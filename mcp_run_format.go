...
func formatRunSnapshot(snap *api.RunSnapshot) string {
	...
	for _, t := range snap.Tasks {
		...
		// #602: what the branch actually is. Printed for every task that has a probe, because "0
		// commits, worktree gone, not in the base" is the shape of a run that reported done and
		// produced nothing — the exact diagnosis this tool exists for.
		if bs := t.BranchState; bs != nil {
			fmt.Fprintf(&b, "       branch state: %s · worktree %s · %s\n",
				branchDiffPhrase(bs), existsPhrase(bs.WorktreeExists), mergedPhrase(bs.Merged))
		}
		if t.Summary != "" {
...
// branchDiffPhrase / existsPhrase / mergedPhrase keep the three facts from collapsing into each
// other in prose. An uncounted probe must not print "0 commits ahead" (a model would read it as
// "the worker did nothing"), and an unknown containment must not print "not merged" (a model would
// read it as "safe to reclaim").
func branchDiffPhrase(bs *api.BranchState) string {
	if !bs.Counted {
		return "commits ahead unknown (no base to compare against)"
	}
	base := bs.Base
	if base == "" {
		base = "the base"
	}
	if bs.CommitsAhead == 0 {
		return fmt.Sprintf("0 commits ahead of %s — nothing was produced", base)
	}
	return fmt.Sprintf("%d commit(s) ahead of %s, %d file(s) changed +%d -%d",
		bs.CommitsAhead, base, bs.FilesChanged, bs.Insertions, bs.Deletions)
}

func existsPhrase(exists bool) string {
	if exists {
		return "still on disk"
	}
	return "gone"
}

func mergedPhrase(merged string) string {
	switch merged {
	case "merged":
		return "contained in the base branch (merged)"
	case "unmerged":
		return "NOT in the base branch yet"
	default:
		return "merge state UNKNOWN (could not be determined — not the same as unmerged)"
	}
}
