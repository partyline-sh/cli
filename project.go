...
func projectMain(args []string) {
	...
	// THE LOCAL VERBS RUN WITHOUT AN ACCOUNT. A project's repos and its shared memory are files
	// plus a git remote; there is nothing for a server to authorise, and putting a sign-up in
	// front of "join my project" is a wall on a teammate's first day.
	switch sub {
	case "init":
		projectInit(args)
		return
