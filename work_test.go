...
func TestWorkerPrompt(t *testing.T) {
	p := workerPrompt("do a thing", "", true, false, false)
	...
	if containsSub(workerPrompt("x", "", false, false, false), "remember") {
	...
	if !containsSub(workerPrompt("x", "", false, false, false), "do NOT have general shell/Bash") {
		t.Fatal("no-bash prompt must tell the worker it has no shell access")
	}
	if !containsSub(workerPrompt("x", "", false, true, false), "VERIFY your change") {
		t.Fatal("bash-enabled prompt must tell the worker to verify by running tests")
	}
	// #575: granted command prefixes must be NAMED in the posture — a worker told "every attempt
	// is denied" would never use them.
	workerToolGrants = &api.ToolGrants{Shell: []string{"gh *"}}
	defer func() { workerToolGrants = nil }()
	granted := workerPrompt("x", "", false, false, false)
	if !containsSub(granted, "EXCEPTION") || !containsSub(granted, "gh *") {
		t.Fatal("granted prefixes must be named in the no-bash posture")
	}
}

func TestWorkerPromptInjectsContext(t *testing.T) {
	ctx := "What this is about:\n• #12 [decision] use Postgres for the store  — user:me"
	p := workerPrompt("build the thing", ctx, true, false, false)
	...
	if containsSub(workerPrompt("x", "", false, false, false), "Shared team context") {
