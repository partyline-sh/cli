...
// #556 — codex gets the partyline MCP by default now.
//
// It used to require PARTYLINE_MCP_EXPERIMENTAL, so we shipped "partyline runs codex" while a codex
// session silently got none of the partyline tools. An opt-in nobody is told about is the same as
// the feature not existing. These pin the new shape, including the escape hatch — because codex MCP
// still has not been verified end-to-end the way claude's has, and the rollback must not require a
// release.
func TestMCPActiveByEngine(t *testing.T) {
	for _, tc := range []struct {
		name, engine, env string
		want              bool
	}{
		{"claude is on", "claude", "", true},
		{"codex is on by DEFAULT — the whole point of #556", "codex", "", true},
		{"codex can be turned off without touching claude", "codex", "PARTYLINE_NO_CODEX_MCP", false},
		{"gemini stays off — no per-invocation MCP config", "gemini", "", false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("PARTYLINE_NO_MCP", "")
			t.Setenv("PARTYLINE_NO_CODEX_MCP", "")
			if tc.env != "" {
				t.Setenv(tc.env, "1")
			}
			r := &partyRunner{engine: tc.engine}
			if got := r.mcpActive(); got != tc.want {
				t.Fatalf("engine=%s env=%s: mcpActive()=%v want %v", tc.engine, tc.env, got, tc.want)
			}
		})
	}
}

// The codex switch must be CODEX-ONLY. If it ever also silenced claude, someone hitting a codex
// problem would have to disable MCP for their working engine too — which is what the global
// PARTYLINE_NO_MCP is for, and why this one has to be narrower.
func TestCodexKillSwitchLeavesClaudeAlone(t *testing.T) {
	t.Setenv("PARTYLINE_NO_MCP", "")
	t.Setenv("PARTYLINE_NO_CODEX_MCP", "1")
	if !(&partyRunner{engine: "claude"}).mcpActive() {
		t.Fatal("PARTYLINE_NO_CODEX_MCP disabled MCP for claude — it must only affect codex")
	}
}

// The global kill switch still wins over everything, for every engine.
func TestGlobalKillSwitchStillWins(t *testing.T) {
	t.Setenv("PARTYLINE_NO_MCP", "1")
	for _, e := range []string{"claude", "codex"} {
		if (&partyRunner{engine: e}).mcpActive() {
			t.Fatalf("PARTYLINE_NO_MCP did not disable MCP for %s", e)
		}
	}
}
