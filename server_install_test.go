...
// "https://127.0.0.1/activate?..." on a box answering only on :8443. The installer used to WARN
// and proceed (an operator read the warning, proceeded, and got a broken sign-in); it now fixes
// the site itself, so the plan must show the portful address.
func TestPlanWarnsWhenSiteOmitsARemappedPort(t *testing.T) {
	...
	got := out.String()
	if !strings.Contains(got, "site gets the box's port: https://box.local:8443") {
		t.Errorf("the port was not added to the site (or silently):\n%s", got)
	}
	if !strings.Contains(got, "https://box.local:8443") {
		t.Errorf("plan does not carry the portful site:\n%s", got)
	}
...
func TestEnsureSitePortAppendsTheBoxPort(t *testing.T) {
	var out strings.Builder
	base := installConfig{httpPort: 8080, httpsPort: 8443, explicit: map[string]bool{}}

	c := base
	c.site = "https://192.168.1.170"
	if got := ensureSitePort(c, &out).site; got != "https://192.168.1.170:8443" {
		t.Errorf("https on a remapped box: %q", got)
	}
	c = base
	c.site = "http://192.168.1.170"
	if got := ensureSitePort(c, &out).site; got != "http://192.168.1.170:8080" {
		t.Errorf("http on a remapped box: %q", got)
	}
	// the operator's explicit port is their word
	c = base
	c.site = "https://192.168.1.170:9999"
	if got := ensureSitePort(c, &out).site; got != "https://192.168.1.170:9999" {
		t.Errorf("explicit port overridden: %q", got)
	}
	// default ports: nothing to add
	c = installConfig{site: "https://box.example.com", httpPort: 80, httpsPort: 443, explicit: map[string]bool{}}
	if got := ensureSitePort(c, &out).site; got != "https://box.example.com" {
		t.Errorf("default-port site mangled: %q", got)
	}
	// tunnel shape: the public port IS 443 — appending ours would break every link
	c = installConfig{site: "https://box.example.com", tls: tlsOff, httpPort: 8080, httpsPort: 8443, explicit: map[string]bool{}}
	if got := ensureSitePort(c, &out).site; got != "https://box.example.com" {
		t.Errorf("tunnel-fronted site mangled: %q", got)
	}
	if !strings.Contains(out.String(), "site gets the box's port") {
		t.Error("the append was silent")
	}
}
...
// A failing request must leave a trace. Caddy logs only warnings by default and Next logs
// nothing per request, so a reported 500 could not be confirmed to have even reached the
// instance. Errors are logged; successes are not (this is not an access log).
func TestCaddyfileLogsErrors(t *testing.T) {
	got := caddyfileFor(installConfig{site: "https://ptln.example", dir: "/opt/partyline"})
	for _, want := range []string{"log {", "output stderr", "level ERROR"} {
		if !strings.Contains(got, want) {
			t.Errorf("Caddyfile has no error log — %q missing", want)
		}
	}
	if strings.Contains(got, "level INFO") || strings.Contains(got, "level DEBUG") {
		t.Error("the log block is an ACCESS log — only 4xx/5xx belong here")
	}
}

// The bus is only reachable through Caddy — nothing publishes port 2223. If this route is
// missing or lands after the catch-all, every CLI's /bus upgrade reaches the Next app instead
// of the relay and silently falls back to polling, which is the failure the /auth/* route
// already taught us to test for.
func TestCaddyfileRoutesTheBusToTheRelay(t *testing.T) {
	got := caddyfileFor(defaultInstallConfig())
	if !strings.Contains(got, "handle /bus* {") || !strings.Contains(got, "reverse_proxy relay:2223") {
		t.Fatalf("Caddyfile does not route /bus to the relay:\n%s", got)
	}
	if strings.Index(got, "handle /bus*") > strings.Index(got, "\thandle {") {
		t.Errorf("the /bus route falls after the catch-all, so it never matches:\n%s", got)
	}
}

// An upgrade regenerates the Caddyfile to pick up new routes. It must not change how TLS is
// terminated while doing so.
//
// This is not hypothetical. A box behind Cloudflare serves plain HTTP at the origin; the mode is
// recorded nowhere but the Caddyfile, so an upgrade re-derived it from the site URL, saw a public
// name, and chose ACME. Caddy then redirected HTTP to HTTPS on the origin, Cloudflare followed
// the redirect back to itself, and every request became an infinite 308 loop. The site was down
// until the site block got its http:// back.
func TestAnUpgradeKeepsTheTLSModeTheInstallAlreadyUses(t *testing.T) {
	...
	for body, want := range cases {
		dir := t.TempDir()
		if err := os.WriteFile(filepath.Join(dir, "Caddyfile"), []byte(body), 0o644); err != nil {
			t.Fatal(err)
		}
		if got := tlsModeOfCaddyfile(dir); got != want {
			t.Errorf("tlsModeOfCaddyfile = %q, want %q for:\n%s", got, want, body)
		}
		// And the upgrade path must carry it through rather than re-deriving from the site.
		cfg := installConfig{dir: dir, site: "https://box.example.com", explicit: map[string]bool{}}
		if got := keepExistingTLSMode(cfg, installOps{}).tls; got != want {
			t.Errorf("an upgrade would switch this install to %q, want %q", got, want)
		}
	}
}

// The internal door is always http://caddy and says nothing about how the public site is served.
// Reading it as the decision would downgrade every install to plain HTTP on the next upgrade.
func TestTheInternalDoorIsNotMistakenForTheTLSDecision(t *testing.T) {
	dir := t.TempDir()
	body := caddyfileFor(defaultInstallConfig())
	if err := os.WriteFile(filepath.Join(dir, "Caddyfile"), []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
	if got := tlsModeOfCaddyfile(dir); got == tlsOff {
		t.Fatalf("the http://caddy door was read as the site's mode:\n%s", body)
	}
}

// An explicit --tls on this run still wins: the operator changing the mode deliberately is the
// one case where it should change.
func TestAnExplicitTLSFlagStillWins(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "Caddyfile"), []byte("http://box {\n}\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	cfg := installConfig{dir: dir, tls: tlsInternal, explicit: map[string]bool{}}
	if got := keepExistingTLSMode(cfg, installOps{}).tls; got != tlsInternal {
		t.Errorf("an explicit --tls was overridden by the existing file: %q", got)
	}
}
