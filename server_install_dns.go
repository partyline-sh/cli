...
const (
	...
	dnsResolvesLoopback                   // it resolves to 127.x — a hosts-file alias no container can use
)
...
func checkSiteDNS(site string, lookup func(string) ([]string, error), local []string) (dnsResult, []string) {
	...
	if loopbackOnly {
		// Debian's /etc/hosts points the machine's own hostname at 127.0.1.1. From this shell
		// that looks like it works; from inside a container 127.x is the container, so the box
		// installs and then nobody can sign in. Its own state, because the FIX is different
		// from a stale public record: use an address other machines hold.
		return dnsResolvesLoopback, addrs
	}
...
func dnsNote(r dnsResult, addrs []string) string {
	...
	case dnsResolvesLoopback:
		return "resolves to " + strings.Join(addrs, ", ") + " — a hosts-file alias; containers cannot use it"
	}
	return ""
}
