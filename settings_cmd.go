...
func settingsMain(_ []string) {
	...
	// Party MCP registrations (this machine). Counted, never probed: `ptln settings` is a map you
	// read in a second, and probing every party would make it wait on the network.
	if home, err := os.UserHomeDir(); err == nil {
		n := len(scanPartyRegistrations(home, cwd))
		fmt.Printf("  party wiring   %d party MCP registration(s)            → ptln join-mcp status\n", n)
	}
	...
	fmt.Println("  schedules      run a trigger on a clock, not a webhook  → ptln trigger set <slug> --cron '0 3 * * *'")
	// Separate line from the one above deliberately: "where do I configure it" and "what has it
	// actually been doing" are different questions, and the second is the one someone asks at 3am.
	fmt.Println("  trigger log    what your triggers have DONE lately     → ptln trigger activity · ptln trigger log <slug>")
	...
	// The identity trust root: WHICH control plane this machine believes when it says "this joiner
	// is alice@acme.com". Stated here because it is the one setting whose wrong value is invisible
	// until someone joins your terminal as a person they are not — and because with no pin the
	// answer is the key compiled into this binary, which is not discoverable anywhere else.
	if fp, source, base, err := identity.TrustRoot(); err != nil {
		fmt.Printf("  identity root  UNUSABLE — joins are refused: %v\n", err)
	} else if base != "" {
		fmt.Printf("  identity root  %s (pinned from %s)  → ptln login <url> [--accept-new-key]\n", fp, base)
	} else {
		fmt.Printf("  identity root  %s — %s  → ptln login <url> to pin a self-hosted one\n", fp, source)
	}

	// Self-host only, and stated as a COUNT so the line is useful without running the check: on a
	// laptop this reads 0/N and that is correct — these are a BOX's integrations, not this
	// machine's. Named here because a setting you cannot find is a setting an agent cannot fix.
	if reg, err := features.Parse(featuresJSON); err == nil {
		configured := 0
		for _, st := range reg.Status(os.Getenv) {
			if st.Configured {
				configured++
			}
		}
		fmt.Printf("  server features %d/%d configured in this environment  → ptln server doctor\n", configured, len(reg.Features))
	}
