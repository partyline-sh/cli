...
func renderBoard(b *api.Board) string {
	...
	fmt.Fprintf(&s, "WORK BOARD — backlog %d · building %d · blocked %d · review %d · shipped %d\n",
		len(b.Backlog), len(b.Building), len(b.Blocked), len(b.Review), len(b.Shipped))
