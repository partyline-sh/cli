...
func tmuxConf() string {
	...
	// for terminals that honor it. no-clear keeps the highlight so a drag LOOKS selected.
	w(`set -g set-clipboard on`)
	if clip := clipboardCmd(); clip != "" {
		w(`bind -T copy-mode MouseDragEnd1Pane send -X copy-pipe-no-clear ` + shQuote(clip))
		w(`bind -T copy-mode-vi MouseDragEnd1Pane send -X copy-pipe-no-clear ` + shQuote(clip))
		...
		w(`bind -T copy-mode MouseDragEnd1Pane send -X copy-selection-no-clear`)
		w(`bind -T copy-mode-vi MouseDragEnd1Pane send -X copy-selection-no-clear`)
	}
	// After a drag the pane is still in copy-mode: the highlight lingers and keystrokes go to
	// copy-mode, not the app, until escape. A CLICK now cancels — highlight gone, back live —
	// which is what a selection does in every ordinary terminal.
	w(`bind -T copy-mode MouseDown1Pane { select-pane; send -X cancel }`)
	w(`bind -T copy-mode-vi MouseDown1Pane { select-pane; send -X cancel }`)
