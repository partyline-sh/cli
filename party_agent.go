...
func (r *partyRunner) wake(ctx context.Context, trig []api.PartyMsg) {
	...
	// Live turn streaming (Option B): stream the reply AS IT FORMS as ordered "turn" frames the web
	// assembles into a growing chat bubble, then swaps for the durable message (stamped with turnID).
	// A single consumer goroutine posts in order so prose never scrambles, and the parse loop never
	// blocks on HTTP — frames buffer, and a full buffer drops frames (best-effort; the durable message
	// is the source of truth). Superset of the activity feed: this is the reply, not just step telemetry.
	turnID := fmt.Sprintf("%s-%d", r.name, time.Now().UnixNano())
	type turnFrame struct {
		event string
		text  string
		tool  *api.PartyTurnTool
	}
	turnCh := make(chan turnFrame, 256)
	...
	go func() {
		...
		for f := range turnCh {
			seq++
			_ = r.pc.PostTurn(r.name, turnID, seq, f.event, f.text, f.tool)
		}
	}()
	emitTurn := func(f turnFrame) {
		select {
		case turnCh <- f:
		default: // buffer full — drop this frame (best-effort)
		}
	}
	emitTurn(turnFrame{event: "start"})
	onTurn := func(event, text string, tool *api.PartyTurnTool) {
		emitTurn(turnFrame{event: event, text: text, tool: tool})
	}
	// Drain the turn stream BEFORE the durable message posts below, so every frame reaches the browser
	// ahead of the swap (an "end"/"text" frame arriving AFTER the durable row would resurrect an empty
	// buffer until the grace timer). Frames are few and PostTurn is fast, so the wait is short.
	drainTurn := func() {
		emitTurn(turnFrame{event: "end"})
		close(turnCh)
		<-turnDone
	}
	switch {
	case r.customCmd != "":
		fmt.Printf("\n↯ %s waking (%s)…\n", r.name, r.customCmd)
		out, err = runAgentPlain(ctx, append(splitFields(r.customCmd), r.passthru...), prompt, "", true, false, nil, onActivity, onTurn)
	default:
		...
		out, err = runAgentPlain(ctx, argv, prompt, r.resumeDir, spec.stdin, spec.stream, env, onActivity, onTurn)
	}
	drainTurn() // flush all live-turn frames to the browser before any durable message posts below
	if err != nil {
		...
		_, _ = r.pc.PostMeta(r.name, "⚠️ couldn't respond this turn — "+note, "msg", map[string]any{"turn_id": turnID})
		return
	}
	...
	if out == "" {
		if len(edits) > 0 {
			// A doc-only turn used to post NOTHING (out==""), so "working" never cleared and the
			// streaming buffer vanished to nothing — the "does nothing" bug. Post a short durable
			// message (stamped with turnID) so the indicator clears and the buffer swaps in place.
			fmt.Printf("   ✓ proposed %d edit(s) (%.1fs)\n", len(edits), time.Since(start).Seconds())
			_, _ = r.pc.PostMeta(r.name, "📝 Proposed edits to the working doc — see the doc panel.", "msg", map[string]any{"turn_id": turnID})
		} else {
	...
	if _, err := r.pc.PostMeta(r.name, out, "msg", map[string]any{"turn_id": turnID}); err != nil {
...
func runAgentPlain(ctx context.Context, argv []string, prompt, dir string, stdin, stream bool, env []string, onActivity func(string), onTurn func(event, text string, tool *api.PartyTurnTool)) (string, error) {
	if len(argv) == 0 {
				...
				case "assistant":
					for _, b := range ev.Message.Content {
						...
						case "tool_use":
							...
							if onTurn != nil {
								onTurn("tool", "", &api.PartyTurnTool{Name: b.Name, Brief: strings.TrimSpace(briefInput(b.Input))})
							}
						case "text":
							if s := strings.TrimSpace(b.Text); s != "" {
								...
								if s != strings.TrimSpace(lastText) {
									if onActivity != nil {
										onActivity(s)
									}
									// Live turn stream (Option B): the reply prose forming as a growing bubble.
									// Block-level today (one frame per text block); token-level is a later flag.
									if onTurn != nil {
										onTurn("text", b.Text, nil)
									}
								}
								lastText = b.Text
							}
