...
func partyMain(args []string) {
	...
	r := &partyRunner{
		...
		idleTimeout: idleTimeout, maxTimeout: maxTimeout,
	}
	r.loop(ctx)
	fmt.Printf("\npl party: %s left.\n", name)
}
...
func (r *partyRunner) wake(ctx context.Context, trig []api.PartyMsg) {
	...
	case r.customCmd != "":
		fmt.Printf("\n↯ %s waking (%s)…\n", r.name, r.customCmd)
		out, err = runAgentPlain(ctx, append(splitFields(r.customCmd), r.passthru...), prompt, "", true, false, nil, r.idleTimeout, r.maxTimeout, onActivity)
	default:
		...
		out, err = runAgentPlain(ctx, argv, prompt, r.resumeDir, spec.Stdin, spec.Stream, env, r.idleTimeout, r.maxTimeout, onActivity)
	}
	if err != nil {
...
func runAgentPlain(ctx context.Context, argv []string, prompt, dir string, stdin, stream bool, env []string, idle, backstop time.Duration, onActivity func(string)) (string, error) {
	if len(argv) == 0 {
