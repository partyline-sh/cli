...
//
// It also carries this turn's LIVENESS. The web used to infer "is working" from step timing, which
// meant a long single generation — no tool calls, no prose blocks, nothing to flush — looked exactly
// like a dead runner after 90 seconds, and the room went quiet on a human who was still owed an
// answer. So the runner ASSERTS it: a heartbeat at turn start, every heartbeatEvery while the engine
// runs, and one at turn end. Distinct stream from step output, so it is never rendered as a step.
type activityFeed struct {
	...
	seq  int64
	// send serializes whole flushes (drain + POST), so batches reach the control plane in the order
	// they were buffered. Two flushes overlap routinely: the 600ms ticker can still be mid-POST when
	// the turn's final flush fires. Unserialized, the batch holding `end` could COMMIT BEFORE an
	// earlier batch holding `alive`, giving the `alive` row the later created_at — and the web, which
	// resolves "did this turn finish" from row order, would read a cleanly finished turn as a runner
	// that went quiet. It is deliberately NOT `mu`: buffering must never wait on the network.
	send sync.Mutex
	// ended is set by the `end` beat and makes it TERMINAL: no later heartbeat can be buffered for this
	// turn. Without it the flusher goroutine's select is a coin flip at teardown — when the stop channel
	// and the heartbeat tick are both ready, Go picks one at random, so an `alive` could be appended
	// AFTER `end`, in the same batch, with a higher seq. Same created_at, higher seq: the web's
	// tie-break would pick `alive` and render a cleanly finished turn as "its runner may have stopped".
	// Guarded by mu alongside buf, because it is exactly the same invariant — what may still be added.
	ended bool
	...
	inTok, outTok int
	usageDirty    bool
	// Tickers, overridable so the teardown race is testable at speed. Zero means the real cadence.
	beatEvery, flushEvery time.Duration
}

// activitySink is the one thing the feed needs from the party API: somewhere to put lines.
// *api.PartyClient satisfies it; a test can substitute a sink that fails or stalls, which is how we
// hold the line that telemetry never costs the turn.
type activitySink interface {
	AppendActivity(name string, lines []api.PartyActivityLine) error
}

// Heartbeat cadence and phases. heartbeatEvery is duplicated in web/src/app/p/[id]/agent-liveness.ts,
// which sizes its staleness window from it — change one, change the other.
const (
	heartbeatEvery = 10 * time.Second
	activityFlush  = 600 * time.Millisecond
	beatStart      = "start" // the turn has begun
	beatAlive      = "alive" // still generating
	beatEnd        = "end"   // the turn is over — finished, as opposed to gone quiet
)

// beat asserts liveness for this turn. Buffered like any other line, so it costs the turn nothing:
// the flush ticker ships it within its next tick, and a failed ship is simply a missed beat — the web
// tolerates several in a row before it says anything.
//
// `end` is terminal: once it is buffered, every later beat is DROPPED. See the `ended` field — this is
// what makes the flusher's teardown select safe rather than a coin flip.
func (a *activityFeed) beat(phase string) {
	a.mu.Lock()
	defer a.mu.Unlock()
	if a.ended {
		return
	...
	a.seq++
	a.buf = append(a.buf, api.PartyActivityLine{Seq: a.seq, Stream: "heartbeat", Body: phase})
}

// startTurn opens a turn: it beats `start`, then runs the background flusher that streams buffered
// lines (rather than dumping them at the end) and keeps beating while the engine generates. The
// returned endTurn closes the turn — stop the flusher, beat `end`, drain — and is idempotent.
//
// This lifecycle lives here, not inline in wake(), so the teardown ordering the whole feature rests on
// is reachable from a test without standing up an engine and a control plane.
func (a *activityFeed) startTurn() (endTurn func()) {
	beatEvery, flushEvery := a.beatEvery, a.flushEvery
	if beatEvery <= 0 {
		beatEvery = heartbeatEvery
	}
	if flushEvery <= 0 {
		flushEvery = activityFlush
	}
	a.beat(beatStart)
	stop := make(chan struct{})
	go func() {
		t := time.NewTicker(flushEvery)
		hb := time.NewTicker(beatEvery)
		defer t.Stop()
		defer hb.Stop()
		for {
			select {
			case <-stop:
				return
			case <-hb.C:
				a.beat(beatAlive) // liveness even when the engine produces nothing to show
			case <-t.C:
				a.flush()
			}
		}
	...
	return func() {
		once.Do(func() {
			close(stop)
			// The turn is OVER — say so, so the web can tell "finished" from "went quiet". A beat the
			// flusher races in after this point is dropped by `ended`, so `end` is always the turn's
			// last heartbeat regardless of which way that select falls.
			a.beat(beatEnd)
			// Drains everything, including anything the flusher buffered on its way out. Ordered after
			// any in-flight flush by `send`, so this can wait for one POST — bounded, once per turn,
			// and it happens after the agent's reply has already been posted.
			a.flush()
		})
	}
}
...
//
// Flushes are serialized END TO END — `send` is taken BEFORE the buffer is drained, not just around
// the POST. Draining first and then queueing for the wire would let a later flush win the race for
// the wire and commit its batch (the one carrying `end`) ahead of an earlier one (carrying `alive`),
// which is precisely the ordering the web reads to tell a finished turn from a runner that died.
// Serializing the drain too makes "later batch ⇒ later created_at" true by construction.
//
// This never gates the turn: beat/add take only `mu`, so the agent's own goroutine keeps buffering at
// full speed while a slow control plane holds up the sender.
func (a *activityFeed) flush() {
	...
	a.mu.Unlock()
	if a.pc == nil {
		return // no sink wired — telemetry is optional, and a missing one must never panic a turn
	}
...
func (r *partyRunner) wake(ctx context.Context, trig []api.PartyMsg) {
	...
	var out string
	var err error
	// Live activity: mirror each step to the operator's terminal AND to the party feed (the web polls
	// it every 3s — the Realtime subscription is long gone). A background ticker flushes the buffer
	// during the turn so it streams, not dumps.
	feed := &activityFeed{pc: r.pc, name: r.name}
	// The whole turn's telemetry lifecycle, including the `end` beat, behind one deferred call — so it
	// runs on EVERY exit path, the engine-error return below included. See activityFeed.startTurn.
	endTurn := feed.startTurn()
	defer endTurn()
...
// mcpActive reports whether this turn hands the agent the partyline MCP tools. claude and codex are
// both on by default; gemini is not (no per-invocation MCP config + auth-coupled ~/.gemini, B.3).
// Not for --cmd agents, and PARTYLINE_NO_MCP is a global kill switch.
//
// CODEX WAS UN-GATED HERE (#556). It used to require PARTYLINE_MCP_EXPERIMENTAL, which meant the
// marketing claim and the product disagreed: we say partyline runs codex, and a codex session
// silently got none of the partyline tools unless someone knew to set an env var. An opt-in nobody
// is told about is the same as the feature not existing.
//
// The old gate's caution was still right about one thing: codex MCP has never been verified
// end-to-end the way claude's has. So the default flips but the escape hatch stays — now a
// codex-SPECIFIC off switch rather than an experimental on switch. That matters because
// PARTYLINE_NO_MCP is global: without this, someone hitting a codex MCP problem could only fix it
// by disabling MCP for claude too, or by waiting for a release. Off-by-default protects us; a
// rollback that needs no release protects them.
func (r *partyRunner) mcpActive() bool {
	...
	case "claude":
		return true
	case "codex":
		return os.Getenv("PARTYLINE_NO_CODEX_MCP") == ""
	}
