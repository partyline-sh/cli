...
func (r *partyRunner) wake(ctx context.Context, trig []api.PartyMsg) {
	...
	case r.customCmd != "":
		fmt.Printf("\n↯ %s waking (%s)…\n", r.name, r.customCmd)
		out, err = runAgentPlain(ctx, append(splitFields(r.customCmd), r.passthru...), prompt, "", true, false, onActivity)
	default:
		...
		out, err = runAgentPlain(ctx, argv, prompt, r.resumeDir, spec.stdin, spec.stream, onActivity)
	}
	if err != nil {
	...
	if out == "" {
		...
		return // agent chose to stay silent
	}
...
func runAgentPlain(ctx context.Context, argv []string, prompt, dir string, stdin, stream bool, onActivity func(string)) (string, error) {
	if len(argv) == 0 {
