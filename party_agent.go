...
func runAgentPlain(ctx context.Context, argv []string, prompt, dir string, stdin, stream bool, env []string, onActivity func(string)) (string, error) {
	...
	// Turn deadline. A turn is killed only when it's genuinely STUCK, not merely slow:
	...
	//     times out; only silence for agentIdleTimeout kills it. agentMaxTimeout is an
	//     absolute backstop so a runaway can't run forever.
	//   - non-stream engines: no progress to observe, so a plain wall-clock (agentMaxTimeout).
	...
	hardStop := time.AfterFunc(agentMaxTimeout, func() { timedOut.Store(true); cancel() })
	defer hardStop.Stop()
	var idleStop *time.Timer
	if stream {
		idleStop = time.AfterFunc(agentIdleTimeout, func() { timedOut.Store(true); cancel() })
		defer idleStop.Stop()
	}
	full := argv
	if !stdin {
	...
	fail := func(werr error) error {
		if timedOut.Load() {
			if stream {
				return fmt.Errorf("agent stalled — no output for %s", agentIdleTimeout)
			}
			return fmt.Errorf("agent timed out after %s", agentMaxTimeout)
		}
	...
	for {
		...
		if len(line) > 0 {
			idleStop.Reset(agentIdleTimeout) // progress — the agent isn't stuck; extend the idle deadline
			var ev claudeEvent
