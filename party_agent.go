...
func partyMain(args []string) {
	...
	if clone {
		...
		fmt.Printf("✓ Cloned this session into the party as @%s — a headless agent forked from your full context,\n"+
			"  now live in the party and auto-responding to @%s. It runs until the party ends or you kill it.\n"+
			"  log: %s\n", name, name, logPath)
		return
	}
	...
	if customCmd == "" {
		if _, ok := engines[engine]; !ok {
			fmt.Fprintf(os.Stderr, "ptln party: unknown --engine %q (try: claude, gemini, codex — or --cmd \"<command>\")\n", engine)
			os.Exit(2)
		}
	}
	...
	r := &partyRunner{
		...
		resumeID: resumeID, resumeDir: resumeDir, evidence: evidence, verifier: verifier,
	}
	r.loop(ctx)
	fmt.Printf("\npl party: %s left.\n", name)
}
...
func partyUsage() {
	fmt.Println(`Usage: ptln party '<join-link>' --name <name> [--role "<what it does>"] [--model <m>]
...
party's page on the web.
  ...
  --engine which agent CLI to wake: claude (default) · gemini · codex. claude streams
           its tool activity to this terminal; others post their reply (plain).
  ...
  --clone  run from INSIDE a Claude Code session: clone THIS session (its full context)
           into a detached headless agent that joins the party and auto-responds — like
           sending a copy of yourself in. Your session keeps going; the clone logs to
           ~/.partyline/clones/<name>.log and runs until the party ends or you kill it.
