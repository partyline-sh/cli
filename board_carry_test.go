...
// The reorder math: one write when a gap exists, a 10-step renumber when Odoo's default
// everything-is-10 leaves none, and nothing at all for a single-card column.
func TestReorderWrites(t *testing.T) {
	// gap between neighbours → the moved card alone gets the midpoint
	got := reorderWrites([]api.BoardCard{{ID: "a", Seq: 10}, {ID: "m", Seq: 99}, {ID: "c", Seq: 20}}, "m")
	if len(got) != 1 || got[0].seq != 15 {
		t.Fatalf("gap drop should midpoint to 15, got %+v", got)
	}
	// top: below-neighbour minus ten
	got = reorderWrites([]api.BoardCard{{ID: "m", Seq: 99}, {ID: "a", Seq: 10}}, "m")
	if len(got) != 1 || got[0].seq != 0 {
		t.Fatalf("top drop: want seq 0, got %+v", got)
	}
	// bottom: above-neighbour plus ten
	got = reorderWrites([]api.BoardCard{{ID: "a", Seq: 10}, {ID: "m", Seq: 5}}, "m")
	if len(got) != 1 || got[0].seq != 20 {
		t.Fatalf("bottom drop: want seq 20, got %+v", got)
	}
	// fresh Odoo: every card ships Seq 10 — no gap anywhere → full renumber, only changed rows
	got = reorderWrites([]api.BoardCard{{ID: "a", Seq: 10}, {ID: "m", Seq: 10}, {ID: "c", Seq: 10}}, "m")
	if len(got) != 2 { // a→10 unchanged; m→20, c→30
		t.Fatalf("tie renumber: want 2 writes, got %+v", got)
	}
	if reorderWrites([]api.BoardCard{{ID: "m", Seq: 10}}, "m") != nil {
...
func (f *fakeOrderSource) SetCardSeq(_ string, id string, seq int) error {
	f.writes = append(f.writes, seqWrite{id, seq})
	return nil
}
