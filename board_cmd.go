...
func (m *boardModel) handleKey(b []byte, c *api.Client) (bool, bool) {
	...
	// A grabbed card owns the keys the same way a modal does: arrows carry it, ⏎ drops it,
	// esc puts it back — and nothing else can fire underneath the thing being held.
	if m.carry != nil {
		return m.carryKey(b, c)
	}
	...
	case 'm':
		return false, m.grabCard()
	case 'e':
		return false, m.editCard(c)
	case 'c':
		return false, m.commentCard(c)
	case 'd':
		m.detailPane(c)
	case 'o':
		m.openPR()
