...
func (m *boardModel) columnLines(col api.BoardColumn, w, h int, focused bool) []string {
	...
	for i := top; i < len(rows) && len(lines) < bodyH; i++ {
		...
		if m.carry != nil && rows[i].Card.ID == m.carry.card.ID {
			lines = append(lines, m.tileLinesLifted(*rows[i].Card, w)...)
			continue
		}
...
func (m *boardModel) tileLines(c api.BoardCard, w int, sel bool) []string {
	...
	// The foreign tracker's own number rides the top border: visible on every tile without
	// costing a line, and quotable to whoever works in that tracker. It is dropped rather than
	// truncated on a column too narrow to hold it and the selection marker both.
	lead := "╭"
	if sel {
		lead = "╭─▸"
	}
	ref := ""
	if c.Ref != "" && visWidth(lead)+visWidth(c.Ref)+3 <= inner {
		ref = " " + c.Ref + " "
	}
	top := lead + ref + strings.Repeat("─", max(0, inner+1-visWidth(lead)-visWidth(ref))) + "╮"
	...
	// State FIRST, then the live line. The state is the fact you scan a column for; what the agent
	// last said, or how stale the answer is, qualifies it. Printing the qualifier above the thing
	// it qualifies reads backwards.
...
func (m *boardModel) statusLine(w int) string {
	// Outranks everything: a board rendering last week's code is a board whose every other line is
	// suspect, and there is no way to tell from the inside otherwise.
	if m.binaryReplaced() {
		return clipVis(boardWarn+"a newer ptln is installed — this board is still the old one; q and reopen"+reset, w)
	}
	if m.toast != "" {
		clr := boardGood
		if m.toastBad {
...
// binaryReplaced reports whether ptln was updated underneath this running board.
//
// `ptln update` swaps the file on disk; a process already running keeps executing the code it
// started with. The board is long-lived and the ctrl-\ menu REUSES an open one rather than making
// a second, so the obvious action after an update hands you the stale board with nothing saying so.
// That has cost real time more than once.
//
// Compared by modification time rather than by asking the binary its version: no subprocess, no
// network, and "the file changed" is the fact that matters — a downgrade or a local build is just
// as much a mismatch as an upgrade.
func (m *boardModel) binaryReplaced() bool {
	if m.selfStamp.IsZero() {
		return false // never sampled (a test model, or the executable was unreadable at start)
	}
	fi, err := os.Stat(selfExe())
	if err != nil {
		return false // it moved or vanished; that is not something to nag about mid-board
	}
	return !fi.ModTime().Equal(m.selfStamp)
}

// ── the loading indicator ────────────────────────────────────────────────────────────────────────

// busyFrames is the spinner. Braille because it animates in place at a constant width — a spinner
// built from / - \ | jitters, and one built from box characters cannot show partial motion.
var busyFrames = []rune("⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏")

// busyBarWidth is the sweep's track. Wide enough to read as motion, narrow enough to sit under a
// line of text without becoming the loudest thing on an otherwise empty screen.
const busyBarWidth = 24

// loadingLines is what the board shows before its first read lands.
//
// A static "reading the board…" is indistinguishable from a hang, and a foreign board is a network
// round trip to somebody else's tracker that can genuinely take seconds. So: a spinner that moves,
// a sweep that moves, and the elapsed seconds once it has been long enough that you would start to
// wonder — which is the number that tells you whether to keep waiting.
func (m *boardModel) loadingLines(w, h int) []string {
	what := m.busy
	if what == "" {
		what = "reading the board"
	}
	step := 0
	if !m.busySince.IsZero() {
		step = int(time.Since(m.busySince) / busyPulse)
	}

	spin := string(busyFrames[((step%len(busyFrames))+len(busyFrames))%len(busyFrames)])
	head := boardCol + spin + reset + " " + boardMid + what + "…" + reset

	// A sweep, not a percentage: nothing here knows how far along the read is, and a bar that
	// invents a fraction is a bar that lies. This one says "still working", honestly.
	pos := step % (busyBarWidth * 2)
	if pos >= busyBarWidth {
		pos = busyBarWidth*2 - pos - 1
	}
	var bar strings.Builder
	for i := 0; i < busyBarWidth; i++ {
		switch {
		case i == pos:
			bar.WriteString(boardCol + "█" + reset)
		case i == pos-1 || i == pos+1:
			bar.WriteString(boardMid + "▓" + reset)
		default:
			bar.WriteString(boardDim + "░" + reset)
		}
	}
	...
	put := func(row int, content string) {
		...
		pad := max(0, (w-visWidth(content))/2)
		lines[row] = padVis(strings.Repeat(" ", pad)+content, w)
	}
	mid := h / 2
	put(mid-1, head)
	put(mid, bar.String())
	// Only once waiting is long enough to be worth measuring; a counter from zero on every refresh
	// makes a fast board look slow.
	if el := time.Since(m.busySince); !m.busySince.IsZero() && el > 2*time.Second {
		put(mid+2, boardDim+fmt.Sprintf("%ds", int(el.Seconds()))+reset)
	}
	return lines
}

// tileLinesLifted draws the card in hand: amber, bold-railed, marked ✥ — unmistakably picked
// up, not merely selected. During a hop (carryReveal > 0) only the first lines draw, so the
// tile visibly unfolds at its new position: that partial-then-full redraw IS the animation.
func (m *boardModel) tileLinesLifted(c api.BoardCard, w int) []string {
	lift := boardWarn + "\x1b[1m"
	tw := tileTextWidth(w)
	inner := tw + 2

	top := "╭─✥ carrying " + strings.Repeat("─", max(0, inner-12)) + "╮"
	bar := func(s string) string { return lift + s + reset }
	edge := bar("│")

	lines := []string{bar(top)}
	for _, t := range tileTitleLines(c, w) {
		lines = append(lines, edge+" "+padVis("\x1b[1m"+t+reset, tw)+" "+edge)
	}
	state, _ := cardState(c)
	lines = append(lines, edge+" "+padVis(boardMid+clipVis(state, tw)+reset, tw)+" "+edge)
	lines = append(lines, bar("╰"+strings.Repeat("─", inner)+"╯"))
	lines = append(lines, strings.Repeat(" ", w))
	for i := range lines[:len(lines)-1] {
		lines[i] = padVis(lines[i], w)
	}
	if m.carryReveal > 0 && m.carryReveal < len(lines) {
		lines = lines[:m.carryReveal]
	}
	return lines
}
