...
func (m *boardModel) headerLines(w int) []string {
	counts := ""
	urgent := countUrgent(m.board)
	if m.board != nil {
		counts = fmt.Sprintf("%s%d backlog · %d building · %d blocked · %d review%s",
			boardDim, len(m.board.Backlog), len(m.board.Building), len(m.board.Blocked), len(m.board.Review), reset)
	}
	if urgent > 0 {
		counts += fmt.Sprintf("   %s%d need you%s", boardWarn, urgent, reset)
	}

	title := boardWordmark() + "  " + boardDim + "work board" + reset
...
func (m *boardModel) tabStrip(w int) string {
	...
	build := func(short bool) string {
		start, count := visibleColumns(w, m.col, len(api.BoardColumns))
		var parts []string
		for i, col := range api.BoardColumns {
			n := 0
			if m.board != nil {
				n = len(m.board.Column(col))
			}
			name := col.Title()
			if short {
...
func (m *boardModel) bodyLines(w, h int) []string {
	...
	for i := 0; i < count; i++ {
		...
		cols[i] = m.columnLines(api.BoardColumns[start+i], cw, h, start+i == m.col)
	}
...
func (m *boardModel) columnLines(col api.BoardColumn, w, h int, focused bool) []string {
	...
	head := fmt.Sprintf("%s (%d)", col.Title(), len(m.board.Column(col)))
	if focused {
