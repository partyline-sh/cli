...
func cgDrive(t *testing.T, keys string, fn func()) string {
	return cgDriveChunks(t, []string{keys}, fn)
}

// cgDriveChunks is cgDrive with the input delivered in SEPARATE writes, so a test can prove what
// happens when one logical thing (a bracketed paste of a few KB) arrives across several reads — which
// is what a real clipboard does, and the reason the compose field buffers rather than acting per-read.
func cgDriveChunks(t *testing.T, chunks []string, fn func()) string {
	t.Helper()
	...
	for _, k := range chunks {
		if _, err := ptmx.WriteString(k); err != nil {
			t.Fatalf("feeding keys: %v", err)
		}
		if len(chunks) > 1 {
			time.Sleep(20 * time.Millisecond) // force the reader to see separate reads
		}
	}
	select {
	case <-done:
	case <-time.After(5 * time.Second):
		t.Fatalf("the modal never returned for keys %q — it can be got stuck in", chunks)
	}
	...
	if *before != *after {
		t.Errorf("terminal state was not restored after keys %q", chunks)
	}
