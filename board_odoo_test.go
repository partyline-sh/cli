...
// The tracker's own number is on the tile: a person reading the board has to be able to quote
// the ticket to somebody working in Odoo. It rides the top border, so every line of the tile
// stays exactly as wide as it was.
func TestTileCarriesTheForeignRef(t *testing.T) {
	m := &boardModel{}
	card := api.BoardCard{ID: "28188", Task: "Decide grace-renew", Column: api.BoardColumn("205"), Foreign: true, Ref: "#28188"}

	lines := m.tileLines(card, 30, false)
	if !strings.Contains(lines[0], "#28188") {
		t.Fatalf("top border carries no ref: %q", lines[0])
	}
	w0 := visWidth(lines[0])
	for i, l := range lines[1 : len(lines)-1] {
		if got := visWidth(l); got != w0 {
			t.Errorf("line %d is %d wide, border is %d — the tile would tear", i+1, got, w0)
		}
	}
	// selected: the marker and the ref coexist, same width
	sel := m.tileLines(card, 30, true)
	if !strings.Contains(sel[0], "#28188") || !strings.Contains(sel[0], "▸") {
		t.Errorf("selected tile lost the ref or the marker: %q", sel[0])
	}
	if visWidth(sel[0]) != w0 {
		t.Errorf("selected border %d != unselected %d", visWidth(sel[0]), w0)
	}
	// too narrow: the ref is dropped, never truncated into a broken border
	narrow := m.tileLines(card, 14, true)
	if visWidth(narrow[0]) != visWidth(narrow[1]) {
		t.Errorf("narrow tile tore: border %d vs body %d", visWidth(narrow[0]), visWidth(narrow[1]))
	}
	// a partyline card has no ref and looks exactly as it did
	plain := m.tileLines(api.BoardCard{ID: "u", Task: "local", Column: api.ColBacklog}, 30, false)
	if strings.Contains(plain[0], "#") {
		t.Errorf("partyline tile grew a ref: %q", plain[0])
	}
}
