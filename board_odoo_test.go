...
// Without a project chosen there is no board to read, and the message has to say which key fixes it.
func TestOdooWithoutAScopeSaysWhichKey(t *testing.T) {
	s := fleetSource(t, nil)
	_, err := s.Load("")
	if err == nil || !strings.Contains(err.Error(), "p") {
		t.Fatalf("err = %v, want it to name the scope key", err)
	}
}
