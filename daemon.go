...
// pendingLaunch is a launch request received on the stream, awaiting the owner's confirm.
// The join ref is held only in memory (never persisted) and re-delivered fresh on reconnect.
...
func daemonRun() {
	...
	for {
		err := api.DaemonStream(ctx, d.Base, d.Token,
			...
			func(ev api.LaunchEvent) {
				mu.Lock()
				pending[ev.RequestID] = pendingLaunch{ref: ev.PartyJoinRef, label: ev.ProjectLabel, preset: ev.Preset, party: ev.PartyID}
				mu.Unlock()
...
// daemonApprove runs the full owner-confirmed launch: local registry check (decline if the
// label isn't ours), accept → exchange the single-use ref for a party link → resolve against
// the LOCAL registry → spawn detached → report status. Each failure marks the request failed
...
func daemonApprove(d daemonDevice, mu *sync.Mutex, pending map[string]pendingLaunch, id string) {
	...
	reg := loadDaemonRegistry()
	if projectByLabel(reg, p.label) == nil {
		_ = api.SetLaunchStatus(d.Base, d.Token, id, "declined", "unknown project: "+p.label)
		clearPending(mu, pending, id)
		fmt.Printf("declined %s — no project %q in this machine's registry\n", id, p.label)
		return
	}
	fail := func(stage string, err error) {
		_ = api.SetLaunchStatus(d.Base, d.Token, id, "failed", stage+": "+err.Error())
		clearPending(mu, pending, id)
		fmt.Printf("✗ %s failed at %s (%v)\n", id, stage, err)
	}
	if err := api.SetLaunchStatus(d.Base, d.Token, id, "accepted", ""); err != nil {
		fmt.Printf("approve failed (%v)\n", err)
		return
	}
	link, _, err := api.ExchangeJoinRef(d.Base, d.Token, id, p.ref)
	if err != nil {
		fail("exchange", err)
		return
	}
	argv, dir, err := resolveLaunch(reg, launchRef{ProjectLabel: p.label, PartyLink: link})
	if err != nil {
		fail("resolve", err)
		return
	}
	pid, logPath, err := spawnLaunch(argv, dir, p.label)
	if err != nil {
		fail("spawn", err)
		return
	}
	recordLaunch(id, pid, p.label) // so `daemon kill <id>` (a separate process) can find it
	_ = api.SetLaunchStatus(d.Base, d.Token, id, "spawned", "")
	clearPending(mu, pending, id)
	fmt.Printf("✓ launched %q in %s (pid %d)\n  log: %s\n", p.label, dir, pid, logPath)
}
