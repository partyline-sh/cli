...
func runWorkerStreaming(ctx context.Context, cmd *exec.Cmd, timeout time.Duration, onLog func(string)) (workerOutcome, error) {
	...
	var (
		resultText      string
		resultTokens    int     // Total (input+output+cache_create+cache_read) — the O.5 ceiling signal
		resultFresh     int     // input+output — the honest spend to DISPLAY (excludes cached context)
		resultCacheRead int     // cache_read only — shown as a muted "+N cached" detail, never the headline
		resultCost      float64 // total_cost_usd claude reports — the unambiguous cost figure
	...
	for sc.Scan() {
		...
		if res != nil {
			...
			resultFresh, resultCacheRead, resultCost = res.Usage.Fresh(), res.Usage.CacheReadInputTokens, res.CostUSD
			if res.SessionID != "" {
	...
	if ctx.Err() == context.DeadlineExceeded {
		...
		return workerOutcome{text: fallback, tokens: resultTokens, freshTokens: resultFresh, cacheReadTokens: resultCacheRead, costUSD: resultCost, resumeHandle: resumeHandle, rateReset: rlReset, rateBlocked: rlBlocked, rateNote: rlNote, invokedSkills: invoked}, fmt.Errorf("hit the %s time budget — stopped", timeout)
	}
	if sawResult {
		return workerOutcome{text: resultText, tokens: resultTokens, freshTokens: resultFresh, cacheReadTokens: resultCacheRead, costUSD: resultCost, resumeHandle: resumeHandle, rateReset: rlReset, rateBlocked: rlBlocked, rateNote: rlNote, invokedSkills: invoked}, waitErr
	}
...
type workerStreamEvent struct {
	...
	CostUSD   float64         `json:"total_cost_usd"` // claude's own cost figure on the terminal result event
...
func humanizeStreamEvent(raw []byte) (lines []string, result *eng.Result) {
	...
	case "system":
		...
		return nil, nil
	case "result":
		r := &eng.Result{Text: ev.Result, Usage: ev.Usage, CostUSD: ev.CostUSD, SessionID: ev.SessionID}
		return nil, r
	case "assistant":
