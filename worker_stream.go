...
func runWorkerStreaming(ctx context.Context, cmd *exec.Cmd, timeout time.Duration, onLog func(string)) (workerOutcome, error) {
	...
	if ctx.Err() == context.DeadlineExceeded {
		...
		return workerOutcome{text: fallback, tokens: resultTokens, resumeHandle: resumeHandle, rateReset: rlReset, invokedSkills: invoked}, fmt.Errorf("hit the %s time budget — stopped", timeout)
	}
	if sawResult {
		return workerOutcome{text: resultText, tokens: resultTokens, resumeHandle: resumeHandle, rateReset: rlReset, invokedSkills: invoked}, waitErr
	}
	...
	return workerOutcome{text: stderr.String(), resumeHandle: resumeHandle, rateReset: rlReset, invokedSkills: invoked}, waitErr
}
