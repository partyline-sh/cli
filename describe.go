...
func describeMain(args []string) {
	...
	if threadID == "" {
		fmt.Println("cancelled — nothing recorded")
		return
	}
	...
	for {
		stop := describeWaiting("the engine to answer")
...
// describeLine reads one REPL line at the `> ` prompt. /quit and /exit are honoured HERE, at every
// prompt in the loop — they used to be forwarded to the model at two of the three, which read as
// the command being ignored. ok=false means quit (or stdin closed).
func describeLine(in *bufio.Scanner) (string, bool) {
	fmt.Print("> ")
	if !in.Scan() {
		return "", false
	}
...
// describeWaiting shows a ONE-line elapsed counter, redrawn in place, while a turn runs: a turn can
// take the full 4-minute timeout, and silence for four minutes is indistinguishable from a hang.
// Terminal-only — a \r counter in a pipe or a log file is noise, not progress. Returns the stopper,
// which waits for the ticker goroutine before erasing the line so nothing prints after it.
func describeWaiting(what string) func() {
	if !stdoutIsTTY() {
		return func() {}
	}
	...
	go func() {
		...
		for {
			...
			case <-done:
				return
			case <-t.C:
				fmt.Printf("\r  %s\x1b[K", dim(fmt.Sprintf("… waiting for %s — %ds (ctrl-c aborts)", what, int(time.Since(start).Seconds()))))
			}
		}
	}()
	return func() {
...
func resolveDescribeThread(client *api.Client, flagID string) string {
	...
	// thread — created lazily, umbrella-walked — so `ptln plan` in a repo needs no flags at all.
	...
	// A typo or a closed stdin used to `fatal` here. Pick cancels instead and the caller stops
	// cleanly — nothing has been created yet, so there is nothing to be brave about.
	fmt.Println("\nFile this item under which thread?")
	n, ok := Pick("number", threads, func(t api.Thread) string { return t.Title })
	if !ok {
		return ""
	}
	return threads[n].ID
...
func runInterviewTurn(dir, engineName, prompt, resume string, timeout time.Duration) (string, string, error) {
	...
	groupSpawn(cmd) // #814 — killable as a tree; see proctree.go
	out, err := cmd.Output()
	if ctx.Err() == context.DeadlineExceeded {
		return "", "", fmt.Errorf("timed out (>%s)", timeout)
	}
...
func confirmAndCreate(client *api.Client, threadID, kind string, item reqItem, in *bufio.Scanner) bool {
	...
	fmt.Println()
	yes, ok := Confirm("record this to the backlog?", false)
	if !ok {
		return true // cancelled outright — stop the interview, don't loop back for edits
	}
	if !yes {
		return false
	}
...
STEP 0 — SEARCH BEFORE YOU SPECIFY. Kept in sync with the live describe preamble (web party-modes.ts).
Before any decomposition, call the search_work_items tool with the idea's key terms (a few words, not the
whole spec; try a second phrasing if the first returns nothing) to check whether the team has ALREADY
planned this. If a returned item genuinely covers the same ground — read the snippet, don't match on a
shared word — say so in "assessment" before anything else: name it (kind + title + status), give the link
/work/compare?party=<this party's id>&b=<the existing item's id> — the tool hands you that line already
built, so copy it rather than assembling one, and drop the party part if you have no party id (the item id
alone is still useful) — state in a line what it covers that this idea does and doesn't,
and RECOMMEND either extending that item or creating a new one, saying which of the two looks more
comprehensive (more tasks, more acceptance criteria, more detail). Then still emit your plan: this is
ADVICE and it never blocks anything. If nothing matches, or the tool is not available to you here, say
nothing about it and proceed exactly as normal.
...
ASSERT THE OUTCOME, NEVER THE SHAPE. A criterion says what must be TRUE when the work lands. It must
NOT dictate where the work lives — no filename you invented, no test name nobody has written yet, no
symbol you chose. Where code goes is the builder's decision, and a task that fails because a file was
named differently has tested nothing about the result.

The test to apply: picture two competent engineers building this with different layouts and different
names. A good criterion holds for BOTH of them. If it only holds for the version you happen to be
imagining, you are testing your guess, not the work.

<example>
Task: an export endpoint must reject requests for more than 10,000 rows.
BAD  — "the file src/export/limit.ts exists and exports checkLimit()"
       (dictates layout and naming; fails correct work that organised it differently)
GOOD — "requesting 10,001 rows returns a refusal rather than data, and requesting 10,000 succeeds"
       (holds however it is built, and cannot pass before the work is done)
</example>
...
ASK ONLY WHAT THE HUMAN ALONE CAN ANSWER. Anything you could settle by reading the repo, you settle
by reading the repo — asking it is the annoying-junior move and it burns the attention you need for
the real forks. Save your questions for what is genuinely theirs: scope, product behaviour, the
trade-off between two workable approaches. When you decide something yourself because it was not
specified, say so in "assessment" rather than presenting it as given.

ASSERT THE OUTCOME, NEVER THE SHAPE. Each acceptance criterion says what must be TRUE when the work
lands — not where the code lives. No filename you invented, no test name nobody has written, no
symbol you chose; where it goes is the builder's call. The test: picture two competent engineers
building this with different layouts. A good criterion holds for both.

<example>
BAD  — "the file src/export/limit.ts exists and exports checkLimit()"
GOOD — "requesting 10,001 rows returns a refusal rather than data, and 10,000 succeeds"
</example>
