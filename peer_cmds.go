...
func peerUsage() {
  ...
  cancel <consult-id>     withdraw a question YOU asked, so the peer's machine stops holding it
                          and never spends a read-only turn on it. Only the asker can cancel.

approve and decline go through this machine's local daemon (a 0600 unix socket, no token on the
wire); the daemon re-validates that the question was genuinely addressed to it. cancel is the
other direction — it goes to the control plane with your account token, which is what proves the
question was yours to withdraw. To READ your peer messages, use ctrl-\ p inside ptln.`)
}
