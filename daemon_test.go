...
// The consult caps were documented as the machine-wide way to stop auto-answer while the installed
// unit inherited only PATH and PARTYLINE_API — so exporting them did nothing for the common install.
// Baking them is the convenience half of the fix (the persisted switch is the real one), and it must
// stay conditional: an unset variable must not appear in the unit at all, or an empty value would look
// like a deliberate setting.
func TestServiceUnitBakesConsultCapsOnlyWhenSet(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	render := func() string {
		if runtime.GOOS == "linux" {
			return systemdUnit()
	...
	if runtime.GOOS != "darwin" && runtime.GOOS != "linux" {
		t.Skipf("no unit format on %s", runtime.GOOS)
	}

	t.Setenv(envConsultAutoDaily, "")
	t.Setenv(envConsultAutoDailyTotal, "")
	t.Setenv("PARTYLINE_API", "")
	unset := render()
	for _, name := range []string{envConsultAutoDaily, envConsultAutoDailyTotal, "PARTYLINE_API"} {
		if strings.Contains(unset, name) {
			t.Fatalf("unset %s must be omitted from the unit entirely:\n%s", name, unset)
		}
	}

	t.Setenv(envConsultAutoDaily, "0")
	t.Setenv(envConsultAutoDailyTotal, "5")
	t.Setenv("PARTYLINE_API", "https://example.test")
	set := render()
	for _, want := range []string{envConsultAutoDaily, "0", envConsultAutoDailyTotal, "5", "PARTYLINE_API", "https://example.test"} {
		if !strings.Contains(set, want) {
			t.Fatalf("unit missing %q once set:\n%s", want, set)
		}
	}
	if strings.Count(set, envConsultAutoDaily) != 2 { // the name, plus the _TOTAL that contains it
		t.Fatalf("expected exactly one entry per cap:\n%s", set)
	}
}
...
func TestRunRefFromEventResolves(t *testing.T) {
	...
	ev := api.RunEvent{RunID: "11111111-2222-3333-4444-555555555555", ProjectLabel: "proj", ThreadID: "plt-thr-1", Tasks: []string{"do x"}, Preset: "build"}
