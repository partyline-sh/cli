...
COMMANDS
  ...
  work           run ONE task autonomously in a sandboxed worktree, then leave a
                 ...
                 [--thread <id>] [--allow-bash]. Read/edit tools only by default;
  ...
  shape          shape a plan: the agent interviews a rough idea into a well-specified,
                 ...
                 your local claude (your auth, no server key). In a registered repo it
                 needs NO flags: the project implies the thread (umbrella-aware, created
                 on first use). ptln shape [--kind epic|feature|task] [--thread <id>]
                 [--quick]. /quick /deep switch modes, /done, /quit. (alias: describe)
  crank          drive a BACKLOG one task at a time, each in its own worktree, sharing
                 ...
                 [--max N] [--max-tokens N] [--halt-on-fail K]. Halts on
                 cap/tokens/failures; prepares N branches for review — nothing
                 is pushed or merged.
...
DAEMON (remote launch — let the web start agents on this machine)
  ...
  ptln daemon add-project <label> [dir] [--preset spec|chat]
