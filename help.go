...
COMMANDS
  ...
  login [url]    authenticate this machine (device flow). A url logs in to a
                 self-hosted instance and pins its identity key, printing the
                 fingerprint; --accept-new-key re-pins after a key change
  setup          connect this machine end to end: account → always-on worker →
                 engine → code locations → projects → PRs → agent memory. Runs
                 after every login; asks only about what's missing.
  ...
  party context  switch a LIVE party's persona/project, keeping the conversation
  ...
  wt             session worktrees — list/remove the isolated dirs that
                 "ptln new <tool> --worktree <name>" and crank create. "ptln wt
                 prune" clears finished crank worktrees (dry run; --yes applies,
  ...
  join-mcp [link] add a Party to the LLM session you're already in (MCP), so it
                 ...
                 Posts from that session go out under YOUR partyline login, so
                 the party reads them as you; without one they read as an agent
                 (run "ptln login")
  ...
  trigger        inbound entry points — an address other software POSTs to, which
                 starts work here (deploy triage, ticket intake). Configurable
                 entirely from the CLI, so an agent with your shell can set it up
                 for you: ptln trigger help
                 or on a CLOCK: ptln trigger set <slug> --cron '0 3 * * *'
                 what they have DONE: ptln trigger activity · trigger log <slug>
  template       reusable agent personas a trigger can run: ls | show | create | rm
                 one template, many triggers — written once, not retyped
  webhook        outbound — where your team's events go: ls | add | rm
  chat           reach your projects from Telegram or Discord: link | unlink
                 one bot, many personas — say "let's plan a feature for <project>"
                 and it switches persona + project without losing the thread
  ...
  project        projects — ls | new | show <label> (machines, runs, threads)
                 setup [<label>]  set THIS repo up as a project — creates it, pins its
                 thread in .partyline.json, and REGISTERS this directory so your team's
                 agents may build here unattended (undo: daemon remove-project <label>)
  ...
  server         self-host diagnostics for a partyline BOX (not your laptop):
                 "ptln server doctor" reads features.json — the registry of which
                 env vars gate which feature — and reports each one configured or
                 NOT configured, naming the variables a not-configured one is
                 missing and the next step. Names and set/unset only: a value is
                 never printed, so the output is safe to paste into an issue.
                 "ptln server bootstrap" checks a fresh box (docker, compose,
                 the stack's ports, free disk, whether deploy/stack is on the
                 machine, which required variables are unset) and PRINTS the
                 exact ordered install commands, naming only paths it confirmed
                 exist. It never writes .env, generates a secret, or runs
                 docker. Exit 1 if a prerequisite is missing. --json for the
...
SESSION FLAGS
  ...
  --invite <targets>   comma-separated emails and/or teammate @handles to invite when
                       the session opens. Prints who was actually reached; a target that
                       resolved to nobody, and one that resolved but wasn't delivered,
                       are each named — neither is ever counted as sent.
  ...
  --relay <host:port>  relay to use (default: the relay your instance registers; '' to disable)
...
IN A SESSION
  ...
  typed:        /pwho · /phud [on|off] · /pgrant [name] · /pinvite <email|@handle> · /plock · /pexit · /phelp
...
DAEMON (remote launch — let the web start agents on this machine)
  ...
  ptln daemon scan-root add|rm|ls [dir]   advertise repos from extra directories
                                   (beyond your home dir — e.g. a /mnt/data mount)
...
MCP TOOLS (the partyline-context-threads server — auto-wired for claude + codex)
  ...
  send_to_partyline          send work to the backlog to be BUILT — one task, or a whole
                             epic ▸ feature ▸ task decomposition. Checks it is actually
                             buildable first (a real project, a command that verifies it,
                             a spec beyond the title) and hands back the questions to ask
                             you when something is missing, rather than filing something
                             that would sit there refusing to start
  ...
  setup_read                 what THIS instance still needs: every setup step with its
                             state (done / todo / blocked) and what to do about it, which
                             step is next, and whether it is set up at all. Recomputed
                             from the world every call, never a "finished" flag
  setup_write [instance_name] [allow_signups]
                             name this deployment and open or close signups — the two
                             things the app is allowed to change about itself. Everything
                             else is .env plus a restart. Instance-admin only (the first
                             account to sign in owns the instance); anyone else is told so

DOCS  https://partyline.sh/docs
      https://partyline.sh/llms-full.txt — the whole product as one plain-text file, for an
      AI assistant that has nothing installed. No account needed; fetch it or paste it in.
`)
}
