...
USAGE
  ...
  ptln new <tool>      start a fresh AI session — claude | codex | gemini | opencode | antigravity
...
COMMANDS
  ...
  work           run ONE task autonomously in a sandboxed worktree, then leave a
                 ...
                 claude|codex|gemini|opencode overrides; codex requires --allow-bash,
...
PARTY (humans + agents)
  ptln party '<link>' --name <name> --role "<what it does>" [--cmd "<agent>"]
                ...
                Runs --engine claude by default (codex/gemini/opencode/antigravity too);
