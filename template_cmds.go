...
func templateList(args []string) {
	ts, err := apiClientOrFatal().ListTemplates()
	if err != nil {
		fatal(err)
	}
...
func templateShow(args []string) {
	...
	full, err := apiClientOrFatal().GetTemplate(t.ID)
	if err != nil {
...
func templateRm(args []string) {
	...
	t := findTemplate(args[0])
	detached, err := apiClientOrFatal().DeleteTemplate(t.ID)
	if err != nil {
...
func templateCreate(args []string) {
	...
	id, err := apiClientOrFatal().CreateTemplate(name, body, stop, status)
	if err != nil {
		fatal(err)
	}
