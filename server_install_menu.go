...
type menuField struct {
	...
	edit  func(installConfig, installOps) (installConfig, bool, string)
}
	...
	return []menuField{
		...
		},
		{
			...
			note: func(c installConfig, _ installOps) string {
				if strings.TrimSpace(c.dns) == "" {
					// MagicDNS lives on the HOST. A container asking the Docker resolver for
					// a .ts.net name gets nothing, and the web container must resolve the
					// site to reach the identity provider.
					if _, host, _ := strings.Cut(c.site, "://"); strings.HasSuffix(stripHostPort(strings.TrimRight(host, "/")), ".ts.net") {
						return "SET THIS to 100.100.100.100 — containers cannot use MagicDNS"
					}
		...
		},
		{
			...
			note: func(c installConfig, _ installOps) string {
				...
				case tlsOff:
					if tunnelFronted(c) {
						return "plain HTTP behind YOUR tunnel (Tailscale/Cloudflare terminates TLS)"
					}
		...
		},
		{
			...
			value: func(c installConfig) string {
				if c.minio {
					return "bundled MinIO"
				}
				return "off"
			},
			note: func(c installConfig, _ installOps) string {
				if !c.minio {
					return "attachments dark; two fewer containers"
				}
				return "attachments on (default)"
			},
			edit: editStorage,
		},
		{
...
func runSetupMenu(cfg installConfig, ops installOps) (installConfig, bool) {
	...
	// status is the last thing worth telling the operator — a rejected value, a watch result.
	// It is rendered INSIDE the frame, because anything printed below it is wiped by the next
	// paint's screen-clear after one visible frame. That flash is how a rejected port read as
	// the menu ignoring the keystroke.
	status := ""
	for {
		...
		lines = append(lines, "")
		if status != "" {
			lines = append(lines, "  "+brand.Fg(brand.AmberRGB)+status+cgOff)
			status = ""
		}
		if ready {
		...
		case answer == "" && ready:
			return cfg, true
		case answer == "":
			status = why // repeat the blocker where it will actually be seen
			continue
		...
		if convErr != nil || n < 1 || n > len(fields) {
			status = fmt.Sprintf("Enter a number between 1 and %d, or press enter.", len(fields))
			continue
		}
		updated, ok, msg := fields[n-1].edit(cfg, ops)
		if ok {
			cfg = updated
		}
		status = msg
...
func menuReady(cfg installConfig, ops installOps) (bool, string) {
	...
	if ops.lookup != nil && ops.localIPs != nil {
		...
		case dnsResolvesLoopback:
			// SHORT, because the frame clips at its edge: the first wording put the usable
			// address past the ellipsis, which is the one part that matters.
			hint := "an address other machines hold"
			if ips := ops.localIPs(); len(ips) > 0 {
				hint = "http://" + ips[0]
			}
			return false, "Hosts-file alias, this machine only — use " + hint
