...
and `ptln man` cover all of it; the hosted control plane at partyline.sh adds the planning
board and the Build / Ship pipeline on top.
...
Two licences, drawn along one line: **the client is yours, the server is ours to
run as a service.**

| | |
|---|---|
| **`ptln`, the CLI** | **MIT** — published at [partyline-sh/cli](https://github.com/partyline-sh/cli). Use it however you like, against our servers or your own. |
| **This repository** (web app, control plane, self-host stack) | **[Elastic Licence 2.0](LICENSE)** |
...
commercially, inside your own company, with no seat count and no key to ask us
for. [`/docs/self-host`](https://partyline.sh/docs/self-host) exists so you can.

The one thing it forbids is offering this software to third parties as a hosted
or managed service — i.e. selling partyline-as-a-service. That is the business,
and it is the only restriction.
