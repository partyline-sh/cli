...
1. **`ptln llms` — your AI session manager.** Find, resume, and run every Claude Code /
   Codex / Gemini / `llm` session on your machine from one launcher — and host several at
   once in a single multiplexed terminal instead of a tab per agent.
2. **Share any session, live.** Hand a teammate a link and they watch your terminal in real
   time over an end-to-end-encrypted, blind relay — pair on a bug, review an agent's work,
   run an incident.
...
- **Run several in one terminal.** Open sessions as windows in one multiplexer and switch
  with **`ctrl-\ w`** (a picker), cycle `n`/`p`, jump `1`–`9`. No more tab-per-agent.
...
```
host:    ptln                       # session up — share the printed join link
...
```
   ...
   │ time ("windows"), switched with ctrl-\ w / n / p / 1–9:       │
   │                                                               │
...
- **Sharing bridges to the relay.** `S` (and bare `ptln` / `ptln start`) hosts a session via
