...
Beyond those, the same binary carries the rest of the toolkit — project memory
(`ptln memory`), the work board (`ptln board`), autonomous single-task runs (`ptln work`)
and backlog cranking (`ptln crank`), the Planning agent (`ptln plan`), parties (humans +
agents on one channel), Context Threads, an org skill library (`ptln skill`), a macOS
menu-bar tray, and `ptln state` for scripts. `ptln help` and `ptln man` cover all of it.
...
board, teams, projects and parties live on a partyline **server you run**, which this repo
builds:
...
- **Run several in one terminal.** Open sessions as windows in one multiplexer and switch
  from the tab bar — **`ctrl-\ ←/→`** (⏎ to commit) or **`ctrl-\ 1`–`9`** to jump.
  **`ctrl-\`** on its own opens the menu: every command, with its key. No more tab-per-agent.
...
- **New session** (`n` with options, `N` right now), **pin**, **rename**, **diff**,
  **split** two sessions into one tab (`|`), and switch **colour themes** (`t`).

```
↑↓ move · ⏎ open · space multi-select · | split · o new tab · S share
 n new · d diff · / search · s sort · t theme · R refresh · ? all keys
```

`?` prints the full list; it is generated from the same table the manager binds, so it
...
## Project memory — what the team has learned

```sh
ptln memory brief                   # what a session is told before you type
ptln memory add gotcha "the POS callback retries 3x with no jitter"
```

A project is a set of repositories with one memory repo they share. Facts travel by git,
so a teammate reads them without an account and without your repos cloned. Agents read and
write it through the `project_memory` and `remember` MCP tools; sessions are briefed at
start and again whenever a teammate records something.

Full docs: <https://partyline.sh/docs/memory>

## Context Threads — one effort's working feed

Project memory is what the team knows. A **Context Thread** is the working feed for a
single effort — a team-scoped, attributed timeline of decisions, constraints and contracts
that any session can read and add to:
...
ptln new claude --thread <id>       # launch an agent wired to it — reads it at startup,
                                    # and can recall/curate it via MCP tools
```
...
`remember` writes to project memory, not to a thread.
...
Your roadmap lives in Jira, Linear, Productboard, GitHub or a spreadsheet. partyline is the
execution layer, not a second backlog, so bring items across and keep them linked.

Two doors:

- **`ptln board`** — connect the tracker as a board source (any MCP server that declares a
  board; an Odoo provider ships with partyline). `S` switches source, `p` picks the project,
  `i` imports the focused card as planned work.
- **`POST /api/v1/work-items/import`** — for scripts. Keyed on `source_tool` + `source_id`,
  so re-importing a roadmap resumes the same conversation instead of starting a second one.

partyline holds no tracker credentials and learns no tracker API. What travels is text.

Each ticket opens a **planning conversation** seeded with it verbatim — not a task. A ticket
states a problem; the conversation produces buildable work. Importing never starts work.

Full docs: <https://partyline.sh/docs/import>

## API keys & webhooks — connect the rest of your stack

Other software can hear about work here.
...
A webhook carries **ids and links, never your content**, so nothing your team writes ends up in a third party's request logs.
