...
func answerConsult(d daemonDevice, ev api.ConsultEvent) {
	...
	// The project's consult engine is a SUGGESTION; preferEngine falls back to this machine's local
	// per-project engine when it names one we cannot run. Passing "" here (the old behaviour) meant
	// the project could never express a preference at all.
	engineName, _ := resolveRunEngine(reg, ev.ProjectLabel, ev.ConsultEngine)
	answer, err := runConsultAnswer(proj.Path, engineName, ev.ConsultModel, ev.Question)
	if err != nil {
...
func runConsultAnswer(dir, engineName, model, question string) (string, error) {
	...
	argv, stdinPrompt, err := spec.OneShotArgs(prompt, model, posture)
	if err != nil {
