...
func (o *detailOverlay) lines(m *boardModel, w, h int) []string {
	body := append(o.factLines(m, w), o.logLines(w)...)
...
func (o *detailOverlay) factLines(m *boardModel, w int) []string {
	c := o.card
	state, _ := cardState(c)
	...
	add("state", state)
	add("project", c.Title)
	// The column's NAME, from the board that is loaded. Column.Title() knows partyline's five and
	// falls back to the raw key for anything else, which on an Odoo board printed the stage id.
	//
	// Skipped for a foreign card: its state IS its column there, and the same word twice under two
	// labels reads as two facts.
	if !c.Foreign {
		if m != nil && m.data != nil {
			add("column", m.data.Title(c.Column))
		} else {
...
func (m *boardModel) detailPane(c *api.Client) {
	...
	// Tail to the newest line for a RUN — a build you open mid-flight should show what is happening
	// now. A foreign card has no live tail: it has a description and a conversation, and opening
	// those at the bottom drops you into the last comment with the task itself scrolled away.
	o := &detailOverlay{card: *card, tail: !card.Foreign}
