...
func TestInheritRepoBindSpec(t *testing.T) {
	...
	// Outside any bound repo → untouched.
	if out := inheritRepoBindSpec(ptymux.Spec{Argv: []string{"claude"}, Dir: t.TempDir()}); out.Thread != "" {
		t.Fatalf("unbound dir must stay threadless: %+v", out)
	}
