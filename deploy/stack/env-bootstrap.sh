...
# STORAGE OFF IS A SUPPORTED STATE, expressed as MINIO_REPLICAS=0 written BEFORE this script runs
# (the installer's default). Honour it: writing S3_ENDPOINT=http://minio:9000 against a MinIO that
# will never start swaps the clear "object storage is not configured" error for connection-refused,
# which is strictly worse. Re-enable later: set MINIO_REPLICAS=1, re-run this script, compose up.
if [ "$(get MINIO_REPLICAS)" = "0" ] && ! have S3_ENDPOINT; then
  echo "→ object storage: off (MINIO_REPLICAS=0) — attachments stay dark until you enable it"
else
...
{
  "realm": "partyline",
  "loginTheme": "partyline",
  "displayName": "partyline",
  "displayNameHtml": "<div style=\\"font-family:ui-monospace,monospace;letter-spacing:.35em;font-weight:700;color:#ff9838\\">&#9742;&nbsp;PARTYLINE</div>",
  "enabled": true,
  ...
  "clients": [
    {
      ...
      "attributes": { "post.logout.redirect.uris": "$SITE/*" },
      "webOrigins": ["$SITE"]
    }
  ],
  "users": [
