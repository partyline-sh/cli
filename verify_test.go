...
func TestReadChecks(t *testing.T) {
	...
	if len(got) != len(want) || got[0].Cmd != want[0] || got[1].Cmd != want[1] {
		t.Fatalf("readChecks commands = %+v, want %v", got, want)
	}
	// G.4: an existing verify file has no names, and must not acquire them. An auto-name is a
	// display label; treating it as a policy key would let a settings row bind to a command that
	// changes underneath it.
	for _, c := range got {
		if c.Named {
			t.Errorf("bare command %q was read as a NAMED check", c.Cmd)
		}
	}
}
...
func TestRunChecks(t *testing.T) {
	...
	if vr := runChecks(base, wt, defaultResolved(strings.Join([]string{"true", "true"}, "\n")), timeout); !vr.ran || !vr.ok {
	...
	vr := runChecks(base, wt, defaultResolved(strings.Join(checks, "\n")), timeout)
	if vr.ok || !vr.ran {
...
func TestRunChecksBaselineAware(t *testing.T) {
	...
	vr := runChecks(base, wt, defaultResolved(strings.Join([]string{"exit 1", "touch " + marker}, "\n")), timeout)
	if !vr.ran || !vr.ok {
	...
	vr = runChecks(base, wt, defaultResolved(strings.Join([]string{"exit 1", "test -f ok.txt"}, "\n")), timeout)
	if vr.ok {
...
// defaultResolved is the no-policy resolution — every repo check blocking and always-run — which is
// what these tests have always exercised and what a project that never opens the settings page gets.
func defaultResolved(body string) []resolvedCheck { return applyPolicy(parseChecks(body), nil, nil) }

// ---- G.6: the policy half, which until now had no production caller at all ----
//
// project_checks has existed as a table, applyPolicy as a function, and neither reached a run: the
// gate went straight from readChecks to runChecks. These tests pin the wire, not the pieces.

// THE REASON G.4 EXISTS. A check that cannot pass yet — partyline's own lint has 38 pre-existing
// errors — can now be watched without rejecting every clean diff. It must RUN (so a human sees the
// output) and must NOT quarantine.
func TestAnAdvisoryCheckRunsAndWarnsInsteadOfFailing(t *testing.T) {
	base, wt := t.TempDir(), t.TempDir()
	// A REGRESSION, not pre-existing debt: it passes on base and fails in the worktree. Written this
	// way deliberately — a check that fails on base too is warned about by the baseline-aware path
	// that predates G.4, so an advisory test built on one would pass without advisory ever working.
	if err := os.WriteFile(filepath.Join(base, "ok.txt"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	marker := filepath.Join(wt, "advisory-ran")
	resolved := applyPolicy(
		parseChecks("lint: touch "+marker+"; test -f ok.txt"),
		[]checkPolicy{{Name: "lint", Enabled: true, Blocking: false}},
		nil,
	)

	vr := runChecks(base, wt, resolved, 10*time.Second)
	if !vr.ok {
		t.Fatalf("an advisory failure quarantined the task: %+v", vr)
	}
	if _, err := os.Stat(marker); err != nil {
		t.Error("the advisory check never ran — watching a check requires executing it")
	}
	if !strings.Contains(vr.warn, "lint") {
		t.Errorf("warn = %q, want the advisory failure surfaced rather than swallowed", vr.warn)
	}
}

// A path glob means a Go-only change stops paying for a full web build. The saving is only real if
// the command genuinely does not execute, and the skip has to be RECORDED — an unexplained absence
// reads as "it passed".
func TestASkippedCheckDoesNotExecuteAndSaysWhy(t *testing.T) {
	base, wt := t.TempDir(), t.TempDir()
	marker := filepath.Join(wt, "should-not-exist")
	resolved := applyPolicy(
		parseChecks("webbuild: touch "+marker),
		[]checkPolicy{{Name: "webbuild", Enabled: true, Blocking: true, PathGlob: "web/**"}},
		[]string{"crank.go"},
	)

	vr := runChecks(base, wt, resolved, 10*time.Second)
	if _, err := os.Stat(marker); err == nil {
		t.Fatal("a check whose glob did not match still executed")
	}
	if vr.ran {
		t.Error("every check was skipped, so the gate did not run — reporting ran=true would let a project glob its way to a green gate having executed nothing")
	}
	if !strings.Contains(vr.warn, "web/**") {
		t.Errorf("warn = %q, want the glob named so a human can see why it was skipped", vr.warn)
	}
}

// A disabled check is resolved away entirely, and the gate must still be able to say it RAN when
// something else did.
func TestADisabledCheckLeavesTheOthersGating(t *testing.T) {
	base, wt := t.TempDir(), t.TempDir()
	resolved := applyPolicy(
		parseChecks("slow: exit 1\nbuild: true"),
		[]checkPolicy{{Name: "slow", Enabled: false}},
		nil,
	)
	vr := runChecks(base, wt, resolved, 10*time.Second)
	if !vr.ran || !vr.ok {
		t.Fatalf("got %+v, want the remaining check to run and pass", vr)
	}
}

// THE TRUST RULE, at the gate rather than in isolation: policy naming a check the repo does not
// declare cannot cause anything to run. This is the whole reason there is no command column.
func TestPolicyCannotMakeTheGateRunSomethingTheRepoNeverDeclared(t *testing.T) {
	base, wt := t.TempDir(), t.TempDir()
	resolved := applyPolicy(
		parseChecks("build: true"),
		[]checkPolicy{{Name: "deploy", Enabled: true, Blocking: true}},
		nil,
	)
	if len(resolved) != 1 || resolved[0].Name != "build" {
		t.Fatalf("resolved = %+v — policy conjured a check", resolved)
	}
	if vr := runChecks(base, wt, resolved, 10*time.Second); !vr.ok {
		t.Fatalf("got %+v", vr)
	}
}
