...
func TestRunChecks(t *testing.T) {
	...
	if vr := runChecks(base, wt, parseChecks(strings.Join([]string{"true", "true"}, "\n")), timeout); !vr.ran || !vr.ok {
	...
	vr := runChecks(base, wt, parseChecks(strings.Join(checks, "\n")), timeout)
	if vr.ok || !vr.ran {
...
func TestRunChecksBaselineAware(t *testing.T) {
	...
	vr := runChecks(base, wt, parseChecks(strings.Join([]string{"exit 1", "touch " + marker}, "\n")), timeout)
	if !vr.ran || !vr.ok {
	...
	vr = runChecks(base, wt, parseChecks(strings.Join([]string{"exit 1", "test -f ok.txt"}, "\n")), timeout)
	if vr.ok {
