...
func realRunner(dir string) cmdRunner {
	// Resolve the operator's LOCAL GitHub token once and inject it as GH_TOKEN so `gh pr create`
	// works even though this daemon (a background service) doesn't inherit the login shell's gh auth.
	// "" just means "let gh use whatever it finds" — unchanged behavior. Harmless for the git calls.
...
func applyMergePolicy(run cmdRunner, branch, title, policy string) (note, prURL string) {
	...
	if err != nil {
		...
		return "pushed; pr create failed: " + firstLine(out, err) + " — run `ptln daemon doctor`; set GitHub PR access with `ptln daemon set-github-token`", ""
	}
