...
func loginMain(args []string) {
	...
	for _, a := range args {
		switch {
		case a == "--no-worker":
			// For a machine that should hold an account token without becoming a worker: a laptop
			// you only drive the board from, or a CI job that just needs to authenticate.
			noWorker = true
		...
		case strings.HasPrefix(a, "-"):
			fatal(fmt.Errorf("ptln login: unknown flag %q (usage: ptln login [<url>] [--no-worker] [--accept-new-key])", a))
		case instance == "":
			instance = strings.TrimRight(strings.TrimSpace(a), "/")
		default:
			fatal(fmt.Errorf("ptln login: unexpected argument %q (usage: ptln login [<url>] [--no-worker] [--accept-new-key])", a))
		}
	}
	...
	// Ask the instance who it is BEFORE anything is written. api.ConfigDir() resolves through the
	// instance registry, so this call is what decides where the token lands — and on an instance
	// that has changed address, it is what makes this machine recognise the partyline it is
	// already enrolled with instead of arriving as a stranger with a fresh, empty directory.
	adoptInstanceIdentity(api.Base())
	...
	if err != nil && api.IsUnknownAuthority(err) {
		...
		if trustInstanceCert(c.Base) {
			...
			// The identity probe above went through the same unpinned trust store and will have
			// failed for the same reason. Now that the pin exists, it can succeed.
			adoptInstanceIdentity(c.Base)
			ds, err = c.DeviceStart()
		}
	}
	if err != nil {
	...
	// Remember WHICH partyline this machine belongs to, so every later command reaches it without
	// PARTYLINE_API. Signing in is the moment that answer becomes known, and leaving it unrecorded
	// is what made a signed-in machine report "not logged in" to its own bare commands.
	if err := api.SaveInstance(c.Base); err != nil {
		fmt.Fprintln(os.Stderr, "note: signed in, but could not record the instance: "+err.Error())
	}
...
// adoptedInstance guards against re-probing within one login: the call sites exist because the
// first attempt can fail on an untrusted certificate, not because the answer changes.
var adoptedInstance bool

// adoptInstanceIdentity learns which instance a URL belongs to and files it, so this machine's
// credentials are keyed by the instance rather than by whatever address it answers on today.
//
// BEST EFFORT, ALWAYS. An instance too old to serve /.well-known/partyline, or one whose database
// is restarting, simply leaves the machine on hostname-keyed config — exactly how it behaved
// before this existed. Nothing here may turn a working login into a failed one.
func adoptInstanceIdentity(base string) {
	if adoptedInstance || strings.TrimSpace(base) == "" {
		return
	}
	id, moved, err := api.AdoptInstance(base)
	if err != nil || strings.TrimSpace(id.InstanceID) == "" {
		return
	}
	adoptedInstance = true
	if moved {
		// Worth saying out loud: the operator moved their instance and this machine just
		// reconnected to the enrolment it already had. Silence here looks identical to a fresh
		// login, which is how the previous behaviour hid a fleet-wide outage.
		fmt.Printf("  ☎ this is the partyline you were already enrolled with — keeping this machine's credentials\n")
	}
}
...
func logoutMain() {
	...
	api.ForgetInstance()   // and stop pointing this machine at that instance
