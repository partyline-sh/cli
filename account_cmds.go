...
func loginMain() {
	...
	// Login is the one moment someone has explicitly said "I want the connected experience", so
	// it's the one ACTIVE setup trigger. Asked once ever; local-only users who never log in are
	// never prompted at all. (setup.go)
	offerSetupAfterLogin()
}
