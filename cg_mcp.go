...
func (s *cgServer) handleCall(enc *json.Encoder, req rpcReq) {
	...
	if s.thread == "" {
		s.toolResult(enc, req.ID, "No thread is attached to this session (PARTYLINE_THREAD_ID is unset). Start it with: ptln llms new <tool> --thread <id>.", true)
		return
	}
	switch p.Name {
	...
	case "remember":
		...
		b, err := s.c.Remember(s.thread, kind, p.Args.Body, s.agent, s.engine, 0)
		if err != nil {
