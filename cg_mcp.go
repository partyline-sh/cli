...
func (s *cgServer) markConnected() {
	...
	// The client is captured BEFORE the goroutine, never read from the struct inside it: handle()
	// reassigns s.c per request (refreshClient), and a background read of that field while the
	// request loop writes it is a data race. A presence ping carrying a slightly older client is
	// fine; a race detector failure in a server people leave running for hours is not.
	c := s.c
	thread, engine := s.thread, s.engine
	go func() {
		...
		_ = c.MarkConnected(thread, engine, machine, branch)
	}()
}

// refreshClient rebuilds the API client when the endpoint or the credential on disk has moved.
//
// Cheap and idempotent: two small file reads, and it returns without allocating when nothing has
// changed, which is the common case. It reports a change on stderr — the server's only log — so the
// next person debugging this sees "the instance moved under me" instead of a bare 401.
func (s *cgServer) refreshClient() {
	base, tok := api.Base(), api.LoadToken()
	if s.c != nil && s.c.Base == base && s.c.Token == tok {
		return
	}
	was := ""
	if s.c != nil {
	...
	s.c = api.New()
	if was != "" && was != base {
		fmt.Fprintf(os.Stderr, "partyline: instance changed under this session (%s → %s); reconnected\n", was, base)
	}
}
...
func gitOriginURL(dir string) string {
	...
	return canonicalRemote(strings.TrimSpace(string(out)), sshResolvedHost)
}

// canonicalRemote rewrites an ssh HOST ALIAS in a remote to the real host it stands for.
// `git@github-acr:owner/repo.git` is a per-machine spelling (two GitHub identities, one box)
// of the same repository every teammate names `github.com/...` — and the server-side
// normalizer rightly refuses a dotless host as an identity, which turned every repo behind
// an alias into "could not ask the control plane" (doctor) and an unstampable project.
// resolve answers what the alias means (ssh -G in production); on any doubt, the raw remote
// goes through untouched.
func canonicalRemote(raw string, resolve func(host string) string) string {
	host, rewrite := "", func(string) string { return raw }
	// scp-style only when there is no scheme: "https://host/path" would otherwise parse as
	// host "https" with the rest as its path.
	if m := aliasScpRe.FindStringSubmatch(raw); m != nil && !strings.Contains(raw, "://") {
		host = m[2]
		rewrite = func(real string) string { return m[1] + real + ":" + m[3] }
	} else if m := aliasSSHURLRe.FindStringSubmatch(raw); m != nil {
		host = m[2]
		rewrite = func(real string) string { return m[1] + real + m[3] }
	}
	// Only a host that cannot be a real one (no dot) is worth an ssh lookup; github.com is
	// already an identity, and rewriting hosts that work would be pure risk.
	if host == "" || strings.Contains(host, ".") || resolve == nil {
		return raw
	}
	real := strings.TrimSpace(resolve(host))
	if real == "" || real == host || !strings.Contains(real, ".") {
		return raw
	}
	return rewrite(real)
}
...
var (
	// The user@ is OPTIONAL. `github-acr:acr-retail/repo.git` is a perfectly ordinary way to
	// write a remote against an ssh alias, and requiring `git@` meant that spelling was never
	// canonicalised: one repo in a project ended up recorded as "github-acr:..." while its
	// siblings were "git@github.com:...". A teammate cloning the same repo the normal way would
	// not match it, so facts about it silently failed to line up across machines.
	aliasScpRe    = regexp.MustCompile(`^([A-Za-z0-9._-]+@)?([^:/]+):(.+)$`)
	aliasSSHURLRe = regexp.MustCompile(`^(ssh://(?:[^@/]+@)?)([^:/]+)((?::\d+)?/.+)$`)
)

// sshResolvedHost asks ssh itself what a host alias means — the one authority on the
// machine's own ssh config. An unknown host echoes back unchanged, which canonicalRemote
// treats as "leave it alone".
func sshResolvedHost(host string) string {
...
var cgToolDefs = append(append(append(append(cgBaseToolDefs, cgRunReadToolDefs...), askSessionTools...), cgSetupToolDefs...), memoryTools...)
...
var cgBaseToolDefs = []map[string]any{
	...
	},
	{
		...
		"description": "MAKE THIS REPO A PARTYLINE PROJECT, END TO END — the \"set this up so we can run work on it\" verb, and the one to reach for when someone asks to set a project up. " +
			...
			"This is the ONLY door that sets a repo up for running work: it covers every machine, not just this one, which is why projects used to end up runnable on exactly one box.",
		"inputSchema": map[string]any{
			"type": "object",
			"properties": map[string]any{
...
func (s *cgServer) handle(line []byte, enc *json.Encoder) {
	...
	// The endpoint and the credential are re-read HERE, per request, not captured at boot.
	//
	// This server outlives the thing it talks to. It is spawned once when an engine session opens
	// and then lives for hours, across `ptln login`, across a token refresh, and across the
	// operator pointing this machine at a different instance. A client built at boot keeps the base
	// URL and token it was born with, and every later call goes to an endpoint that may no longer
	// exist with a credential that may no longer be valid.
	//
	// Observed: after an instance moved to a new hostname, a running session's tools returned
	// "unauthenticated" on every call while `ptln doctor` in the same terminal reported a healthy
	// login — because the guards below check api.LoadToken() (current) and then call through a
	// client holding the OLD one. The session looked signed in and signed out at the same time, and
	// the only fix anyone could find was restarting the engine.
	//
	// The lazy-thread resolve above already accepts that the world changes under a long-lived
	// server; this extends the same treatment to the half that actually carries the credential.
	s.refreshClient()
...
func (s *cgServer) handleCall(enc *json.Encoder, req rpcReq) {
	...
	case "setup_project":
		...
		return
	case "project_memory":
		// Before the thread guard: this is how an agent finds out the repo has no project at
		// all, so refusing it for lack of one would hide the answer behind the problem.
		s.handleProjectMemory(enc, req)
		return
	case "remember":
		// `remember` writes to PROJECT MEMORY and nothing else. It used to fall through to the
		// thread switch below as well, which wrote the fact twice and put two JSON-RPC results
		// on the wire under one request id.
		s.handleRemember(enc, req)
		return
	case "propose_fact":
		s.handleProposeFact(enc, req)
		return
	case "setup_project_memory":
		s.handleSetupProjectMemory(enc, req)
		return
	case "read_run":
	...
	case "planning_finalize":
		...
		s.toolResult(enc, req.ID, fmt.Sprintf(
			"Filed %d item(s). Root id %s.\n\nThe plan is readable at %s — the specification plus how it "+
				...
				"NOW TELL THE USER EXACTLY WHAT YOU DID, in the conversation:\n"+
				"  · every item you filed, BY TITLE — a count tells them nothing they can act on\n"+
				"  · where to read it: %s\n"+
				"  · that it is filed and NOT running, and that you can start it with promote_work_item\n"+
				"They have been waiting through an interview; this is the answer to it.%s",
			count, rootID, partyURL, partyURL, sourceNote+shapeNote+proofNote), false)
	...
	case "promote_work_item":
		...
		runID, perr := s.c.PromoteWorkItem(itemID, daemonID, label, policy, false)
		if perr != nil {
