...
var cgToolDefs = []map[string]any{
	...
	{
		"name":        "remember",
		"description": "Record a durable SEAM fact to the shared thread so teammates' sessions see it: a decision, a constraint you hit, an API contract, an open question, or a note. Use this only for things that cross the boundary between people or components — not your own local working notes.",
...
func formatContextBlocks(blocks []api.ContextBlock) string {
	...
	for _, blk := range blocks {
		// Agents only ever see CONFIRMED context: never 'proposed' (a scribe suggestion awaiting
		// human accept) and never 'superseded' (old value). This is the safety guarantee — an
		// un-reviewed distill can't reach an agent and steer it.
		if blk.Status == "superseded" || blk.Status == "proposed" {
			continue
		}
		if n == 0 {
