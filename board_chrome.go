...
func (m *boardModel) hintLine(w int) string {
	...
	if c, ok := m.focused(); ok && c.Foreign {
		if m.canDrill() {
			hints = append(hints, brand.Hint{Key: "⏎", Label: "open project"})
		} else {
			hints = append(hints, brand.Hint{Key: "⏎", Label: "read"})
		}
	...
	// A foreign board with NO focused card — empty, or failed to load — still needs its keys. The
	// branch above only ran when a card was focused, so a board that could not load advertised no
	// way to fix itself while the status line said "press p".
	if m.scoped() {
		if len(m.sources) > 1 {
			hints = append(hints, brand.Hint{Key: "S", Label: "source"})
		}
