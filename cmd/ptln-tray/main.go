...
//
// THE GUARDRAIL (Epic O, slice O.13): the tray shows STATE and offers CONTROL. It never
...
const (
	...
	// maxInvites bounds the invite rows. Small on purpose: this is a "someone is waiting for you"
	// list, not an inbox — past a handful the number in the badge is the useful signal, not the rows.
	maxInvites = 5
...
type daemon struct {
	...
	// Link is the REAL connection state — a heartbeat the instance accepted, recently. `Active` is
	// only `launchctl list` succeeding, so a daemon crash-looping or beating at an endpoint that
	// stopped listening is Active and not connected; the menu used to call that "connected" and
	// kept saying so through a fleet-wide outage. Absent against an older `ptln`, which degrades to
	// the previous Active-based line rather than claiming a state it cannot know.
	Link *link `json:"link"`
}

// link mirrors the CLI's linkHealth.
type link struct {
...
type machine struct {
	...
	// Invites: live sessions a teammate opened and asked you into. nil against a `ptln` that predates
	// them — same degrade-to-hidden contract as Peers.
	Invites []invite `json:"invites"`
	// Finished: runs that ENDED on this machine recently. nil against a `ptln` that predates it —
	// same degrade-to-hidden contract as Peers and Invites above.
	Finished []finishedRun `json:"finished"`
	// Projects: the canonical projects this machine's owner can see (names + flags, never paths).
	// nil against a `ptln` that predates them — same degrade-to-hidden contract as everything above.
	Projects []trayProject `json:"projects"`
}

// finishedRun mirrors the CLI's row. Only the fields a notification needs.
...
// invite mirrors the CLI's inviteRow. Only what the menu needs: who, and the link that admits you.
type invite struct {
...
// label is what the row and the banner say. A person's name is actionable; a bare join code is a
// lookup, so it's only the fallback (an older control plane sends no host).
...
// copyToClipboard hands the join link to pbcopy. Joining is a TERMINAL action (`ptln join <link>`)
// and the tray cannot put you in a terminal — so the honest help is to put the link where you can
// paste it, rather than opening a web page that only tells you to run the command.
...
func onReady() {
	...
	// Session invites. Above sessions for the same reason peers are: a teammate holding a line open
	// for you is a person waiting, and that outranks a machine working. Clicking COPIES the join
	// link rather than opening anything — joining is a terminal action (`ptln join <link>`) and the
	// tray cannot put you in a terminal, so the honest help is to hand you the thing you need to
	// paste. Pre-allocated like every other row: systray can't grow a menu after start.
	mInvHdr := systray.AddMenuItem("", "sessions you have been invited to")
	mInvHdr.Disable()
	invRows := make([]*systray.MenuItem, maxInvites)
	invLink := make([]atomic.Value, maxInvites)
	for i := range invRows {
		invRows[i] = systray.AddMenuItem("", "click to copy the join link")
		invRows[i].Hide()
		go func(i int) {
			for range invRows[i].ClickedCh {
				if v, _ := invLink[i].Load().(string); v != "" {
					copyToClipboard(v)
					notify("join link copied — run: ptln join <paste>")
				}
			}
		}(i)
	}
	...
	// Projects. Status rows, not buttons — same reasoning as sessions: the actions live in the
	// terminal and the web app, and the row's job is the at-a-glance fact (✓ = buildable HERE).
	// Pre-allocated like every other section: systray can't grow a menu after start.
	mProjHdr := systray.AddMenuItem("", "your team's projects (✓ = buildable on this machine)")
	mProjHdr.Disable()
	mProjHdr.Hide()
	projRows := make([]*systray.MenuItem, maxProjects)
	for i := range projRows {
		projRows[i] = systray.AddMenuItem("", "")
		projRows[i].Disable()
		projRows[i].Hide()
	}
	mProjMore := systray.AddMenuItem("", "")
	mProjMore.Disable()
	mProjMore.Hide()
	...
	lastInvites := -1 // same contract for invites
	tip := ""

	finishedRuns := newFinishedWatch()
	projectsWatch := newProjectWatch()
	refresh := func() {
		m, fail := readState()
		...
		// Invite rows + their banner. Rendered before the badge because an invite counts toward it:
		// someone is holding a session open waiting for you, which is exactly "blocked on you".
		inv := m.Invites
		if len(inv) > maxInvites {
			inv = inv[:maxInvites]
		}
		for i, r := range invRows {
			if i < len(inv) {
				invLink[i].Store(inv[i].Link)
				r.SetTitle("☎ " + inv[i].label() + " invited you")
				r.Show()
			} else {
				invLink[i].Store("")
				r.Hide()
		...
		if len(inv) > 0 {
			mInvHdr.SetTitle("INVITED")
			mInvHdr.Show()
		} else {
			mInvHdr.Hide()
		}
		// Same rise-edge rule as waiting sessions: announce only when the count GOES UP, and never on
		// the first pass — otherwise starting the tray re-announces every invite already open.
		if lastInvites >= 0 && len(inv) > lastInvites {
			if len(inv) == 1 {
				notify(inv[0].label() + " invited you to a session")
			} else {
				notify(fmt.Sprintf("%d session invites waiting", len(inv)))
			}
		}
		if fail == readOK {
			lastInvites = len(inv)
		...
		blocked := m.Waiting + m.Peers.Blocked() + len(inv)
		if blocked > 0 {
		...
		// Projects: the team's portfolio, ✓-marked where THIS machine can build. Hidden entirely
		// when the CLI predates the field or the team has none — same degrade as every section.
		if len(m.Projects) > 0 {
			mProjHdr.SetTitle("PROJECTS")
			mProjHdr.Show()
		} else {
			mProjHdr.Hide()
		}
		for i, r := range projRows {
			if i < len(m.Projects) {
				r.SetTitle(projectLine(m.Projects[i]))
				r.Show()
			} else {
				r.Hide()
			}
		}
		if extra := len(m.Projects) - maxProjects; extra > 0 {
			mProjMore.SetTitle(fmt.Sprintf("…and %d more", extra))
			mProjMore.Show()
		} else {
			mProjMore.Hide()
		}
		// A project newly visible to this account, said once — the "someone added a project"
		// banner. Edge-triggered inside the watch; the first poll seeds silently.
		for _, body := range projectsWatch.notices(fail == readOK, m.Projects) {
			notify(body)
		}
		...
		case m.Daemon.Active:
			// GREEN ONLY FOR A BEAT THE SERVER ACCEPTED. The dot is a coloured emoji rather than
			// "●": a menu item's text is drawn in the system label colour, so the old bullet was
			// always the same grey whatever it meant, and a status light that cannot change colour
			// is decoration. Emoji carry their own colour through NSMenu.
			mDaemon.SetTitle("Daemon: " + daemonLine(m.Daemon))
			mStart.Hide()
			mStop.Show()
		default:
			mDaemon.SetTitle("Daemon: " + dotStopped + " stopped")
			mStart.Show()
			mStop.Hide()
		}
		...
		// A run that ended on THIS machine, said once. Edge-triggered inside the watch: `ptln state`
		// republishes the same rows for up to two hours, so notifying on presence would re-fire
		// every poll — which is how a notification feature becomes the first thing someone disables.
		for _, body := range finishedRuns.notices(m.Finished) {
			notify(body)
		}
	}
	refresh()
