...
func daemonScanRoot(args []string) {
	...
	case "add":
		...
		if home, _ := localRepoHome(); home != "" && (abs == home || strings.HasPrefix(abs, home+string(filepath.Separator))) {
			fatal(fmt.Errorf("%s is already covered — your home directory is always scanned", abs))
		}
		for _, r := range reg.ScanRoots {
			if r == abs {
				fmt.Printf("already scanning %s\n", abs)
				return
			}
		}
		if len(reg.ScanRoots) >= localRepoScanRootsMax {
			fatal(fmt.Errorf("scan-root limit reached (%d) — remove one first", localRepoScanRootsMax))
		}
