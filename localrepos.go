...
// repoScan is ONE walk's worth of knowledge, and the reason it exists: the heartbeat needs two
// views of the same filesystem — the repos this machine has, and the directories it could put one
// in — and deriving them from separate scans walked the home directory twice per beat.
type repoScan struct {
	roots  []string          // the walked roots; roots[0] is $HOME when there is one
	home   string            // "" on a machine with no resolvable home
	paths  []string          // sorted absolute repo paths
	byRoot map[string]string // repo path → the root it was found under (display only)
}

// scanRepoTree performs the walk. Every FRESH derivation (a handle resolution, `ptln daemon repos`)
// goes through here; only the heartbeat reads the cache below.
func scanRepoTree() repoScan {
	roots, managed := localRepoRoots()
	home := ""
	if len(roots) > 0 {
		home = roots[0]
	}
	byRoot := scanAllRepoPaths(roots, managed)
	paths := make([]string, 0, len(byRoot))
	for p := range byRoot {
		paths = append(paths, p)
	}
	sort.Strings(paths)
	return repoScan{roots: roots, home: home, paths: paths, byRoot: byRoot}
}
...
// ONE cache, holding the WALK rather than either rendered list. Caching the two lists separately is
// what let a single heartbeat walk the disk twice; deriving both from this costs only string work.
var repoScanCache struct {
	mu   sync.Mutex
	at   time.Time
	scan repoScan
}

// cachedRepoScan is the heartbeat's view of the filesystem — one walk behind a TTL, shared by
// cachedLocalRepos and cachedDestinations.
func cachedRepoScan() repoScan {
	repoScanCache.mu.Lock()
	defer repoScanCache.mu.Unlock()
	if !repoScanCache.at.IsZero() && time.Since(repoScanCache.at) < localRepoTTL {
		return repoScanCache.scan
	}
	repoScanCache.scan = scanRepoTree()
	repoScanCache.at = time.Now()
	return repoScanCache.scan
}

// cachedLocalRepos is the heartbeat's repo list.
func cachedLocalRepos() []api.LocalRepo { return localReposFrom(cachedRepoScan()) }

// invalidateLocalRepoCache forces the next heartbeat to re-scan — called after a bind, so a
// just-registered repo (and the destination list derived from the same walk) is reflected without
// waiting out the TTL.
func invalidateLocalRepoCache() {
	repoScanCache.mu.Lock()
	repoScanCache.at = time.Time{}
	repoScanCache.mu.Unlock()
}

// scanLocalRepos is a FRESH repo list — what `ptln daemon repos` and `ptln settings` show.
func scanLocalRepos() []api.LocalRepo { return localReposFrom(scanRepoTree()) }

// localReposFrom renders a scan as what the heartbeat advertises: handle + display metadata,
// never a path.
func localReposFrom(s repoScan) []api.LocalRepo {
	if len(s.roots) == 0 {
		return nil
	}
	out := make([]api.LocalRepo, 0, len(s.paths))
	for _, p := range s.paths {
		parent := displayParent(p, s.home)
		if root := s.byRoot[p]; root != s.home {
...
func bindLocalRepo(ev api.BindRepoEvent) error {
	...
	return registerLocalProject(label, abs, ev.Preset, ev.Engine)
}

// registerLocalProject writes one label→path binding into THIS machine's registry and re-advertises it.
// The single registry-write chokepoint for every web-driven path (bind a local repo, land a cloned
// assignment), so both end in the exact same state as `ptln daemon add-project` typed here — one
// entry, mirrored back on the next sync. `abs` is always derived locally; nothing server-supplied
// reaches it as a path.
func registerLocalProject(label, abs, preset, engine string) error {
	return registerLocalProjectPolicy(label, abs, preset, engine, "")
}

// registerLocalProjectPolicy is registerLocalProject plus the project's RUN MODE.
//
// policy "" means LEAVE IT ALONE, and that default is load-bearing: policy decides whether this
// machine runs dispatched work unattended, so a call that only meant to re-point a directory must
// never quietly change who may execute code here. Only an explicit "auto"/"ask" moves it.
func registerLocalProjectPolicy(label, abs, preset, engine, policy string) error {
	...
	if !found {
		np := daemonProject{Label: label, Path: abs, Preset: preset, Engine: engine}
		if policy == "auto" || policy == "ask" {
			np.Policy = policy
		}
		reg.Projects = append(reg.Projects, np)
	}
	if err := saveDaemonRegistry(reg); err != nil {
		return err
	}
