...
func tmuxFeedToggle() {
	...
	// The feed carries the project of the session it sits beside, resolved HERE rather than
	// inherited as a working directory: a pane's cwd follows the agent around, and the feed's
	// subject should not.
	args := []string{"split-window", "-h", "-d", "-l", feedPaneWidth, "-t", window,
		"-c", paneDirOf(window), "-P", "-F", "#{pane_id}", selfExe(), "feed"}
	if label := projectLabelOf(paneDirOf(window)); label != "" {
		args = append(args, "--project", label)
	}
...
// paneDirOf is the active pane's working directory in a window.
func paneDirOf(window string) string {
	out, err := tmuxCmd("display-message", "-p", "-t", window, "#{pane_current_path}").Output()
	if err != nil {
		return ""
	}
...
// projectLabelOf resolves a directory to its project label, or "".
func projectLabelOf(dir string) string {
	if dir == "" {
		return ""
	}
	if ws, ok := projectForDir(dir); ok {
		return ws.Label
	}
	return ""
}
