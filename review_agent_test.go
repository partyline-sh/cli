...
func TestReviewGradePromptIntegrityLayers(t *testing.T) {
	...
	// No criteria / no tree note → those sections are absent, honesty rule stays.
	p = reviewGradePrompt("d", nil, "")
	if strings.Contains(p, "ACCEPTANCE CRITERIA") || !strings.Contains(p, "VERIFICATION HONESTY") {
