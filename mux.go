...
// selfExe is the path to this running binary — used to re-invoke partyline (e.g. to host a
// shared session via `partyline start`).
func selfExe() string {
...
// same launcher (no Key → never dedupes; each is its own window).
func shellSpec() *ptymux.Spec {
	...
	label := "terminal"
	if cwd != "" {
		label = "terminal · " + filepath.Base(cwd)
	}
	return &ptymux.Spec{Label: label, Argv: []string{sh}, Dir: cwd}
}
