...
func runRender(wtPath, script, shotsDir string, timeout time.Duration) error {
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	c := exec.CommandContext(ctx, "sh", "-c", script)
	c.Dir = wtPath
	c.Env = append(os.Environ(), "PARTYLINE_SHOTS_DIR="+shotsDir)
...
// runVisualReview is the T2d gate. ran=false when it's off (no `.partyline/visual`) OR when the diff
// touched no UI files (honest skip — nothing visual to look at). Otherwise it renders, collects
// screenshots, and asks a vision-capable independent reviewer for a verdict — FAIL-CLOSED on a
// render failure, no screenshots, engine error, timeout, or an unparseable reply.
func runVisualReview(baseRepo, wtPath, task string, timeout time.Duration) verifyResult {
	enabled, script := readVisual(baseRepo)
	if !enabled {
	...
	if err := runRender(wtPath, script, shotsDir, timeout); err != nil {
