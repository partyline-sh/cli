...
//   - the question is bounded (MAX_QUESTION = 32000) and the answer is bounded (maxConsultAnswer)
...
func consultGlobalPolicyPath() string {
	return filepath.Join(daemonDir(), "consults-global.mode")
}
...
// consultAsker renders who is asking, for the console prompt and the approval queue.
//
// The handle is resolved SERVER-SIDE from profiles and arrives on the event; it is never taken from
// anything the asker controls, because choosing your own displayed identity is exactly the abuse the
// budget ledger's design notes warn about. An older control plane sends nothing, and a machine that
// cannot name the asker says so plainly rather than printing a blank or guessing — "someone on your
// team" is true, and it keeps the prompt readable while the fleet is mid-upgrade.
func consultAsker(ev api.ConsultEvent) string {
	if h := strings.TrimSpace(ev.FromHandle); h != "" {
		return "@" + h
	}
