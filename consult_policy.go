...
// decideConsult is the whole inbound policy decision, in one place, taking ONE argument: the advertised
// project label. That signature is the security property made structural — there is no parameter
// through which the asker's request could influence the outcome, so the answer to "can a requester get
// their own request auto-approved?" is no by construction rather than by careful reading. The inputs
// are this machine's registry and this machine's daily ledger, both local files it owns.
//
...
//
// ORDER MATTERS: policy first, budget second. The env caps (PARTYLINE_CONSULT_AUTO_DAILY et al) are a
// SPEND bound, reached only once policy has already said yes, so no environment value — however large —
// can override a persisted OFF. Setting the cap to 0 remains a second way to stop auto-answer wherever
// the daemon can actually see it; the persisted switch is the one that works regardless.
func decideConsult(label string) (consultDisposition, string) {
	...
	if ok, why := claimConsultAutoAnswer(label); !ok {
