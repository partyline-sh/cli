...
func recoverModal(s aiSession, capturedRepo string) func() {
	return func() {
		...
		lines = append(lines,
			...
			"",
			"  "+brand.HintBar("RECOVER", []brand.Hint{
				{Key: "q · esc · ⏎", Label: "back to the switchboard"}}, 0))
		...
		case 'd':
			...
			nd := relocateSession(s)
			if nd == "" {
				return // empty / not a dir / unmigratable tool (already explained + paused) → cancel
			}
			dir, relocated = nd, true
		case 'r':
			u, uok := cgAsk("Recover session",
				[]string{dim("paste a git URL — it's cloned into " + dir)}, "git remote URL", "")
			if repo = strings.TrimSpace(u); !uok || repo == "" {
				return
			}
		case 'e':
			repo = "" // empty dir
...
func relocateSession(s aiSession) string {
	line, lok := cgAsk("Recover session",
		[]string{dim("where did you move it to? the conversation and the code are both intact")},
		"new directory path", "")
	...
	if err != nil || !fi.IsDir() {
		cgNote("Recover session", []string{fmt.Sprintf("  %s✗ not a directory: %s%s", cgBad, p, cgOff)})
		return ""
	}
	...
	if err := migrateSessionStore(s, p); err != nil {
		cgNote("Recover session", []string{fmt.Sprintf("  %s✗ %v%s", cgBad, err, cgOff)})
		return ""
	}
	...
	meta[s.ID] = mt
	saveLLMMeta(meta)
	cgNote("Recover session", []string{
		fmt.Sprintf("  %s✓%s relocated → %s", cgOK, cgOff, p),
		"  " + dim("(store migrated; remembered for next time)")})
	return p
}
