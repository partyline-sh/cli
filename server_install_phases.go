	...
	return []installStep{
		...
		},
		{
			...
			do: func(c installConfig, o installOps) error {
				...
				// env-bootstrap prints the FIRST SIGN-IN credentials exactly once — a fresh
				// realm's username and its one-time temporary password. Swallowing that
				// output (this step used to) left a working install nobody could sign in
				// to: the operator opened the login page and concluded there were no
				// accounts. Reprint the identity block verbatim; it is the only copy the
				// operator will ever be shown, and it is theirs.
				if i := strings.Index(out, "IDENTITY"); i >= 0 {
