...
func installPreflight(cfg installConfig, ops installOps) []string {
	...
	if st, err := ops.stat(parent); err != nil {
		...
		problems = append(problems, fmt.Sprintf("%s is not writable by this user — re-run with sudo, or pick a --dir you own", parent))
	}
	...
	for _, p := range installWantedPorts(cfg) {
		...
		if ops.portBusy(cfg.bind, p.port) && !installAlreadyOurs(cfg, ops) {
			problems = append(problems, fmt.Sprintf("%s port %d is already in use on %s — pass --%s-port to move it, or stop what is there",
				p.label, p.port, cfg.bind, p.flag))
		}
	}
	return problems
}
