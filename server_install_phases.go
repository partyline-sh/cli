...
func runInstall(cfg installConfig, ops installOps) bool {
	...
	// ── phase 1: preflight ──────────────────────────────────────────────────────────────────
	// Reads the machine. Writes nothing, including on the happy path.
	...
	// Port conflicts are a QUESTION, not a failure. This runs before the preflight report so a
	// busy port never reaches it — on a box that already runs something, which is most boxes,
	// refusing here used to mean the first install could not succeed without two round trips.
	if resolved, ok := resolvePortConflicts(cfg, ops); ok {
		cfg = resolved
	}

	// Interactive gaps come after preflight, so a box that cannot install never asks questions.
	var err error
	...
	return []installStep{
		...
		},
		{
			...
			do: func(c installConfig, o installOps) error {
				...
				// Never clobber an edited edge config. An operator who has tuned their
				// Caddyfile must not lose it to a re-install.
				if _, err := o.stat(path); err == nil {
					return nil
				}
				return o.writeFile(path, []byte(caddyfileFor(c)), 0o644)
			},
			check: func(c installConfig, o installOps) error {
		...
		},
		{
			...
			do: func(c installConfig, o installOps) error {
				out, err := o.run(c.dir, "docker", "compose", "up", "-d")
				if err != nil {
