...
func discoverBoardProviders() []boardSource {
	...
	for name, srv := range loadMCPCatalog() {
		...
		case strings.TrimSpace(srv.URL) != "":
			p = providerSource{name: name, url: strings.TrimSpace(srv.URL), headers: srv.Headers}
		case strings.TrimSpace(srv.Command) != "":
			p = providerSource{name: name, command: srv.Command, args: srv.Args, env: srv.Env}
		default:
			continue
		}
		// kind chooses WHO knows how to build the board: the server, or us. Unset keeps the
		// original meaning of "board": true — the server serves partyline://board itself.
		switch srv.Board.Kind {
		case "":
			out = append(out, p)
		case "odoo":
			out = append(out, odooSource{p: p, cfg: srv.Board,
				poll: time.Duration(srv.Board.Poll) * time.Minute})
		default:
			// A kind this binary does not have is a config error the operator can fix, and saying
			// nothing would look like the server being unreachable.
			out = append(out, brokenSource{name: name, err: fmt.Errorf(
				"unknown board kind %q — this ptln knows: odoo", srv.Board.Kind)})
		}
	}
	return out
}

// brokenSource stands in for a board that cannot be built at all, so a misconfiguration appears in
// the picker with its reason rather than vanishing from it.
type brokenSource struct {
	name string
	err  error
}

func (b brokenSource) Name() string                    { return b.name }
func (b brokenSource) Scopes() ([]boardScope, error)   { return nil, b.err }
func (b brokenSource) Load(string) (*boardData, error) { return nil, b.err }
