...
func (m *boardModel) rows(col api.BoardColumn) []boardRow {
	...
	return columnRows(col, m.board.Column(col), m.collapsed)
}

// focusedColumn is the column key the cursor is in.
func (m *boardModel) focusedColumn() api.BoardColumn {
	if m.col < 0 || m.col >= len(api.BoardColumns) {
		return api.BoardColumns[0]
	}
	return api.BoardColumns[m.col]
}
...
func (m *boardModel) restoreFocus() {
	if m.board == nil || (m.focusID == "" && m.focusChainID == "") {
		m.clamp()
		return
	}
	for ci, col := range api.BoardColumns {
...
func (m *boardModel) moveColumn(d int) {
	n := len(api.BoardColumns)
	for step := 1; step <= n; step++ {
		i := ((m.col+d*step)%n + n) % n
		if len(m.rows(api.BoardColumns[i])) > 0 {
			m.col = i
			m.rememberFocus()
			return
		}
...
func countUrgent(b *api.Board) int {
	if b == nil {
		return 0
	}
	n := 0
