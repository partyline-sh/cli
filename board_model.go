...
type boardModel struct {
	...
	// Why each source last failed, if it did. The board picker shows this in place of the scope, so
	// an unreachable server or a bad token is visible in the list rather than being something you
	// find out by selecting it.
	srcErr map[string]string
	// The API client, kept so the source list can be rebuilt when a board is added mid-session.
	client *api.Client
	// When the ptln binary on disk was last written, sampled at startup. See binaryReplaced.
	selfStamp time.Time

	// A read in flight: what it is, and since when. Drives the loading indicator — see beginBusy.
	busy      string
	busySince time.Time
	...
	// A card in hand (the `m` grab-and-carry). While set, the cursor is the drop target and
	// board_carry.go owns the keys. Never survives a refresh commit — set and cleared in one
	// key-loop iteration or an explicit cancel.
	carry *carryState
	// rankWrite is the Backlog reorder writer — c.SetRunRank in production, injected in tests
	// so a commit can be asserted without a network.
	rankWrite   func(id string, rank float64) error
	carryReveal int       // >0: the carried tile draws only its first N lines (the hop animation)
	carryLast   time.Time // last carry movement — key-repeat suppresses per-hop frames
...
func columnRows(col api.BoardColumn, cards []api.BoardCard, collapsed map[string]bool) []boardRow {
	return columnRowsOrdered(col, cards, collapsed, false)
}

// columnRowsOrdered is columnRows with the source's own order preserved when keep is set —
// an Odoo column arrives in kanban sequence order, and re-sorting it by recency (the old
// behavior) showed rows in an order Odoo itself never displays.
func columnRowsOrdered(col api.BoardColumn, cards []api.BoardCard, collapsed map[string]bool, keep bool) []boardRow {
	sorted := cards
	if !keep {
...
func (m *boardModel) rows(col api.BoardColumn) []boardRow {
	...
	base := columnRowsOrdered(col, m.data.Column(col), m.collapsed, m.data.KeepOrder)
	if m.carry == nil {
		return base
	}
	// While a card is carried it renders AT THE CURSOR, not where it really lives: the same
	// arrows that move the cursor visibly move the card, neighbours shift around it, and the
	// screen always shows exactly what ⏎ would commit. (The v0.105.5 carry moved only the
	// cursor — the operator pressed m, arrowed, and watched nothing happen.)
	stripped := make([]boardRow, 0, len(base))
	for _, r := range base {
		if r.Card != nil && r.Card.ID == m.carry.card.ID {
			continue
		}
		stripped = append(stripped, r)
	}
	if col != m.focusedColumn() {
		return stripped
	}
	i := m.cursor[col]
	if i < 0 {
	...
	out := make([]boardRow, 0, len(stripped)+1)
	out = append(out, stripped[:i]...)
	out = append(out, boardRow{Card: &m.carry.card})
	out = append(out, stripped[i:]...)
	return out
}
