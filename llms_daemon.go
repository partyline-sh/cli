...
func (p *daemonPresence) run(ctx context.Context) {
	...
	for {
		...
		err := api.DaemonStream(ctx, d.Base, d.Token,
			...
			func(ev api.RunEvent) { // O.2 run-profile — the TUI manager only auto-runs EXPLICITLY
				// Auto projects; everything else waits for Start on the board. (autoRunProject, not
				// isAutoProject: sitting in the backlog must not mean "begin building".)
