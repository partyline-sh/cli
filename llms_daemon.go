	...
	for {
		...
		err := api.DaemonStream(ctx, d.Base, d.Token,
			...
			func(ev api.LaunchEvent) { // pending — auto-accept an Auto project, else queue a banner
				...
				if insidePtlnTmux() {
					// the launcher fixture hosts this stream, but the human is usually in a
					// session window — the approval must reach them where they're looking
					tmuxTargets{}.SetBanner("⚑ launch approval waiting: " + ev.ProjectLabel + " — ctrl-\\ then o to review")
				}
			},
			...
			func(ev api.RunEvent) { // O.2 run-profile — dispatch ONLY what a human Started.
				// `go` means the owner pressed Start on the board. Nothing else runs: sitting in the
				// backlog must not mean "begin building", and the machine executing the work is the
				// wrong place to decide which work executes.
				if ev.Go {
			...
			nil,             // web bind_repo: the always-on `daemon run` service owns the local registry
