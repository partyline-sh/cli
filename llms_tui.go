...
func (m *aiMenu) applyFilter() {
	...
	for _, s := range m.all {
		...
		if q != "" {
			hay := strings.ToLower(s.Tool + " " + filepath.Base(s.Cwd) + " " + projLabel(s.projKey()) + " " + s.WtName + " " + s.Title + " " + m.meta[s.ID].Name + " " + s.ID)
...
func (m *aiMenu) sessionRow(s *aiSession, sel bool) string {
	...
	return marker + fg(dotClr) + glyph + " " + fg(242) + fmt.Sprintf("%4s ", humanAge(s.LastActive)) +
		fg(toolColor(s.Tool)) + fmt.Sprintf("%-6s ", toolLabel(s.Tool)) + wt + titleClr + title
}
