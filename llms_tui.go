...
const (
	sortRecent  sortMode = iota // newest first (default)
	sortOldest                  // oldest first
	sortProject                 // cluster by project, newest within each
)

func (s sortMode) label() string {
	return [...]string{"recent", "oldest", "project"}[s]
}
...
func (m *aiMenu) applyFilter() {
	...
	// Pinned always float to the top; within that, the chosen sort order.
...
func (m *aiMenu) listLines(w, bh int) []string {
	...
	for i := 0; i < bh; i++ {
		...
		row := marker + fg(dotClr) + "● " + fg(242) + fmt.Sprintf("%4s ", humanAge(s.LastActive)) +
...
func (m *aiMenu) footer() string {
	...
	return " " + key("↑↓", "move") + " " + key("⏎", "open") + " " + key("→", "detail") + " " +
		key("r", "rename") + " " + key("d", "diff") + " " + key("/", "search") + " " +
		key("s", "sort") + " " + key("R", "refresh") + " " + key("a", allTag) + " " + key("?", "help") + " " + key("q", "quit") +
...
func (m *aiMenu) renderHelp() {
	...
	rows := [][2]string{
		...
		{"", "  once live: ctrl-o launcher · ctrl-\\ n/p switch · x close · q quit"},
		...
		{"/", "search (esc clears)"},
		{"s", "cycle sort: recent · oldest · project"},
