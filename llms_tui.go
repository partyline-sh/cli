...
func (m *aiMenu) footer() string {
	...
	return " " + key("↑↓", "move") + " " + key("⏎", "open") + " " + key("w", "switch") + " " + key("S", "share") + " " + key("n", "term") + " " +
		key("space", "select") + " " + key("r", "rename") + " " + key("d", "diff") + " " + key("/", "search") + " " +
...
func (m *aiMenu) renderHelp() {
	...
	rows := [][2]string{
		...
		{"w", "picker: jump to one of your open (▸live) sessions"},
