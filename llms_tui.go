...
// recoverClosure recovers a session whose working directory was removed. Runs as a mux SUSPEND fn
// (full cooked terminal, like the diff pager), so it can show clone progress and land the user in
// the resumed conversation. capturedRepo is the origin we snapshotted while the dir was alive; if
// empty (legacy sessions from before capture), it prompts for a remote on stdin — so this works for
// both. Steps: recreate the dir (clone the remote, or empty if skipped) → resume the original
// conversation there (the transcript survived; only the dir was lost). Best-effort; on any failure
// it explains and returns to the launcher.
func recoverClosure(s aiSession, capturedRepo string) func() {
	return func() {
		in := bufio.NewReader(os.Stdin)
		dir := s.Cwd
		fmt.Printf("\r\n↻ Recover session — its directory %q had been removed.\r\n", dir)
		repo := capturedRepo
		if repo != "" {
			fmt.Printf("Recorded remote: %s\r\n", repo)
		} else {
			fmt.Printf("No remote was recorded for this session.\r\nPaste a git remote to clone, or press enter to recreate an empty directory: ")
			line, _ := in.ReadString('\n')
			repo = strings.TrimSpace(line)
		}
		if repo != "" {
			_ = os.MkdirAll(filepath.Dir(dir), 0o755) // git clone needs the parent to exist (the leaf must not)
			fmt.Printf("\r\nCloning %s → %s …\r\n", repo, dir)
			...
			if err := os.MkdirAll(dir, 0o755); err != nil {
				fmt.Printf("\r\n✗ couldn't recreate %s: %v\r\nPress enter to return.\r\n", dir, err)
				_, _ = in.ReadString('\n')
				return
			}
			fmt.Printf("Recreated %s (empty — clone your code in when ready).\r\n", dir)
		}
		...
		fmt.Printf("\r\n✓ Restored. Resuming the conversation in %s…\r\n", dir)
...
// sessRepo reads a session's git origin URL from its (still-live) cwd — best-effort, bounded, and
// NETWORK-FREE (reads .git/config). "" if not a git repo / no origin / unreadable. We capture this
// WHILE the directory is alive so a session whose dir later vanishes can be recovered (recreate + clone).
...
// stripURLCreds removes embedded credentials from an https remote (https://user:token@host/… →
// https://host/…) so a token in the origin URL never lands in the persisted sidecar. scp-style
// ssh remotes (git@host:path) carry no secret and pass through untouched.
func stripURLCreds(u string) string {
	i := strings.Index(u, "://")
	if i < 0 {
		return u // ssh scp-style or bare — no scheme authority to strip
	}
	scheme, rest := u[:i+3], u[i+3:]
	slash := strings.IndexByte(rest, '/')
	at := strings.IndexByte(rest, '@')
	if at >= 0 && (slash < 0 || at < slash) { // '@' is in the authority
		rest = rest[at+1:]
	}
	return scheme + rest
}

// captureSessionRepos backfills sessMeta.Repo for every session whose cwd STILL EXISTS but has no
// origin captured yet — the info can't be read once the dir is gone, so we grab it on the launcher
// scan ("store it when we find a session"). The first run after upgrade retroactively captures
// existing installs' live sessions; subsequent runs only touch newly-seen ones. Returns whether
// anything changed so the caller persists once.
func captureSessionRepos(all []aiSession, meta map[string]sessMeta) bool {
	changed := false
	for _, s := range all {
		mt := meta[s.ID]
		if s.Cwd == "" || mt.Repo != "" || mt.RepoChecked {
			continue // no dir, already captured, or already looked at — never re-fork git
		}
		if _, err := os.Stat(s.Cwd); err != nil {
			continue // gone/unreadable — nothing to capture (cheap stat, no git fork); may retry when it returns
		}
		mt.Repo = sessRepo(s.Cwd) // "" if the cwd isn't a git repo
		mt.RepoChecked = true     // looked once (repo or not) → this session never git-forks again
		meta[s.ID] = mt
		changed = true
	}
	return changed
...
func saveLLMMeta(m map[string]sessMeta) {
	...
	for k, v := range m {
		if v.Pinned || v.Archived || v.Name != "" || !v.LastUsed.IsZero() || v.Repo != "" || v.RepoChecked {
			out[k] = v
		}
	}
...
func (m *aiMenu) handleKey(b []byte) (bool, *aiSession) {
	...
	case '\r', '\n':
		...
		if s := m.selected(); s != nil {
			...
			// Directory removed → RECOVER (recreate/clone + resume) instead of a doomed spawn into a
			// missing cwd. Runs as a suspend (visible clone + the resumed session in a full terminal).
			if s.Cwd != "" {
				if _, err := os.Stat(s.Cwd); err != nil {
					m.suspendFn = recoverClosure(*s, m.meta[s.ID].Repo)
					return false, nil
				}
			}
			modes := permissionModes(s.Tool)
