...
func captureSessionRepos(all []aiSession, meta map[string]sessMeta) bool {
	changed := false
	for _, s := range all {
		if s.Cwd == "" || meta[s.ID].Repo != "" {
			continue
		}
		if _, err := os.Stat(s.Cwd); err != nil {
			continue // gone/unreadable — nothing to capture (too late once the dir vanishes)
		}
		if url := sessRepo(s.Cwd); url != "" {
			mt := meta[s.ID]
			mt.Repo = url
			meta[s.ID] = mt
			changed = true
...
func saveLLMMeta(m map[string]sessMeta) {
	...
	for k, v := range m {
		if v.Pinned || v.Archived || v.Name != "" || !v.LastUsed.IsZero() || v.Repo != "" {
			out[k] = v
		}
	}
