...
func telemetryDisabled() bool {
	...
	// No control plane, nothing to report to. partyline.sh is documentation now and serves no
	// API, so an unconfigured CLI was announcing an anonymous daily ping and then POSTing it at
	// a documentation site. Saying we collect something we cannot collect is worse than either
	// collecting it or staying quiet — observed on a first `sudo ptln server install`, where
	// root has no config and therefore no instance.
	if api.Unconfigured() {
		return true
	}
