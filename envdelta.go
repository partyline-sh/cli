...
// fetchForDelta updates this clone's remote-tracking refs so the measurement reflects the remote
// rather than the last time someone happened to pull.
//
// Best-effort and deliberately silent: a machine that is offline, or a repo with no remote at all,
// still measures with what it has. That degrades to STALE numbers, not wrong ones — every gap is
// stamped with reported_at and the UI says how old it is, which is exactly the case that timestamp
// exists for. Failing the whole cycle instead would trade a slightly-old answer for no answer.
//
// No --prune: pruning is a write to refs another crank process may be mid-operation against, and
// nothing here needs it. A deleted remote branch simply keeps a lingering ref that reads as merged.
func fetchForDelta(dir string) {
	_, _ = gitDelta(dir, "fetch", "--quiet", "origin")
}
...
func reportEnvDeltas(d daemonDevice) {
	...
	for _, q := range qs {
		...
		// Refresh remote-tracking refs BEFORE measuring. Without this the delta is computed against
		// whatever this clone last happened to see, which is a silent wrong answer rather than a
		// missing one: a merge that landed on the remote ten minutes ago reads as "0 commits
		// waiting", i.e. "everything is live" — the most misleading output the widget can produce.
		//
		// Nothing else in the daemon guarantees a fetch on any schedule. crank fetches during a
		// build, so a busy machine drifts less, but an idle one can be hours stale and still report
