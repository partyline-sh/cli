...
func runOneShot(ctx context.Context, dir string, argv []string, stdinPrompt string, extraEnv ...string) ([]byte, error) {
	...
	cmd.Dir = dir
	// #814 — every engine spawned through here gets its own process group and group teardown. This
	// is the shared one-shot path (review, verify, describe, evidence, non-claude workers), so it is
	// the single highest-leverage place to apply it: without it, any engine that spawns MCP children
	// can outlive its own timeout by holding the output pipe open. See proctree.go.
	groupSpawn(cmd)
