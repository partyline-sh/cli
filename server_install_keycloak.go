...
/opt/keycloak/bin/kcadm.sh update clients/$CID -r partyline -s 'redirectUris=["` + site + `/api/auth/callback"]' -s 'webOrigins=["` + site + `"]' -s 'attributes."post.logout.redirect.uris"=` + site + `/*'
/opt/keycloak/bin/kcadm.sh update realms/partyline -s 'displayNameHtml=<div style="font-family:ui-monospace,monospace;letter-spacing:.35em;font-weight:700;color:#ff9838">&#9742;&nbsp;PARTYLINE</div>' -s displayName=partyline -s loginTheme=partyline
