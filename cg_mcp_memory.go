...
func (s *cgServer) handleProjectMemory(enc *json.Encoder, req rpcReq) {
	...
	s.toolResult(enc, req.ID, stale+renderBrief(ws.Label, ws.repoNameFor(cwd), facts), false)
}
...
func (s *cgServer) handleProposeFact(enc *json.Encoder, req rpcReq) {
	...
	f := fact{Kind: a.Kind, Repo: repo, By: proposedBy, At: time.Now(), Sources: sources, Body: a.Body}
	if _, err := writeFact(ws.Dir, f); err != nil {
		s.toolResult(enc, req.ID, fmt.Sprintf("could not record it: %v", err), true)
		return
	}
	s.toolResult(enc, req.ID, fmt.Sprintf("Proposed %s, citing %s. It is marked as inferred and briefed below facts a person recorded. It is NOT shared until someone reviews it and runs `ptln memory sync`.", f.ID, strings.Join(sources, ", ")), false)
}
