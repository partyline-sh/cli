...
func cgSessionMenu(mx *ptymux.Mux) {
	...
	attach := func(id string) bool {
		...
		if ok && len(argv) > 0 && isWireableEngine(argv[0]) {
			wiredArgv, eng := wireThreadArgv(argv[0], argv, id)
			...
			fmt.Println("  \x1b[36m/mcp__partyline-context-threads__seed_from_history\x1b[0m  \x1b[38;5;245m(type / and pick partyline-context-threads)\x1b[0m")
			mx.SetPendingReattach(ptymux.Spec{Label: label, Key: key + "·cg", Argv: wiredArgv, Dir: dir, Thread: id, ThreadLabel: tl, Engine: eng})
			pause(in)
			return true
		}
	...
	for {
		...
		if target != "" {
			...
			fmt.Println("    d  disconnect this session from the thread")
		} else {
		...
		case "d":
			...
			argv, dir, label, key, ok := mx.ActiveLaunch()
			if ok && len(argv) > 0 && isWireableEngine(argv[0]) {
				unwired := unwireThreadArgv(argv)
				fmt.Printf("\n  \x1b[1mRestarting your session\x1b[0m to disconnect it from \x1b[1m%s\x1b[0m…\n", titleOf(target))
				fmt.Println("  \x1b[38;5;245myour conversation carries over; the agent loses recall/remember for this thread.\x1b[0m")
				mx.SetPendingReattach(ptymux.Spec{Label: label, Key: key + "·x", Argv: unwired, Dir: dir}) // no Thread → unwired
				pause(in)
				return
			}
