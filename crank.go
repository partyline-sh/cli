...
type crankResult struct {
	...
	branchState      *api.BranchState // #602: the branch's probed state at completion; nil = not probed
}
...
func taskBranchName(runID string, i int, task string) string {
	slug := gitwt.FlatSlug(taskSlugWords(originalTask(task), slugWords))
	if runID == "" {
...
func realTaskExec(repo string, o crankOpts, logger *runLogger) taskExec {
	return func(i int, task, prompt string) crankResult {
		...
		// worktree.created — the customer's first chance to act, and the one where a repo that needs
		// bootstrapping (install deps, seed a .env, warm a cache) can do it before the agent starts.
		if n := runHook(repo, HookWorktreeCreated, map[string]string{
			"EVENT": string(HookWorktreeCreated), "BRANCH": branch, "WORKTREE": wtPath, "BASE": o.base, "TASK": originalTask(task),
		}).note(); n != "" {
			logger.note(i, "step", n)
		}
		...
		if werr != nil {
			...
			if strings.HasPrefix(r.note, "committed") {
				...
				// pre.verify / post.verify — a team whose real gate lives outside .partyline/verify (a
				// bespoke harness, a licence check, a house linter) hangs it here without partyline
				// needing to know what it is.
				if n := runHook(repo, HookPreVerify, map[string]string{
					"EVENT": string(HookPreVerify), "BRANCH": branch, "WORKTREE": wtPath, "TASK": originalTask(task),
				}).note(); n != "" {
					logger.note(i, "step", n)
				}
				...
				if n := runHook(repo, HookPostVerify, map[string]string{
					"EVENT": string(HookPostVerify), "BRANCH": branch, "WORKTREE": wtPath,
					"TASK": originalTask(task), "VERIFY_OK": fmt.Sprintf("%t", r.verify.ok),
				}).note(); n != "" {
					logger.note(i, "step", n)
				}
				...
				if r.verify.ran && !r.verify.ok {
					...
					// pre.merge — the last point before the branch leaves this machine. A hook here can
					// push to a mirror, stamp a changelog, or refuse to be surprised later. It cannot
					// stop the merge: the run already did what partyline promised, and a hook's job is
					// to act on that, not to adjudicate it.
					if n := runHook(repo, HookPreMerge, map[string]string{
						"EVENT": string(HookPreMerge), "BRANCH": branch, "BASE": o.base, "TASK": originalTask(task),
					}).note(); n != "" {
						logger.note(i, "step", n)
					}
					...
					// post.merge — what actually happened, including the PR url when one was opened.
					if n := runHook(repo, HookPostMerge, map[string]string{
						"EVENT": string(HookPostMerge), "BRANCH": branch, "BASE": o.base,
						"TASK": originalTask(task), "PR_URL": prURL, "MERGE_POLICY": o.mergePolicy,
					}).note(); n != "" {
						logger.note(i, "step", n)
					}
					if prURL != "" {
		...
		// #602: last thing before the report — what did this task actually leave behind? Deliberately
		// AFTER teardown, so "the worktree still exists" is the state a reader would find on disk
		// rather than the state before we tidied. Report only: nothing here deletes or reclaims
		// anything, and nothing about the task's outcome depends on it.
		r.branchState = probeBranchState(repo, branch, o.base)
		return r
	}
}

// probeBranchState asks LOCAL GIT what a finished task's branch is: commits ahead of the base, its
// diffstat, whether its worktree survived, and whether the base already contains it. No host API and
// no credentials — it behaves the same against GitHub, a self-hosted Gitea or a bare remote.
//
// The tri-state matters more than the numbers: a probe that could not run reports "unknown", never
// "unmerged", because a later janitor reading a wrong "unmerged" deletes work that had actually
// landed — and one reading a wrong "merged" deletes work that had not.
func probeBranchState(repo, branch, base string) *api.BranchState {
	if repo == "" || branch == "" {
		return nil
	}
	st := gitwt.ProbeBranchState(repo, branch, base)
	return &api.BranchState{
		Base: st.Base, CommitsAhead: st.CommitsAhead, FilesChanged: st.FilesChanged,
		Insertions: st.Insertions, Deletions: st.Deletions, Counted: st.Counted,
		WorktreeExists: st.WorktreeExists, Merged: string(st.Contained),
	}
}
...
func (r runReporter) emitResult(idx int, status string, cr crankResult) {
	...
	r.emit(api.RunTaskUpdate{
		...
		// #602: what the branch looks like now — reported alongside the existing fields, changing
		// none of them.
		BranchState: cr.branchState,
