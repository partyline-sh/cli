...
func realTaskExec(repo string, o crankOpts) taskExec {
	return func(i int, task string) crankResult {
		...
		if werr != nil {
			...
			if r.note == "committed" {
				if mp := applyMergePolicy(realRunner(repo), branch, title, o.mergePolicy); mp != "" {
					r.note += " · " + mp
				}
			}
		}
...
func runCrankClaim(repo string, o crankOpts) {
	...
	claim := func() (*api.ClaimedTask, error) { return api.ClaimNextTask(base, token, o.run) }
		...
		go func() {
			...
			for {
				...
				if r.ok {
					...
					report.emit(ct.Idx, r.task, "done", r.branch, r.note)
				} else {
					n := atomic.AddInt64(&consec, 1)
					report.emit(ct.Idx, r.task, "failed", r.branch, r.note)
