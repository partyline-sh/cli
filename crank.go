...
func realTaskExec(repo string, o crankOpts, logger *runLogger) taskExec {
	return func(i int, task string) crankResult {
		...
		r := crankResult{task: task, branch: branch, ok: werr == nil, tokens: out.tokens, summary: strings.TrimSpace(out.text), rateLimitResetAt: out.rateReset, resumeHandle: out.resumeHandle, invokedSkills: out.invokedSkills}
		if werr != nil {
			r.note = werr.Error()
			...
			if r.note == "committed" {
				...
				if r.verify.ran && !r.verify.ok {
					...
					note, prURL := applyMergePolicy(realRunner(repo, mergeGitHubToken(o.run)), branch, title, o.mergePolicy)
					if note != "" {
						r.note += " · " + note
					}
