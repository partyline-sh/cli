...
func quarantinedCount(results []crankResult) int {
	...
	for _, r := range results {
		if r.ok && r.verify.ran && !r.verify.ok {
			n++
		}
	}
	return n
...
func maybePauseForQuarantine(results []crankResult, o crankOpts) {
	...
	if n := quarantinedCount(results); n > 0 {
		fmt.Fprintf(os.Stderr, "\n⏸ %d task(s) failed verification (quarantined, not merged) — needs approval\n", n)
...
func realTaskExec(repo string, o crankOpts, logger *runLogger) taskExec {
	return func(i int, task string) crankResult {
		...
		summary, tokens, werr := runWorker(wtPath, task, "", o.thread, o.allowBash, o.timeout, logger.sink(i))
		...
		r := crankResult{task: task, branch: branch, ok: werr == nil, tokens: tokens, summary: strings.TrimSpace(summary)}
		if werr != nil {
			r.note = werr.Error()
		} else if o.commit {
