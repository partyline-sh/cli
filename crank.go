...
func crankMain(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--skills-dir":
			...
			if i++; i < len(args) {
				o.skills = loadSkillSet(strings.TrimSpace(args[i]))
			}
		}
	}
...
func realTaskExec(repo string, o crankOpts, logger *runLogger) taskExec {
	return func(i int, task string) crankResult {
		...
		_ = gitwt.MaterializeSkills(wtPath, toGitwtSkills(o.skills))
		...
		if werr != nil {
			...
			if r.note == "committed" {
				...
				r.verify = verifyTask(repo, wtPath, task, o.engine, verifyTimeout(o))
