...
type crankResult struct {
	...
	rateLimitResetAt time.Time    // engine throttled us mid-task; when the quota window resets (zero = not rate-limited)
...
func crankMain(args []string) {
   ...
   or: ptln crank --claim --run <id> [--workers N] [flags]   (fleet mode: claim tasks from the run store)`))
	...
	if o.resume && !o.restart && o.run != "" {
		o.resumeSkip, o.resumeHandles = resumeStore(o.run)
	}
...
func resumeStore(runID string) (skip map[int]bool, handles map[int]string) {
...
func realTaskExec(repo string, o crankOpts, logger *runLogger) taskExec {
	return func(i int, task string) crankResult {
		...
		wtPath, branch, err := gitwt.Create(repo, name) // reuses the existing worktree if present (see gitwt.create)
		if err != nil {
		...
		out, werr := runWorker(wtPath, task, "", o.thread, o.allowBash, o.timeout, logger.sink(i), resume)
		...
		r := crankResult{task: task, branch: branch, ok: werr == nil, tokens: out.tokens, summary: strings.TrimSpace(out.text), rateLimitResetAt: out.rateReset, resumeHandle: out.resumeHandle}
		if werr != nil {
			r.note = werr.Error()
			...
			if r.note == "committed" {
				// Trust · T2: VERIFY before merge — two layers. T2a: the project's executable
				// acceptance checks (.partyline/verify). T2b: an independent adversarial reviewer
				// (.partyline/review). Pass (or neither enabled) → the branch is eligible to merge
				// per policy. Either fails → QUARANTINE: skip the merge, leave the branch for a
				// human, carry the reasons (runCrankWith flips the task to `blocked`, not `done`).
				r.verify = verifyTask(repo, wtPath, task, verifyTimeout(o))
