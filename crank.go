...
func realTaskExec(repo string, o crankOpts) taskExec {
	return func(i int, task string) crankResult {
		...
		if werr != nil {
			...
			if r.note == "committed" {
				// Trust · T2a: VERIFY before merge. Run the project's acceptance checks (from the
				// BASE repo's .partyline/verify) against this worktree. Pass (or no checks) → the
				// branch is eligible to merge per policy. Any check fails → QUARANTINE: skip the
				// merge, leave the branch for a human, and carry the reasons (runCrankWith flips
				// the task to `blocked`, not `done`).
				r.verify = runChecks(wtPath, readChecks(repo), verifyTimeout(o))
