...
// indexedOnly forces the 256-colour path even on a truecolor terminal.
//
// The wordmark's gradient is brand identity, and on a truecolor terminal it was emitted as
// 38;2;r;g;b — which package main's themed() cannot rewrite, since that only remaps ANSI-256
// indexes. So the banner stayed amber-and-pink under every theme.
//
// That was never the intent: each theme table in llms_theme.go carries explicit "banner
// gradient →" remaps for exactly those stops. They were simply unreachable on the terminals
// most people use. Under a non-identity theme the wordmark now renders indexed so the remap can
// land; Midnight, the identity theme, keeps full truecolor fidelity.
var indexedOnly bool

// UseIndexedColor forces (or releases) the 256-colour path. Called once at startup, from
// whoever knows whether a theme is active.
func UseIndexedColor(on bool) { indexedOnly = on }
...
// EnvBadge marks a NON-PRODUCTION control plane, and returns "" for production so the common case
// stays unadorned. Rendered next to the wordmark.
//
// This exists because the failure it prevents is silent and expensive: with credentials now scoped
// per environment, the same binary can be signed into staging and production at once, and nothing
// on screen would otherwise distinguish them. Believing you are on staging while acting on prod is
// the kind of mistake you only make once, memorably.
func EnvBadge(label string) string {
	if label == "" {
		return ""
	}
	return Fg(AmberRGB) + "[" + label + "]\x1b[0m"
}
...
// PillRGB is the focused-pill background — the pink behind the active tab segment
// (the RGB behind pill.go's SGR strings, exported for renderers that need the raw value).
var PillRGB = [3]int{0xff, 0x5e, 0x8a}

// Hex renders an RGB triple as a #rrggbb string, for surfaces that take hex colors
// (the tmux backend's generated conf) rather than SGR sequences.
func Hex(rgb [3]int) string { return fmt.Sprintf("#%02x%02x%02x", rgb[0], rgb[1], rgb[2]) }
