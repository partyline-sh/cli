...
var Preset = Vocab{
	...
	Terms: []Term{
		...
		{Key: "report", Doc: "An agent that LOOKS and tells you: it reads the repo read-only, writes an evaluation and a verdict, and produces no branch, commit or pull request. What a trigger should use to answer \"what just happened\" — the posture is enforced by the engine's tool flags, not asked for in the prompt."},
	},
}
...
var Engine = Vocab{
	...
	Terms: []Term{
		...
		{Key: "llm", Doc: "The bridge. A thin CLI over anything OpenAI-compatible, plus plugins for local models — so a self-hosted or proxied endpoint is llm's own configuration and partyline never learns about it. No resume, no stream parsing, no MCP."},
		{Key: "prime-agent", Doc: "Prime Intellect's agent. Headless via -p; model by --model. Its own docs are explicit that it has no sandbox, which matters less here because every task already runs in an isolated worktree. Sessions and a JSON mode exist but are unverified, so no capability is claimed."},
	},
}
