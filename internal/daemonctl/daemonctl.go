...
func SocketPath() string {
	// Under the per-environment config root (api.ConfigDir): a daemon is registered to ONE control
	// plane, so a staging daemon and a prod daemon on the same machine must not share a socket or
	// a state directory — otherwise whichever started last silently owns the other's identity.
	return filepath.Join(api.ConfigDir(), "daemon", "control.sock")
}
