...
// Skill is the minimum a materializer needs: a validated slug + the SKILL.md body. Deliberately NOT
// the api.Skill type — gitwt is a low-level utility and must not depend on the HTTP client. The caller
// converts (api.Skill → gitwt.Skill) at the boundary.
type Skill struct {
	Name string
	Body string
}
