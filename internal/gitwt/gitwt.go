...
// FlatSlug is Slug with '/' collapsed to '-' — a single-segment, D/F-conflict-proof branch name.
// Slug keeps '/' because it's git-legal (a `feat/x` ref), but auto-derived names (a crank task title
// like "Move Epic/Feature/Task…") pick up accidental slashes that then can't coexist with a plain
// `…Move-Epic` branch (git's directory/file ref conflict). Use this whenever the name is generated,
// not author-chosen.
func FlatSlug(name string) string { return strings.ReplaceAll(Slug(name), "/", "-") }

// WorktreeContainer is the single hidden directory that holds ALL of a parent directory's
// worktrees. One dot-dir instead of N loose `repo--branch` siblings: a dev directory with a
// dozen crank tasks in flight stays one `ls` entry, and everything inside is self-evidently
// partyline's to manage.
const WorktreeContainer = ".partyline-worktrees"

// Path returns where a worktree for (repo, branch) lives:
// `<parent>/.partyline-worktrees/<repoName>--<slug>` with '/' in the branch flattened.
// Outside the repo (not a subdirectory) so nothing needs gitignoring and `git status` in the
// main tree stays clean; inside ONE container so the parent directory doesn't sprawl — 20+
// loose `partyline--crank-…` siblings of the repo is what this replaces.
//
// LEGACY FALLBACK: worktrees created before the container existed live as direct siblings
// (`<parent>/<repoName>--<slug>`). If that path exists and the container one doesn't, return
// it — so resume, verify, and removal keep working on in-flight work across the upgrade.
// The legacy path is only ever RETURNED for a dir that already exists; nothing new is created
// there.
func Path(repo, branch string) string {
	name := filepath.Base(repo) + "--" + FlatSlug(branch)
	contained := filepath.Join(filepath.Dir(repo), WorktreeContainer, name)
	if _, err := os.Stat(contained); err == nil {
		return contained
	}
	if legacy := filepath.Join(filepath.Dir(repo), name); dirExists(legacy) {
		return legacy
	}
	return contained
}
...
// DefaultBaseName returns the remote default branch's bare NAME ("main"), for callers that need the
// name rather than a ref (e.g. `gh pr list --base <name>` in the conflict scan). Falls back to
// "main" when the remote's HEAD is unknown — the overwhelmingly common default, and a wrong guess
// merely yields an empty PR list, never a wrong action.
func DefaultBaseName(repo string) string {
	if ref := defaultBranchRef(repo); strings.HasPrefix(ref, "origin/") {
		return strings.TrimPrefix(ref, "origin/")
	}
	return "main"
}

// RemoteBase resolves an EXPLICIT base branch to a fresh "origin/<branch>" ref, or errors.
//
// This is the project's configured base branch (the branch its work forks from and opens PRs into).
// Unlike defaultBranchRef it never falls back: a base that doesn't exist on origin is an operator
// mistake, and silently rooting the work at origin/<default> instead would produce a branch — and
// then a PR — against the wrong target. Failing loudly here is the cheaper error.
func RemoteBase(repo, branch string) (string, error) {
	b := Slug(branch)
	if b == "" {
		return "", fmt.Errorf("base branch %q isn't a usable branch name", branch)
	}
	// Best-effort refresh so we fork from the CURRENT tip, not a stale local copy. An offline
	// daemon still succeeds below if it has the ref cached.
	_, _ = git(repo, "fetch", "origin", b)
	if !hasRemoteBranch(repo, b) {
		return "", fmt.Errorf("base branch %q does not exist on origin", b)
	}
	return "origin/" + b, nil
}
...
func create(repo, name, base string) (string, string, error) {
	...
	// Ensure the container exists — git's worktree add creates leading directories on most
	// versions, but relying on that across every git users run is how a path bug ships.
...
// Dirty reports whether a worktree has uncommitted work — tracked modifications OR untracked files.
//
// The load-bearing guard for automated teardown (#641). A cleanup pass found a crank worktree
// holding 23 uncommitted files; Remove uses `--force` (and falls back to os.RemoveAll), so a reaper
// that did not check first would have destroyed real work with no way back.
//
// FAILS DIRTY. If git cannot be asked — the path is gone, the admin dir was pruned, git errors —
// the honest answer is "I don't know", and the only safe reading of "I don't know" before an
// irreversible delete is "assume there is something here".
