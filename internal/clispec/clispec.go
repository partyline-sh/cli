...
type Spec struct {
	Name    string
	Aliases []string
	// Group names the section this command opens in `ptln help`. Set it on the FIRST command of
	// a section and leave it empty on the rest; the renderer carries it down the list. It is a
	// field rather than a comment so the help text cannot drift from the registry's own order.
	Group string
