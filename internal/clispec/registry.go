...
var Commands = []Spec{
	...
	{
		Name: "tmux", Summary: "host sessions in tmux (opt-in backend; needs tmux 3.3+)",
		Usage: []string{
			"ptln tmux               one window running your quick engine",
			"ptln tmux --resume      one window per saved workspace session",
			"PARTYLINE_MUX=tmux ptln     opt in for ptln/--resume too (built-in mux is the default)",
		},
		Flags: []Flag{
	...
	{Name: "settings", Summary: "every setting, its current state, and where to change it"},
	{
		...
		Usage: []string{
			...
			"ptln server upgrade          refresh the stack from this binary, pull, restart, migrate",
