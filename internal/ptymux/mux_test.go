...
func TestHomeWheelToArrows(t *testing.T) {
	cases := []struct {
		name string
		in   string
		want string
	}{
		{"wheel up → up arrow", "\x1b[<64;10;5M", "\x1b[A"},
		{"wheel down → down arrow", "\x1b[<65;10;5M", "\x1b[B"},
		{"click is dropped", "\x1b[<0;3;4M", ""},
		{"motion is dropped", "\x1b[<35;3;4M", ""},
		{"plain keys pass through", "jk\r", "jk\r"},
		{"wheel between keys", "a\x1b[<64;1;1Mb", "a\x1b[Ab"},
		{"no mouse → identity (no alloc path)", "\x1b[A", "\x1b[A"},
	}
	for _, c := range cases {
		if got := string(homeWheelToArrows([]byte(c.in))); got != c.want {
			t.Errorf("%s: homeWheelToArrows(%q) = %q, want %q", c.name, c.in, got, c.want)
		}
	}
}

func TestMouseAt(t *testing.T) {
	cases := []struct {
		name string
		in   []byte
		btn  int
		...
		{"wheel up", []byte("\x1b[<64;10;5M"), 64, 11, true},
		{"wheel down", []byte("\x1b[<65;10;5M"), 65, 11, true},
		{"left press", []byte("\x1b[<0;3;4M"), 0, 9, true},
		{"left release", []byte("\x1b[<0;3;4m"), 0, 9, true},
		{"trailing bytes ignored", []byte("\x1b[<64;1;1Mabc"), 64, 10, true},
		{"split read (incomplete)", []byte("\x1b[<64;10"), 0, 0, false},
		{"not a mouse seq", []byte("\x1b[A"), 0, 0, false},
		{"plain byte", []byte("x"), 0, 0, false},
		{"malformed", []byte("\x1b[<64;1;1Z"), 0, 0, false},
	}
	for _, c := range cases {
		btn, n, ok := mouseAt(c.in)
		if btn != c.btn || n != c.n || ok != c.ok {
			t.Errorf("%s: mouseAt(%q) = (%d,%d,%v), want (%d,%d,%v)", c.name, c.in, btn, n, ok, c.btn, c.n, c.ok)
		}
	}
}
