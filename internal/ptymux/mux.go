...
func (mx *Mux) drawBar() {
	...
	for k, c := range mx.children {
		...
		infos[k] = tabInfo{label: c.label, key: c.key, active: k == hl, ctx: ctx, shared: c.shared}
	}
	fn := mx.StatusFn
	banner := mx.banner
	...
	for k, ti := range infos {
		...
		// Context marker beside the state dot: this session is attached to a Common Ground
		// thread. Cyan ☎ when the agent is wired (recall/remember), dim when record-only.
		ctxMark := ""
		switch ti.ctx {
		case 1:
			ctxMark = "\x1b[38;5;39m☎\x1b[0m"
		case 2:
			ctxMark = "\x1b[38;5;245m☎\x1b[0m"
		}
		// Live-share marker (magenta ◉): this session is being broadcast over the relay.
		if ti.shared {
			ctxMark += "\x1b[38;5;201m◉\x1b[0m"
		}
		tabs = append(tabs, fmt.Sprintf("\x1b[38;5;%dm●\x1b[0m%s%s", dot, ctxMark, seg))
	}
