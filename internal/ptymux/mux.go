...
func (mx *Mux) spawn(sp Spec) error {
	...
	ch := &child{label: sp.Label, key: sp.Key, model: sp.Model, argv: sp.Argv, dir: sp.Dir, thread: sp.Thread, threadLabel: sp.ThreadLabel, engine: sp.Engine, threadWired: sp.Thread != "", sess: sess, part: part, gate: g}
	mx.mu.Lock()
