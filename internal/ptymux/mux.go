...
func (mx *Mux) liveSpecs() []Spec {
	...
	for _, c := range mx.children {
		out = append(out, Spec{Label: c.label, Key: c.key, Model: c.model, Argv: c.argv, Dir: c.dir, Thread: c.thread, ThreadLabel: c.threadLabel})
	}
	return out
}
...
func (mx *Mux) spawn(sp Spec) error {
	...
	ch := &child{label: sp.Label, key: sp.Key, model: sp.Model, argv: sp.Argv, dir: sp.Dir, thread: sp.Thread, threadLabel: sp.ThreadLabel, threadWired: sp.Thread != "", sess: sess, part: part, gate: g}
	mx.mu.Lock()
...
func (mx *Mux) drawBar() {
	...
	// The focused session's Common Ground context, for a right-aligned indicator.
	ctxOn, ctxWired, ctxLabel := false, false, ""
	if mx.mode == modeLive && mx.active >= 0 && mx.active < len(mx.children) {
		if ch := mx.children[mx.active]; ch.thread != "" {
			ctxOn, ctxWired, ctxLabel = true, ch.threadWired, ch.threadLabel
			if ctxLabel == "" {
				ctxLabel = "context"
			}
		}
	...
	if selecting {
		...
		hint = "▸ ←/→ or 1-9 switch · [ scroll" + ctx + " · o launcher · x close · q quit · esc cancel"
		hintClr = "1;38;5;214m"
	}
	...
	} else if ctxOn && !selecting && !pfx {
		// Right-aligned context indicator: which Common Ground thread THIS session is on. Cyan
		// ☎ when the agent is wired (can recall/remember); dim ·rec when it's record-only.
		seg := fmt.Sprintf("\x1b[38;5;39m☎ %s\x1b[0m", truncRunes(ctxLabel, 22))
		if !ctxWired {
			seg = fmt.Sprintf("\x1b[38;5;245m☎ %s ·rec\x1b[0m", truncRunes(ctxLabel, 22))
		}
		if pad := cols - visLen(bar) - visLen(seg) - 1; pad >= 1 {
			bar += strings.Repeat(" ", pad) + seg
		} else if keep := cols - visLen(seg) - 1; keep > 0 {
			bar = clipANSI(bar, keep) + " " + seg // tight terminal: keep the indicator, clip the left
		}
	}
...
// truncRunes shortens a plain (no-ANSI) string to n runes, appending … if it was cut.
