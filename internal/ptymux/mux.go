...
// or on the launcher). SetActiveThread attaches one to the focused session — the ctrl-\ c menu
// calls it so a thread you pick/create sticks (and the checkup starts watching it), no relaunch.
func (mx *Mux) ActiveThread() string {
	...
	return mx.children[mx.active].thread
}

func (mx *Mux) SetActiveThread(id string) {
	mx.mu.Lock()
	defer mx.mu.Unlock()
...
func (mx *Mux) liveSpecs() []Spec {
	...
	for _, c := range mx.children {
		out = append(out, Spec{Label: c.label, Key: c.key, Model: c.model, Argv: c.argv, Dir: c.dir, Thread: c.thread})
	}
	return out
}
...
func (mx *Mux) spawn(sp Spec) error {
	...
	ch := &child{label: sp.Label, key: sp.Key, model: sp.Model, argv: sp.Argv, dir: sp.Dir, thread: sp.Thread, sess: sess, part: part, gate: g}
	mx.mu.Lock()
