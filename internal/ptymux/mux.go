...
func (mx *Mux) handleBarSelect(data []byte) {
	for i := 0; i < len(data); {
		...
		default: // any other key exits the overlay (the key is dropped)
			mx.barCancel()
		}
		return
	}
...
func (mx *Mux) drawOverlay() {
	...
	if maxRows > avail-4 { // leave room for both borders + the separator + the hint row
		maxRows = avail - 4
	}
	if maxRows < 1 {
		return
	...
	const hint = "↑↓←→ move · ⏎ switch · esc cancel"
	w := visLen(hint)
	for _, it := range items {
		if v := visLen(it); v > w {
			w = v
	...
	boxH := visible + 4 // top border + content rows + separator + hint row + bottom border
	top := (avail-boxH)/2 + 1
	if top < 1 {
		top = 1
	}
	...
	fmt.Fprintf(&b, "\x1b[%d;%dH%s├%s┤\x1b[0m", r, left, frame, strings.Repeat("─", w+2))
	r++
	fmt.Fprintf(&b, "\x1b[%d;%dH%s│\x1b[0m \x1b[38;5;240m%s\x1b[0m %s│\x1b[0m", r, left, frame, pad(hint), frame)
	r++
	fmt.Fprintf(&b, "\x1b[%d;%dH%s╰%s╯\x1b[0m", r, left, frame, strings.Repeat("─", w+2))
	b.WriteString("\x1b8") // DECRC — restore the child's cursor + mode context
	mx.outMu.Lock()
