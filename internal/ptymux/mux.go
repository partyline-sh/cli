...
func (mx *Mux) Run() error {
	...
	// Periodically repaint the status bar in live mode so a BACKGROUND session changing
	// state (finishing its turn → "waiting", or starting work → "active") lights up its tab
	// without you having to switch. The bar owns the reserved bottom row and repaints under
	// a cursor save/restore, so this never disturbs the focused child.
	...
	go func() {
		...
		for {
			...
			case <-t.C:
				mx.mu.Lock()
				live := mx.mode == modeLive && !mx.picking && !mx.confirming && len(mx.children) > 0
				mx.mu.Unlock()
				if live {
					mx.drawBar()
				}
...
func (mx *Mux) drawBar() {
	...
	for k, c := range mx.children {
		infos[k] = tabInfo{label: c.label, key: c.key, active: k == mx.active}
	}
	var active *child
	if mx.active >= 0 && mx.active < len(mx.children) {
		active = mx.children[mx.active]
	}
	fn := mx.StatusFn
	mx.mu.Unlock()
	...
	// Restore the cursor by ABSOLUTE position (CUP) from the active child's emulator —
	// NOT DEC save/restore (\x1b7/\x1b8). Those share a single save slot with the child
	// app (e.g. Claude's input-line redraw), so the ticker's periodic repaint would
	// clobber the child's saved cursor and scramble its input line as you type. We hold
	// outMu across the whole sequence, so no child write interleaves and the child's vt
	// cursor reflects its last completed output.
	mx.outMu.Lock()
	if active != nil {
		cr, cc := active.sess.CursorPos()
		fmt.Fprintf(os.Stdout, "\x1b[%d;1H\x1b[2K%s\x1b[%d;%dH", rows, mx.skin(bar), cr, cc)
	} else {
