...
type gate struct {
	...
	active bool
	paused bool // scroll mode: still sniff modes, but DON'T paint (the viewport owns the screen)
	modes  modeState
}
...
// wantsMouse reports whether the child has any mouse-reporting mode on, i.e. it expects
// to receive click/motion events. The mux forwards non-wheel mouse only to such children.
func (g *gate) wantsMouse() bool {
	g.out.Lock()
	defer g.out.Unlock()
	return g.modes.mouse[1000] || g.modes.mouse[1002] || g.modes.mouse[1003]
}
...
func (mx *Mux) Run() error {
	...
	// Mouse capture (the wheel, so scrollback is per-child) is toggled per mode: switchTo /
	// repaintLive enable it for live children; gotoHome's modeDefaults() disables it for the
	// launcher. So it's not enabled here — the initial gotoLive/gotoHome below sets it.
	...
	for {
		...
		if woke { // Wake() fired with no keypress — repaint the launcher (e.g. a new approval)
			mx.mu.Lock()
		...
		if md == modeHome {
			// In the launcher the wheel moves the cursor (↑/↓); other mouse events are
			// stripped so they don't leak into the menu's key parser.
			norm := legacyKeys(homeWheelToArrows(buf[:n])) // menu wants legacy keys even if a child left kitty mode on
...
func (mx *Mux) handleInput(data []byte) bool {
	...
	i := 0
	for i < len(data) {
		// Mouse reports (we own SGR mouse). Wheel-up opens the per-child scrollback; other
		// mouse events go to the child only if it asked for the mouse. Never falls through to
		// the host's native (shared) scroll — that was the contamination bug.
		if !mx.sawPfx {
			if btn, mn, ok := mouseAt(data[i:]); ok {
				flush()
				switch btn {
				case 64: // wheel up → scroll back
					mx.scrollBy(wheelStep)
				case 65: // wheel down at the live bottom → nothing below
				default:
					if mx.activeWantsMouse() {
						mx.writeActive(data[i : i+mn])
					}
				}
				i += mn
				continue
...
func (mx *Mux) switchTo(i int) {
	...
	out = append(out, muxMouseOn()...) // re-assert AFTER the child's modes so the mux keeps the wheel
	os.Stdout.Write(out)
	ch.gate.active = true
	ch.gate.paused = false // never inherit a frozen gate from a prior scroll
	mx.outMu.Unlock()

	mx.drawBar()
}

// muxMouseOn enables SGR mouse reporting (button events + SGR encoding) so the terminal
// sends wheel ticks and clicks to us instead of scrolling its own (single, shared) buffer.
// This is what makes scrollback per-child; the tradeoff is native click-drag selection is
// captured (hold Shift/Option to select, as in tmux). modeDefaults() turns it off on teardown.
func muxMouseOn() string { return "\x1b[?1000h\x1b[?1006h" }
...
// activeWantsMouse reports whether the active child enabled mouse reporting (so non-wheel
// mouse events should be forwarded to it).
func (mx *Mux) activeWantsMouse() bool {
	mx.mu.Lock()
	ch := mx.curLocked()
	mx.mu.Unlock()
	return ch != nil && ch.gate.wantsMouse()
}
