...
type gate struct {
	...
	modes  modeState
	// barKeep: when the child last had the bar re-asserted under it. The bar is painted on EVENTS
	// (switch, ctrl-\); routine output never repainted it, so once the child scrolled the row away
	// it stayed away until the next event. Rate-limited so a chatty child doesn't repaint per byte.
	barKeep atomic.Int64
...
func (g *gate) Write(b []byte) (int, error) {
	...
	if live {
		// DEFEND THE ROW. A child that resets or over-reaches its scroll region would hand the
		// reserved last row back to the scrolling area, and the next newline would scroll our bar
		// off. Rewrite those on the way out — see scrollguard.go.
		if g.mx != nil {
			if _, bodyRows := g.mx.bodySize(); bodyRows > 0 {
				b = clampScrollRegion(b, bodyRows)
			}
		}
		os.Stdout.Write(b)
		...
		if g.mx != nil {
			...
			// …and RE-ASSERT it. flushBarLocked only paints a paint that was DEFERRED mid-frame; it
			// does nothing on ordinary output. So anything that disturbed the row — a torn region
			// sequence, a full reset, a child that repainted over it — left the bar gone until the
			// next ctrl-\. Repainting here makes the ribbon permanent rather than summoned.
			g.mx.keepBarLocked(&g.modes, &g.barKeep)
		}
	}
	g.out.Unlock()
...
func (mx *Mux) paintBody(ch *child) []byte {
	...
	// PIN THE BAR ROW. Clamping the child's DECSTBM (scrollguard.go) only constrains regions the
	// child actually SETS — a child that never sets one leaves the terminal default, which is the
	// FULL screen, and its first newline at the bottom scrolls our reserved row away. So assert the
	// body region ourselves on every repaint.
	//
	// DECSTBM HOMES THE CURSOR. That is the #238 trap: a region emitted after the snapshot leaves
	// the cursor at 1,1 instead of where the child left it, and every relative move the child makes
	// afterwards is offset — the garbled input line. So the region goes in FIRST and the child's
	// real cursor is re-asserted LAST, which is the same ordering that fixed #238.
	if _, bodyRows := mx.bodySize(); bodyRows > 0 {
		col, row := ch.sess.CursorPos()
		if col > 0 && row > 0 {
			out = append(scrollRegionFor(bodyRows), out...)
...
func (mx *Mux) drawBar() {
	...
	for k, ti := range infos {
		...
		if ti.ctx != 0 {
			...
			ctxMark = fmt.Sprintf("\x1b[38;5;%dm☎%s", ctxClr, segFg)
		}
...
// barKeepalive bounds how often output re-asserts the bar. A frame is ~16ms; painting more often
// than that buys nothing visible and costs a cursor save/restore per write on a chatty child.
const barKeepalive = 100 * time.Millisecond

// keepBarLocked repaints the reserved row under a child that may have scrolled or drawn over it.
// Called from gate.Write with the output lock held.
//
// Deferred while the child is mid-frame for the same reason flushBarLocked is: Claude Code brackets
// repaints in ?2026h…?2026l and draws with RELATIVE motion, so a cursor jump composed into its frame
// offsets everything after it (the interleaved-input-line bug). barPending carries it to the moment
// the frame closes.
func (mx *Mux) keepBarLocked(m *modeState, last *atomic.Int64) {
	if m.midFrame() {
		mx.barPending = true
		return
	}
	now := time.Now().UnixNano()
	if prev := last.Load(); prev != 0 && now-prev < int64(barKeepalive) {
		return
	}
	last.Store(now)
	if len(mx.barBytes) > 0 {
		os.Stdout.Write(mx.barBytes)
	}
}
