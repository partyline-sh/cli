...
func childEnv(sp Spec) []string {
	...
	for _, kv := range env {
		if strings.HasPrefix(kv, "PARTYLINE_THREAD_ID=") || strings.HasPrefix(kv, "PARTYLINE_ENGINE=") ||
			strings.HasPrefix(kv, "PARTYLINE_SESSION_KEY=") {
...
// visBytes counts s the way clipANSI budgets it: non-escape BYTES, not display columns.
//
// DO NOT replace this with brand.VisWidth. It is deliberately NOT the shared column metric: it
// is the exact budget of clipANSI, the clipper it is paired with, which also counts bytes.
// Measuring the bar in columns while clipping it in bytes makes the two disagree by however
// many multi-byte glyphs the tab labels happen to contain, and the right-aligned hint gets cut
// early. Everything else in the app measures display columns via internal/brand; the status
// bar's width budget stays here, in bytes, next to its clipper. See drawBar.
func visBytes(s string) int {
	n := 0
	for i := 0; i < len(s); {
		if s[i] == 0x1b {
			for i < len(s) && s[i] != 'm' {
...
func (mx *Mux) drawBar() {
	...
	bar := barPhone() + strings.Join(tabs, barSep())
	...
	hint := barHint(selecting, pfx, setup, splitLive, cols-visBytes(bar)-2)
	...
	gap := cols - visBytes(bar) - visBytes(hint)
	if gap < 2 {
		gap = 2
	}
