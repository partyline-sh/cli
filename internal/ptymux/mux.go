...
type modeState struct {
	...
	scrollTop    int          // DECSTBM top margin (1-based); 0 = full screen (unset/reset)
	scrollBot    int          // DECSTBM bottom margin (1-based); 0 = full screen
}
...
func (m *modeState) observe(b []byte) {
	...
	// DECSTBM (scroll region) — a CSI WITHOUT '?': ESC [ <top> ; <bot> r (bare ESC[r resets to full).
	// It's how Claude Code (and other TUIs) pin an input box; the mux resets the region on every switch,
	// so if we don't recapture + restore it the child's input misrenders after a tab switch (#238).
	for i := 0; i+1 < len(b); i++ {
		if b[i] != 0x1b || b[i+1] != '[' {
			continue
		}
		...
		if params := string(b[i+2 : j]); params == "" {
			m.scrollTop, m.scrollBot = 0, 0 // ESC[r → full screen
		} else if parts := strings.SplitN(params, ";", 2); len(parts) == 2 {
			t, e1 := atoiSafe(parts[0])
			btm, e2 := atoiSafe(parts[1])
			if e1 == nil && e2 == nil && t >= 1 && btm > t {
				m.scrollTop, m.scrollBot = t, btm
			}
		}
		i = j
	}
...
func (m *modeState) restore() []byte {
	...
	if m.scrollBot > 0 { // restore the child's scroll region (DECSTBM) — homes the cursor, so paintBody re-asserts it after (#238)
		fmt.Fprintf(&b, "\x1b[%d;%dr", m.scrollTop, m.scrollBot)
	}
...
func (mx *Mux) paintBody(ch *child) []byte {
	...
	// restore() may have emitted DECSTBM (scroll region), which HOMES the cursor — re-assert the
	// child's real cursor LAST so its input line lands where the snapshot put it (#238).
	out = append(out, []byte(ch.sess.CursorSeq())...)
	return out
}
