...
type Mux struct {
	...
	wakeR, wakeW int
	barActive    bool      // the ctrl-\ arrow session-switcher overlay is open
	barSel       int       // candidate index (session, or len(children) = the launcher) while barActive
	barScroll    int       // first visible row, kept so the selected row stays in view when the list overflows the box
	barCh        *child    // the child paused (frozen) behind the overlay — resumed when it closes
...
func (mx *Mux) handleInput(data []byte) bool {
	...
	for i < len(data) {
		...
		if mx.sawPfx {
			mx.sawPfx = false
			// Arrow after the prefix opens the boxed session-switcher overlay (←/→/↑/↓ move
			// the selection, ⏎ commits, esc cancels) — handleBarSelect owns input until then.
			if _, an := arrowAt(data[i:]); an > 0 {
				flush()
				mx.startBarSelect(data[i : i+an]) // hands the panel's freeze off to the overlay itself
				i += an
				continue
			}
...
// ---- ctrl-\ arrow: boxed vertical session-switcher overlay ----

// startBarSelect opens the overlay: it takes over the freeze the command panel already put on
// the active child (mx.pfxCh), seeds the selection one step from the active session in the
// arrow's direction, and paints the box. It owns input (handleBarSelect) until ⏎ commits or esc
// cancels. arrow is the raw arrow key bytes (to read its direction).
func (mx *Mux) startBarSelect(arrow []byte) {
	mx.mu.Lock()
	ch := mx.pfxCh // ownership of the freeze transfers from the command panel to the overlay
	mx.pfxCh = nil
	if len(mx.children) == 0 {
		mx.mu.Unlock()
		...
		mx.repaintLive() // nothing to pick between — drop back to the live view (clears the panel)
		return
	}
	mx.barActive = true
	mx.barSel = mx.active
	...
	mx.barCh = ch
	mx.mu.Unlock()
	mx.repaintFrozen(ch) // erase the command panel behind us; the child stays paused for the overlay
	mx.drawBar()         // swaps the bar hint to "session switcher" (barActive is already true)
	if dir, _ := overlayDir(arrow); dir != 0 {
		mx.moveSel(dir) // moves the candidate + repaints the overlay
		return
	}
	mx.drawOverlay()
}

// overlayDir maps an arrow key to a step through the overlay's single column of rows: up/left =
// previous (-1), down/right = next (+1). Unlike arrowAt (which only moves horizontally, for the
// old bar highlight it replaced), every direction moves this list since it has only one column.
func overlayDir(b []byte) (dir, n int) {
...
// moveSel slides the candidate through the session list (wrapping) and repaints the overlay —
// cheap, no session snapshot. The expensive switch happens once, on commit.
func (mx *Mux) moveSel(d int) {
	mx.mu.Lock()
	total := len(mx.children) + 1 // + the virtual launcher row
	if total > 0 {
		mx.barSel = (mx.barSel + d + total) % total
	}
	mx.mu.Unlock()
	mx.drawOverlay()
}

// barCommit switches to the highlighted candidate (the one snapshot repaint) + exits. switchTo/
// gotoHome fully reset the child's gate (active + paused) themselves, so the overlay's freeze on
// barCh needs no separate resume here.
func (mx *Mux) barCommit() {
	...
	mx.barCh = nil
	mx.mu.Unlock()
	if sel >= n { // the virtual launcher row at the end of the list
		mx.gotoHome()
		return
	}
	mx.switchTo(sel)
...
// barCancel exits the overlay without switching: repaints the frozen child's live screen
// (erasing the box) while it's still paused, then resumes it — same no-interleave discipline as
// exitScroll/disarmPanel.
...
// handleBarSelect owns input while the overlay is up: any arrow moves, ⏎ commits, esc cancels,
// 1-9 jump+commit; any other key exits without switching.
func (mx *Mux) handleBarSelect(data []byte) {
	for i := 0; i < len(data); {
		if dir, n := overlayDir(data[i:]); n > 0 {
			if dir != 0 {
				mx.moveSel(dir)
			}
			i += n
		...
		default:
			// A ctrl-\ COMMAND letter fired from the switcher: the modal is a first-class command
			// surface (its footer lists these), so run the command instead of just cancelling. Hand
			// the overlay's freeze back to the command path (barCh → pfxCh) and re-feed the key through
			// handleInput's prefix branch — the SAME proven dispatch the command panel uses, so the
			// pause/resume + repaint discipline is identical (disarmPanel resumes mx.pfxCh). A 'q' here
			// can only reach requestQuit's confirm (there's ≥1 session while the switcher is open), so
			// it never needs to signal an immediate quit up through this void-returning handler.
			if mx.overlayCommandKey(b) {
				mx.mu.Lock()
				mx.barActive = false
				mx.pfxCh = mx.barCh
				mx.barCh = nil
				mx.sawPfx = true // handleInput's prefix branch dispatches this key against pfxCh
				mx.mu.Unlock()
				mx.handleInput([]byte{b})
			} else { // any other key exits the overlay (the key is dropped)
				mx.barCancel()
			}
		}
		return
...
// overlayCommandKey reports whether a byte is a ctrl-\ command that also fires directly from the
// switcher overlay (arrows / 1-9 / ⏎ / esc are handled separately). The capability commands are
// gated on their handler being wired, exactly as drawCmdPanel gates what it lists — so pressing an
// unavailable letter in the overlay falls through to cancel rather than a no-op dispatch.
func (mx *Mux) overlayCommandKey(b byte) bool {
	switch b {
	case 'x', 'q', 'o', '[':
		return true
	}
	...
	case 'n':
		return mx.NewFn != nil
	case 'c':
		return mx.ContextFn != nil
	case 'm':
		return mx.MCPFn != nil
	case 'w':
		return mx.WorktreeFn != nil
	case 'g':
		return mx.KeepGoingFn != nil
	case 's':
		return mx.ShareFn != nil
	}
	return false
}
...
// repaintFrozen redraws child ch's live screen (erasing whatever overlay/panel sat on top of it)
// while leaving its gate PAUSED — used when the session-switcher overlay takes over the freeze
// the command panel already put on the child, so it never flashes live in between the two.
func (mx *Mux) repaintFrozen(ch *child) {
	if ch == nil {
		return
	}
	var out []byte
...
func (mx *Mux) onResize() {
	if c, r, e := term.GetSize(int(os.Stdout.Fd())); e == nil && c > 0 && r > 0 {
		...
		// Drop the session-switcher overlay too: its box geometry is stale, and leaving
		// barActive true would keep routing input to handleBarSelect over a screen the
		// switchTo/renderHome repaint below is about to overwrite — with the child's gate
		// state (paused for the overlay) no longer matching what the flag claims.
		wasBarActive := mx.barActive
		mx.barActive, mx.barCh = false, nil
		ch := mx.curLocked()
		mx.mu.Unlock()
		if (wasScrolling || wasBarActive) && ch != nil {
			ch.gate.setPaused(false)
		}
		mx.resizeAll()
		if md == modeHome {
...
func (mx *Mux) drawBar() {
	...
	// The session-switcher overlay owns its own selection highlight now (ctrl-\ arrow no longer
	// highlights a bar segment) — the bar always reflects the actually-focused tab.
	hl := mx.active
	...
	overlay := mx.barActive // the session-switcher overlay is up and owns input
	mx.mu.Unlock()
	...
	// The launcher is a virtual last item in the switch bar: arrow past the last session (in the
	// overlay) to reach it, ⏎ opens it. Always shown (dimmed) so it's discoverable.
	lseg := "\x1b[38;5;245m ⌂ launcher \x1b[0m"
	...
	switch {
	case overlay:
		// The overlay owns the switch UI, but on a terminal too small for it to paint
		// (drawOverlay's own degrade guard), this is the ONLY visible sign a modal is up and
		// eating input — without it a tiny terminal goes silently unresponsive until esc.
		// Shown unconditionally (not just on the tiny-terminal path) for the same reason
		// drawCmdPanel's bar hint isn't gated on the panel itself fitting.
		hint = "▸ session switcher · esc cancels"
		hintClr = "1;38;5;214m"
	case pfx:
	...
	// While the session-switcher OVERLAY is up it shows the full session list itself (with the
	// command legend in its own footer), so the bottom tabs would just duplicate it — drop them and
	// leave only the modal hint. (The command-panel armed state keeps the tabs: they're your session
	// context while you pick a command, not a duplicate of an on-screen list.)
	if overlay {
		bar = ""
	}
...
// overlayItems builds one pre-colored row per live session (number, activity dot, label, ☎/◉
// markers — exactly the fields drawBar already tracks; no new per-session data), plus a final
// row for the ⌂ launcher. Every color is an index from the documented source palette
// (llms_theme.go:15-18) so drawOverlay's mx.skin() call remaps it correctly per theme.
func (mx *Mux) overlayItems() []string {
	type info struct {
		label  string
		key    string
		ctx    int // 0 none · 1 attached+wired · 2 attached record-only
	...
	mx.mu.Lock()
	infos := make([]info, len(mx.children))
	for k, c := range mx.children {
		ctx := 0
		if c.thread != "" {
			ctx = 2
		...
		busy := c.gate != nil && time.Since(time.Unix(0, c.gate.lastOut.Load())) < 1500*time.Millisecond
		infos[k] = info{label: c.label, key: c.key, ctx: ctx, shared: c.shared, busy: busy}
	}
	fn := mx.StatusFn
	mx.mu.Unlock()

	maxLabel := visLen("launcher")
	for _, in := range infos {
		if l := visLen(in.label); l > maxLabel {
			maxLabel = l
		}
	}
	pad := func(s string) string {
		if p := maxLabel - visLen(s); p > 0 {
			return s + strings.Repeat(" ", p)
		}
		return s
	}
	row := func(num, dot string, dotClr int, label, marks string) string {
		return fmt.Sprintf("\x1b[38;5;245m%2s\x1b[0m \x1b[38;5;%dm%s\x1b[0m \x1b[38;5;252m%s\x1b[0m  %s",
			num, dotClr, dot, pad(label), marks)
	}

	items := make([]string, 0, len(infos)+1)
	for k, in := range infos {
		...
		if fn != nil {
			if s := fn(in.key); s == "active" || s == "waiting" {
				state = s
			}
		}
		dotClr := 245 // idle / unknown (grey — documented dim)
		switch state {
		case "active":
			dotClr = 46 // green — documented "running"
		case "waiting":
			dotClr = 214 // amber — documented "waiting"
		}
		var marks string
		if in.ctx != 0 {
			markClr := 51 // documented "marked" — attached + wired, idle
			if in.ctx == 2 {
				markClr = 245 // record-only (agent not wired) — dim at rest
			}
			if state == "active" || state == "waiting" {
				markClr = dotClr // live: same colour as the activity dot
			}
			marks += fmt.Sprintf("\x1b[38;5;%dm☎\x1b[0m", markClr)
		}
		if in.shared {
			marks += "\x1b[38;5;215m◉\x1b[0m" // documented accent — being broadcast live
		}
		items = append(items, row(fmt.Sprintf("%d", k+1), "●", dotClr, in.label, marks))
	}
	items = append(items, row("⌂", " ", 245, "launcher", ""))
	return items
}

// drawOverlay paints the boxed vertical session-switcher: overlayItems' rows with the current
// candidate highlighted, framed like drawCenteredBox's rounded box but painted with drawCmdPanel's
// overlay discipline — DECSC/DECRC, individual rows repainted rather than a full-screen clear, so
// the frozen child stays visible behind/around it and its output stays gated the whole time.
// Capped at ~half the screen (same budget as drawCmdPanel); scrolls the selected row into view
// when the list is taller than that.
func (mx *Mux) drawOverlay() {
	mx.mu.Lock()
	cols, rows := mx.cols, mx.rows
	sel := mx.barSel
	scroll := mx.barScroll
	...
	mx.mu.Unlock()
	if rows < 3 || cols < 6 {
		return // no room for a box — degrade gracefully on tiny/tty-less (0×0) terminals
	}
	items := mx.overlayItems()
	total := len(items)
	if total == 0 {
		return
	}
	...
	avail := rows - 1 // stay clear of the reserved bottom bar row
	if avail < 5 {
		return
	}
	maxRows := rows/2 - 1 // cap at ~half the screen, same budget as drawCmdPanel (mux.go:2183)
	if maxRows < 1 {
		maxRows = 1
	}
	if maxRows > avail-5 { // leave room for both borders + the separator + the TWO hint rows
		maxRows = avail - 5
	}
	if maxRows < 1 {
		return
	...
	// Two footer lines make the modal a first-class COMMAND surface, not just a picker: the nav line
	// (switch this list) and the command legend (the ctrl-\ commands, which now also fire directly
	// from here — see handleBarSelect). Only the available commands are listed. The box widens to fit
	// whichever line/label is longest (capped at the terminal width), so nothing useful gets clipped
	// on a normal terminal; a narrow one still degrades via the clip-then-pad discipline below.
	const navHint = "↑↓ move · ⏎ switch · 1-9 jump · esc"
	var cmds []string
	if newFn {
		cmds = append(cmds, "n new")
	}
	if ctxFn {
		cmds = append(cmds, "c context")
	}
	if mcpFn {
		cmds = append(cmds, "m mcp")
	}
	if wtFn {
		cmds = append(cmds, "w worktree")
	}
	if kgFn {
		cmds = append(cmds, "g keep-going")
	}
	if shFn {
		cmds = append(cmds, "s share")
	}
	cmds = append(cmds, "[ scroll", "o launcher", "x close", "q quit")
	cmdHint := strings.Join(cmds, " · ")
	w := visLen(navHint)
	if v := visLen(cmdHint); v > w {
		w = v
	}
	for _, it := range items {
		if v := visLen(it); v > w {
	...
	boxH := visible + 5 // top border + content rows + separator + 2 hint rows + bottom border
	top := (avail-boxH)/2 + 1
	if top < 1 {
		top = 1
	}
	...
	const frame = "\x1b[38;5;238m" // documented frame/divider index
	// clip first (a long label or a narrow terminal can push a row past w), then pad — same
	// clip-then-pad discipline drawBar/drawCmdPanel use, so a row can never push the right
	// border off-frame.
	pad := func(s string) string {
		s = clipANSI(s, w)
		if p := w - visLen(s); p > 0 {
	...
	var b strings.Builder
	b.WriteString("\x1b7") // DECSC — save the child's cursor + mode context
	r := top
	fmt.Fprintf(&b, "\x1b[%d;%dH%s╭%s╮\x1b[0m", r, left, frame, strings.Repeat("─", w+2))
	r++
	for i := 0; i < visible; i++ {
		idx := scroll + i
		line := items[idx]
		if idx == sel {
			// Re-assert the selection band (documented bg 236) + selected-text colour (231)
			// after every internal reset in the row, so per-item colours (dot/marks) still show
			// their own hue but the highlight band survives across them.
			hl := strings.ReplaceAll(line, "\x1b[0m", "\x1b[0m\x1b[48;5;236m\x1b[38;5;231m")
			fmt.Fprintf(&b, "\x1b[%d;%dH%s│\x1b[0m\x1b[48;5;236m\x1b[38;5;231m %s \x1b[0m%s│\x1b[0m",
				r, left, frame, pad(hl), frame)
		} else {
			fmt.Fprintf(&b, "\x1b[%d;%dH%s│\x1b[0m %s %s│\x1b[0m", r, left, frame, pad(line), frame)
		}
		r++
	}
	fmt.Fprintf(&b, "\x1b[%d;%dH%s├%s┤\x1b[0m", r, left, frame, strings.Repeat("─", w+2))
	r++
	// Nav line (how to drive the list), then the command legend (the ctrl-\ commands, live from here).
	fmt.Fprintf(&b, "\x1b[%d;%dH%s│\x1b[0m \x1b[38;5;240m%s\x1b[0m %s│\x1b[0m", r, left, frame, pad(navHint), frame)
	r++
	fmt.Fprintf(&b, "\x1b[%d;%dH%s│\x1b[0m \x1b[38;5;245m%s\x1b[0m %s│\x1b[0m", r, left, frame, pad(cmdHint), frame)
	r++
	fmt.Fprintf(&b, "\x1b[%d;%dH%s╰%s╯\x1b[0m", r, left, frame, strings.Repeat("─", w+2))
	b.WriteString("\x1b8") // DECRC — restore the child's cursor + mode context
	mx.outMu.Lock()
