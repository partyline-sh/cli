...
func (mx *Mux) paintBody(ch *child) []byte {
	...
	//
	// The ORDER of region vs. snapshot vs. cursor is the whole of #238 — see repaint.go, which owns
	// it as a pure function so repaint_test.go can assert it against a real vt round-trip.
	_, bodyRows := mx.bodySize()
	col, row := ch.sess.CursorPos()
	return composeRepaint(out, ch.gate.modes.restore(), bodyRows, col, row)
}
