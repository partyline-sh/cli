...
// mouseAt parses an SGR mouse report at the start of b: ESC [ < btn ; col ; row (M|m).
// Returns the button code and bytes consumed. The mux enables SGR mouse (1006) itself,
// so every wheel tick / click arrives in this one encoding. Wheel-up is button 64,
// wheel-down 65. ok is false if b doesn't start with a complete SGR mouse sequence.
func mouseAt(b []byte) (btn, n int, ok bool) {
	if len(b) < 4 || b[0] != 0x1b || b[1] != '[' || b[2] != '<' {
		return 0, 0, false
	}
	j, cur, has, fields := 3, 0, false, 0
	for j < len(b) {
		c := b[j]
		switch {
		case c >= '0' && c <= '9':
		...
		case c == ';':
			if fields == 0 {
				btn = cur // the first field is the button code
			}
			fields++
			cur, has = 0, false
			j++
		case c == 'M' || c == 'm': // press (M) / release (m) — both terminate the report
			if fields == 0 && has {
				btn = cur
			}
			return btn, j + 1, true
		default:
			return 0, 0, false // malformed — let the byte fall through as normal input
		}
	}
	return 0, 0, false // ran off the end mid-sequence (split read) — wait for more
}

// homeWheelToArrows rewrites a raw input chunk for the launcher (home mode): SGR wheel
// reports become ↑/↓ so the mouse moves the cursor (wheel-up = up, wheel-down = down,
// honouring list-vs-detail focus via the menu's normal arrow handling), and any other
// mouse events (clicks / motion) are dropped so their escape sequences never leak into
// the menu's key parser. Non-mouse bytes pass through unchanged.
func homeWheelToArrows(b []byte) []byte {
	// Fast path: no mouse report present → return the input as-is (no allocation).
	if !bytesContainMouse(b) {
		return b
	}
	out := make([]byte, 0, len(b))
	for i := 0; i < len(b); {
		if btn, n, ok := mouseAt(b[i:]); ok {
			switch btn {
			case 64: // wheel up → cursor up
				out = append(out, 0x1b, '[', 'A')
			case 65: // wheel down → cursor down
				out = append(out, 0x1b, '[', 'B')
			} // other mouse events (clicks/motion): dropped
			i += n
			continue
		}
		out = append(out, b[i])
...
// bytesContainMouse reports whether b holds the start of an SGR mouse report (ESC [ <).
func bytesContainMouse(b []byte) bool {
	for i := 0; i+2 < len(b); i++ {
