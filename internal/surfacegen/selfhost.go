...
var notInDotEnv = map[string]string{
	...
	"NEXT_PUBLIC_SUPABASE_ANON_KEY": "the legacy name for the anon key, read only as a fallback. Set PGRST_ANON_KEY — that is the one to set",
	"NEXT_PUBLIC_SUPABASE_URL":      "the legacy name for PostgREST's origin, read only as a fallback. Set PGRST_URL — that is the one to set",
...
func publishedStackFile(root, src, note string, subs ...string) ([]byte, error) {
	...
	b.WriteString(header("#"))
	// NOT "the file partyline.sh itself runs": partyline.sh serves documentation and marketing, with
	// no Postgres, no migrations and no relay behind it. These are the files a partyline BOX runs, and
	// they are generated from the stack in this repo so the published copy cannot drift from it.
	b.WriteString("#\n# Published copy of " + src + " — generated from the stack in this repo, so it\n# cannot drift from the file a partyline box actually runs.\n")
...
var featureNotes = map[string]featureNote{
	...
	// OIDC is the ONLY sign-in path. Keycloak ships in the stack and scripts/env-bootstrap.sh
	// writes its issuer, client id and client secret, so a fresh install already has these set;
	// pointing them at another IdP is how you replace it.
	"oidc": {
		Does:  "Signs people in — the sign-in path for the whole control plane, so a box without it has no way for anyone to log in. The bundled Keycloak is configured automatically; set these to use your own provider (Authentik, Okta, Auth0, Entra) instead.",
	...
	"scribe": {
		Does:  "Lets the server distil a party's chat into durable facts on that party's context thread. On demand only (`ptln thread distill`), and each fact lands `proposed` — invisible to agents until a human accepts it. The party text is sent to the provider on this key, from your box.",
		Where: "an API key for the model provider you want the scribe to use — Anthropic by default. A team can supply its own instead, in Settings → Team → Ambient capture.",
	},
	"session_key_wrap": {
