...
const controlPlane = `The optional web control plane is a small surface around the CLI. You describe
work at /describe and the conversation continues at /p/<id>; /dashboard shows your machines and what
each is running, /threads holds the shared context, /history is the log of what happened, and
/projects is the label that joins machines and threads (the label resolves to a real path only on
the machine itself).
Nothing on the web executes anything by itself — a machine has to be running ` + "`ptln daemon`" + ` and to
...
// surface at generation time: prose is allowed here, unverified prose is not. The work board,
// /runs and /runs/<id> were named here until the pipeline UI was removed; this check is what
// caught the paragraph still describing them.
var anchorRoutes = []string{"/describe", "/dashboard", "/threads", "/history", "/projects", "/p/[id]"}
