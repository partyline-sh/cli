...
func Files(root string) ([]File, error) {
	...
	for _, p := range []struct {
		...
		{"web/public/self-host/docker-compose.yml", "deploy/stack/docker-compose.yml",
			"A byte-for-byte copy of the file our own stack uses — the registry no longer differs. " +
				"It used to: the source pulled from GHCR and only this published copy was rewritten to " +
				"Docker Hub, the anonymously-pullable one. That was survivable while the only way to " +
				"self-host was to download this file, and became a bug the moment `ptln server install` " +
				"started EMBEDDING the source, which sent every installer straight at a registry that " +
				"answers 403. Set WEB_IMAGE / RELAY_IMAGE in your .env to point somewhere else. " +
				"RELAY_PORT and WEB_TAG come from your .env too; the relay service is optional for a single-box install. " +
				"Nothing here hardcodes a port: BIND_ADDR, HTTP_PORT, HTTPS_PORT and RELAY_PORT all come from your .env, " +
				"and docker-compose.direct.yml drops Caddy for a plain-HTTP install on a private network.",
			[]string{
				// NO PLATFORM PIN. There used to be one — `platform: ${SELFHOST_PLATFORM:-linux/amd64}`
				// on both services — because the published images were linux/amd64 ONLY, so on any
				// ARM host (Apple Silicon, Graviton, Ampere, Hetzner's ARM line) an unpinned pull
				// asked for linux/arm64/v8, found no matching manifest and stopped. The pin made the
				// amd64 image run under emulation instead.
				//
				// build-images.yml now builds both images with `docker buildx --platform
				// linux/amd64,linux/arm64` and deploy-prod mirrors that index to Docker Hub, so
				// there is a native arm64 manifest to resolve. KEEPING the pin would be worse than
				// useless now: it would force every ARM self-hoster onto the emulated amd64 image
				// even though a native one is sitting in the same index, and they would have to
				// discover an undocumented environment variable to opt out of a penalty we imposed.
				//
				// scripts/public-images.sh asserts both architectures are present in the published
				// :latest, so if the multi-arch publish ever regresses this fails loudly in CI
				// rather than quietly in a stranger's terminal.
			}},
		// The no-reverse-proxy overlay. Published for the same reason as the compose file itself:
		// a self-hoster on a private network has to be able to fetch it before installing
		// anything, and it is useless without the file it overlays.
		{"web/public/self-host/docker-compose.direct.yml", "deploy/stack/docker-compose.direct.yml",
			"Verbatim. An OVERLAY, not a replacement: docker compose -f docker-compose.yml -f docker-compose.direct.yml up -d. " +
				"It stops Caddy and publishes the app and Keycloak directly, on plain HTTP — a private network only.", nil},
