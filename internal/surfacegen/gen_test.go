...
import (
	"os"
	"partyline.sh/partyline/internal/artifactlock"
	"path/filepath"
	"regexp"
	"strings"
	"testing"
...
func TestCheckDetectsAHandEdit(t *testing.T) {
	// Registered BEFORE the restore below so it runs AFTER it: cleanups are LIFO, and a
	// deferred release would free the lock while the artifact was still hand-edited —
	// which left a window the reader still fell into (1 in 25 under stress).
	t.Cleanup(artifactlock.Acquire())
	files, err := Files(repoRoot)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLLMsFilesDescribeTheCurrentProduct(t *testing.T) {
	...
	for _, f := range files {
		...
		// The terms a model must find to describe the product correctly. "work board" and "/work"
		// were here until the board was removed; asserting them now would demand the artifact
		// advertise a page that does not exist — which is the failure this test exists to catch,
		// pointed the wrong way.
		for _, want := range []string{"crank", "worktree", "verify gate", "context threads", "daemon", "fleet", "/describe"} {
...
func TestPublishedEnvReferenceCarriesNoValues(t *testing.T) {
	...
	// wrong reason and the split has collapsed. The witness is SITE_URL — our staging hostname,
	// which is exactly the kind of value the published copy must never carry. (It used to be the
	// literal "=staging", from NEXT_PUBLIC_PARTYLINE_ENV; that variable existed only to label
	// Sentry events and went with Sentry.)
	for _, f := range files {
		if f.Path == "deploy/stack/env.example" && !strings.Contains(string(f.Body), "SITE_URL=https://staging.partyline.sh") {
