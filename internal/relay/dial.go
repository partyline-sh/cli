...
func ServeHostReconnecting(sess *ptysess.Session, ms *yamux.Session, key []byte, code, relayAddr, token string, inviteOnly bool) {
	ServeHostReconnectingStop(sess, ms, key, code, relayAddr, token, inviteOnly, nil)
}

// ServeHostReconnectingStop is ServeHostReconnecting plus an explicit `stop` channel: closing it
// tears down serving WITHOUT ending the local session (used when a host un-shares a session that
// keeps running locally — e.g. the mux's ctrl-\ s "stop sharing"). A background watcher closes the
// current relay conn on stop so a blocked AcceptStream unwinds; the loop then exits instead of
// reconnecting. stop == nil ⇒ identical to the plain reconnecting serve.
func ServeHostReconnectingStop(sess *ptysess.Session, ms *yamux.Session, key []byte, code, relayAddr, token string, inviteOnly bool, stop <-chan struct{}) {
	defer obs.Guard("relayServe")
	const maxBackoff = 30 * time.Second
	backoff := time.Second

	// Watch `stop`: close whatever relay conn is current so ServeHostE2EE's AcceptStream returns.
	var mu sync.Mutex
	cur := ms
	stopped := func() bool {
		if stop == nil {
	...
	for {
		...
		if stopped() { // host un-shared — stop serving, leave the session running
			return
		}
		...
		for {
			...
			case <-sess.Done:
				return
			case <-stop: // nil channel never fires — safe when sharing isn't stoppable
				return
			...
			if derr == nil {
				ms = next
				mu.Lock()
				cur = next // keep the stop-watcher pointed at the live conn
				mu.Unlock()
				backoff = time.Second
...
func serveJoinerE2EE(sess *ptysess.Session, stream net.Conn, key []byte, code string, inviteOnly bool) {
	...
	// REFUSALS MUST SEND FrameClose. The joiner treats a stream that ends without one as a
	// transport drop and re-dials (join_client.go) — and since the relay is perfectly healthy,
	// every re-dial succeeds instantly, gets refused again, and loops forever at full speed.
	// The 3-minute reconnect deadline never applies: it bounds only the inner redial loop,
	// which exits on the FIRST successful dial. A refusal is a verdict, not a blip; say so.
	refuse := func(msg string) {
		_ = st.WriteFrame(wormhole.FrameOutput, []byte("\r\n"+msg+"\r\n"))
		_ = st.WriteFrame(wormhole.FrameClose, nil)
	}
	if inviteOnly && !verified {
		refuse("partyline: this session is invite-only and you're not signed in.\r\n" +
			"  run `ptln login` on this machine, then open the link again.")
		return
	}
	if sess.Locked() {
		refuse("partyline: the host has locked this session to new joiners.")
		return
	}
