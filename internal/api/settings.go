...
// GetTemplate reads ONE template whole. The list omits `body` on purpose — a persona is a document,
// and twenty of them is a list, not twenty documents — so showing the instruction a trigger will
// actually run needs the single-template read.
func (c *Client) GetTemplate(id string) (*AgentTemplate, error) {
	var out struct {
		Template AgentTemplate `json:"template"`
	}
	err := c.do("GET", "/api/v1/agent-templates/"+id, nil, &out)
	return &out.Template, err
}

// DeleteTemplate removes a persona and reports the triggers that just lost it. Those triggers keep
// firing on their own inline task (the FK is ON DELETE SET NULL), which is worth SAYING rather than
// leaving to be discovered — "tidy up an unused persona" quietly changing what a live pipeline runs
// is the kind of surprise that only shows up at 3am.
func (c *Client) DeleteTemplate(id string) ([]string, error) {
	var out struct {
		Detached []string `json:"detached"`
	}
	err := c.do("DELETE", "/api/v1/agent-templates/"+id, nil, &out)
	return out.Detached, err
}
...
// ── party context (#586/C2 human parity) ──────────────────────────────────────────────────────

type PartyContextOut struct {
	Mode         string `json:"mode"`
	ProjectLabel string `json:"project_label"`
	Applies      string `json:"applies"`
}

// SwitchPartyContext re-points a LIVE party at a different persona and/or project.
//
// The same endpoint the agent's switch_context MCP tool calls, reached with the USER's token
// instead of a party token. Without this a human could only do from the API what their agent could
// do from a tool — the exact asymmetry the four-part rule exists to prevent.
func (c *Client) SwitchPartyContext(partyID, mode, projectLabel string) (*PartyContextOut, error) {
	body := map[string]any{}
	if mode != "" {
		body["mode"] = mode
	}
	if projectLabel != "" {
		body["project_label"] = projectLabel
	}
	var out PartyContextOut
	return &out, c.do("POST", "/api/v1/parties/"+partyID+"/context", body, &out)
}
