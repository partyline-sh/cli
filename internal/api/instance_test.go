...
// withHome points HOME at a scratch dir so a test never reads or writes the operator's real
// ~/.partyline.
//
// EVERY test in this package that asserts UNCONFIGURED behaviour needs this, not just the ones
// about the instance file. Base() resolves PARTYLINE_API → the remembered instance → the historical
// default, so clearing the env var alone no longer means "production" — it means "whatever this
// developer's machine is signed in to". Tests written before that change passed in CI (which has no
// instance file) and failed on any box that had one. Reach for this whenever a test's premise is
