...
// session on the thread — MVP presence for the web. Best-effort; callers ignore the error.
func (c *Client) MarkConnected(threadID, engine string) error {
	return c.do("POST", "/api/v1/threads/"+threadID+"/presence", map[string]string{"engine": engine}, nil)
}
...
func (c *Client) Remember(threadID, kind, body, agent, engine string, supersedes int64) (*ContextBlock, error) {
...
// carries the single-use join ref, so it's the EXECUTION trigger (exchange+spawn). onKill
// fires with a request id when a party member asks to stop that launch. Returns when the
// connection drops so the caller reconnects.
func DaemonStream(ctx context.Context, base, token string, onReady func(), onLaunch func(LaunchEvent), onAccepted func(LaunchEvent), onKill func(string)) error {
	...
	for sc.Scan() {
		...
		case strings.HasPrefix(line, "data: "):
			var ev LaunchEvent
			if json.Unmarshal([]byte(line[6:]), &ev) != nil || ev.RequestID == "" {
				continue
			}
			switch ev.Type {
