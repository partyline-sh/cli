...
// LaunchEvent is a pending launch request pushed down the daemon stream (R3). It carries a
// LABEL + a single-use party_join_ref — never a path, command, or raw party token. The
// daemon resolves the label against its OWN local registry and the owner confirms before
// anything runs.
...
// per pending launch request; onKill fires with a request id when a party member asks to
// stop that launch. Returns when the connection drops so the caller reconnects.
func DaemonStream(ctx context.Context, base, token string, onReady func(), onLaunch func(LaunchEvent), onKill func(string)) error {
