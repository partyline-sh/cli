...
type SkillMeta struct {
	...
	// Status (#229): proposed | active | archived. Only ACTIVE skills may reach an agent surface;
	// empty (an older server) is treated as active for compatibility.
	Status    string `json:"status"`
	Version   int    `json:"version"`
	UpdatedAt string `json:"updated_at"`
}

// AgentVisible is the #229 boundary as one rule: may an AGENT see/run this skill? Enabled AND
// approved (empty status = pre-approval server, grandfathered active).
func (m SkillMeta) AgentVisible() bool {
	return m.Enabled && (m.Status == "" || m.Status == "active")
}
...
// ProposeSkill is PushSkill for an AGENT: the skill lands `proposed` no matter whose token carried
// it, and `reason` records the recurrence that prompted it.
//
// The role check PushSkill inherits is right for a person — an admin pushing IS the approver, so
// making them approve their own push is theatre. It is wrong for an agent holding that admin's
// token: the agent did not decide, and letting it inherit the human's authority is exactly how "a
// human accepts before any agent sees it" would stop being true. Authorship, not credential.
func (c *Client) ProposeSkill(name, description, body, reason string) (string, int, error) {
	var out struct {
		Name    string `json:"name"`
		Version int    `json:"version"`
	}
	in := map[string]string{"name": name, "description": description, "body": body, "proposal_reason": reason}
	if err := c.do("POST", "/api/v1/skills", in, &out); err != nil {
		return "", 0, err
	}
	return out.Name, out.Version, nil
...
// ApproveSkill relays the human verdict on a proposed skill (#229): action = "approve" | "decline".
// Admin/owner only — the server enforces.
func (c *Client) ApproveSkill(name, action string) (string, error) {
	var out struct {
		Name   string `json:"name"`
		Status string `json:"status"`
	}
	if err := c.do("POST", "/api/v1/skills/"+name+"/approve", map[string]any{"action": action}, &out); err != nil {
		return "", err
	}
