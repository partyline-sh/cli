...
func TestPartyReadRetries(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"plan_upsert", func(pc *PartyClient) error {
			_, err := pc.PlanCreateItem(map[string]any{"kind": "task", "title": "x"})
			return err
		}},
...
// TestListConsults pins the inbound/outbound list client: the direction rides as a query param, the
// account token authenticates it, and every field the modal needs (who asked, which project, which
// DAEMON must answer, when it was asked) survives the round trip. DaemonID/DeviceLabel are the ones
// that matter for correctness downstream — they're how the UI knows whether a question can be
// answered on THIS machine or has to be answered on another one.
func TestListConsults(t *testing.T) {
	t.Run("inbound consults round trip", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			if r.Method != "GET" || r.URL.Path != "/api/v1/daemon/consults" {
				t.Errorf("unexpected request %s %s", r.Method, r.URL.Path)
			}
			...
			_ = json.NewEncoder(w).Encode(map[string]any{"consults": []map[string]any{{
				"id": "c1", "direction": "inbound", "status": "pending",
				"project_label": "partyline", "question": "does the limiter retry 429s?",
				"peer": "darcy", "daemon_id": "d-mine", "device_label": "air",
		...
		got, err := c.ListConsults("inbound")
		if err != nil {
			t.Fatalf("ListConsults: %v", err)
		}
		if len(got) != 1 {
			t.Fatalf("got %d consults, want 1", len(got))
		}
		m := got[0]
		if m.ConsultID != "c1" || m.Direction != "inbound" || m.Peer != "darcy" || m.ProjectLabel != "partyline" {
			t.Errorf("consult = %+v", m)
		}
		if m.DaemonID != "d-mine" || m.DeviceLabel != "air" {
			t.Errorf("answering daemon lost: %+v", m)
		}
		if m.CreatedAt.IsZero() {
			t.Error("created_at must parse — the modal shows how long a question has been waiting")
		}
	})

	t.Run("an error surfaces rather than an empty inbox", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			w.WriteHeader(403)
			_ = json.NewEncoder(w).Encode(map[string]any{"error": "unauthenticated"})
		}))
		defer srv.Close()
		c := &Client{Base: srv.URL, Token: "tok", HTTP: srv.Client()}
		if _, err := c.ListConsults("inbound"); err == nil {
