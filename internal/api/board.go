...
type BoardCard struct {
	...
	// Seq is a foreign board's own row order (Odoo's kanban `sequence`, ascending = top).
	// Zero-valued on partyline cards, whose order is Rank.
	Seq int `json:"seq,omitempty"`
	// Ref is a foreign board's own identifier for the card, shown on the tile so a person can
	// quote it to somebody working in that tracker (an Odoo task id: "#28188"). Empty on
	// partyline cards, whose id is a uuid nobody says out loud.
	Ref string `json:"ref,omitempty"`
	// Pri is a foreign board's priority band ("0".."3" on Odoo), which OUTRANKS Seq in the
	// source's own ordering — a drop that crosses bands must write this too or the source
	// will never display the previewed position.
	...
	// Fields and Body are everything that does not fit on a tile, shown in the DETAIL pane.
	//
	// A tile is three lines and has to stay scannable, so a provider that sent its whole task there
	// would make the board unreadable. But the point of showing a foreign board at all is not having
	// to go back to the tracker, and a detail pane with three lines in it sends you straight back.
	// So the tile summarises and `d` carries the rest: Fields for the structured half (assignee,
	// deadline, customer, tags) rendered as an aligned block, Body for the prose.
	Fields []BoardCardField `json:"-"`
	Body   string           `json:"-"`
}

// BoardCardField is one labelled value on a card's detail pane. Ordered, because the provider knows
// which of its fields a reader looks at first and we do not.
