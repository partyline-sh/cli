...
func TestNames(t *testing.T) {
	want := []string{"claude", "codex", "gemini", "opencode", "goose", "antigravity", "llm", "prime-agent"}
...
func TestCaps(t *testing.T) {
	want := map[string]Caps{
		...
		// The bridge claims nothing: no resume, no stream-json, no per-invocation MCP. It reaches
		// arbitrary OpenAI-compatible endpoints and local models, and anything that assumed an agent
		// loop here would assume one it cannot run.
		"llm": {},
		// prime-agent claims nothing either. It HAS sessions (-c/-r/attach) and a json mode, so
		// resume and stream may well work — but unverified is not the same as absent, and a wrong
		// cap fails silently mid-run rather than loudly at startup.
		"prime-agent": {},
