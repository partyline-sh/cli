...
var (
	// ToolsNone: the run may only reason over the prompt — every tool hard-disallowed (claude:
	// --disallowedTools <all> + --strict-mcp-config; NOT the old `--allowedTools ""`, which didn't
	// actually disable anything). Used by the independent reviewer / the ask-peer consult answerer.
	ToolsNone = ToolPosture{kind: postureNone}
	// ToolsReadOnly: inspect the repo, change nothing — Read/Grep/Glob allowed, everything that could
	// ACT (Bash/Edit/Write/…/MCP) hard-disallowed. Used by describe + the graded review.
...
// claude's --allowedTools only PRE-APPROVES; it does NOT disable a tool — Read/Grep/Glob need no
// approval and safe Bash is auto-approved regardless, so `--allowedTools ""` is NOT tool-less (proven:
// a `-p --allowedTools ""` run still reads files). Only --disallowedTools is a hard block (proven: a
// run with Read/Bash/Grep/Glob disallowed reports "no filesystem-access tool available"). So for the
// non-write postures we ENUMERATE the block list with --disallowedTools (comma-separated — the form
// claude actually parses) and drop project/user MCP with --strict-mcp-config so no mcp__* tool sneaks
// in. This is what makes "read-only" and "tool-less" real (the ask-peer consult + the review agent
// both depend on it; see docs/plans/ask-peer.md P0.0).
const (
	// Mutating / executing / network tools — blocked in the read-only posture (Read/Grep/Glob stay).
	claudeDisallowMutating = "Bash,Edit,Write,MultiEdit,NotebookEdit,WebFetch,WebSearch,Task"
	// Everything above PLUS the read tools — blocked in the tool-less posture.
	claudeDisallowAll = claudeDisallowMutating + ",Read,Grep,Glob,TodoWrite"
)

func claudeOneShot(prompt, model string, posture ToolPosture) []string {
	argv := []string{"claude", "-p", prompt, "--output-format", "json"}
	switch posture.kind {
	case postureNone:
		// Genuinely tool-less: hard-disallow every tool + no MCP.
		argv = append(argv, "--allowedTools", "", "--disallowedTools", claudeDisallowAll, "--strict-mcp-config")
	case postureReadOnly:
		// Read/Grep/Glob allowed; everything that could ACT is hard-disallowed + no MCP.
		argv = append(argv, "--allowedTools", "Read Grep Glob", "--disallowedTools", claudeDisallowMutating, "--strict-mcp-config")
	case postureWrite:
		// Build worker: writes are intended. Allowlist gates it; allowBash adds shell.
		list := []string{"Read", "Grep", "Glob", "Edit", "Write", "MultiEdit", "TodoWrite"}
		if posture.allowBash {
			list = append(list, "Bash")
		}
		argv = append(argv, "--allowedTools", strings.Join(list, ","))
	}
	if model != "" {
