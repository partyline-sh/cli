...
// CursorSeq returns the escape that places the cursor at the emulator's current position (1-based).
// The mux re-asserts it after a repaint that emits DECSTBM (which homes the cursor) so the child's
// real cursor isn't left at the scroll-region origin (#238).
func (s *Session) CursorSeq() string {
	s.mu.Lock()
	defer s.mu.Unlock()
	pos := s.vt.CursorPosition()
	return fmt.Sprintf("\x1b[%d;%dH", pos.Y+1, pos.X+1)
}
