...
func New(argv []string, hostName string, openLine bool) (*Session, error) {
	return NewIn("", argv, hostName, openLine)
}

// NewIn is New but starts the program in dir (its working directory). dir=="" inherits
// the current process's cwd. Used by the local multiplexer to resume a session in the
// project directory it was recorded in.
func NewIn(dir string, argv []string, hostName string, openLine bool) (*Session, error) {
...
// CursorPos returns the emulator's current cursor position, 1-based (row, col) —
// the same coordinates a terminal's CUP (\x1b[r;cH) expects. The local mux uses it to
// restore the cursor after painting its status bar, instead of DEC save/restore
// (\x1b7/\x1b8), which shares a single slot with the child app and would clobber it.
