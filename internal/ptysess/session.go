...
// handleRE matches a teammate's @handle (or a team name) — the other thing /pinvite
// accepts. Same shape the control plane enforces for handles; the server still decides
// whether it resolves to anyone the caller may see.
var handleRE = regexp.MustCompile(`^@?[A-Za-z0-9][A-Za-z0-9-]{1,30}$`)
...
// followed by the current screen as a SINGLE CONTINUOUS stream. A caller that has switched to
// the main screen and cleared it can write this so the scrollback lands in the terminal's
// NATIVE buffer (scrolled above) and the current screen is visible — so native mouse-scroll +
// drag-copy show exactly THIS session's history, per session, with no cross-session bleed.
//
// viewRows is the FULL terminal height, which is >= the child screen height when the caller
// reserves a row (e.g. the mux's status bar). The current screen is padded to viewRows so the
// trailing block is exactly one viewport tall: the screen always lands at row 1 and the extra
// pad row (which the caller overwrites with its bar) sits at the bottom. Without this the
// screen floats down by (viewRows - childHeight) when scrollback is present — a stray history
// line clings to row 1 and the cursor address is off by that much (#238).
//
// For an alt-screen program (no scrollback) this degrades to the normal Snapshot.
// maxLines<=0 means "no scrollback". Ends by positioning the cursor.
func (s *Session) SnapshotHistory(maxLines, viewRows int) []byte {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.vt.IsAltScreen() {
		return s.snapshotLocked()
	...
	// The screen padded to viewRows rows (the full viewport), continuous (CRLF BETWEEN lines,
	// none after the last so no extra scroll). Because the trailing block is exactly one
	// viewport tall, it fills the visible area from row 1 down — with the scrollback above it —
	// regardless of how many scrollback lines precede it.
...
// CursorPos returns the emulated cursor's 0-based (col,row) on the current screen. The mux's
// split view needs it to place the REAL cursor at an absolute position inside a pane, since it
// paints each pane row-by-row rather than streaming the child's own escapes.
...
func (s *Session) runPCommand(p *Participant, cmd string) bool {
	...
	case "/pinvite":
		if len(f) < 2 {
			s.Notice(p, "usage: /pinvite someone@example.com — or /pinvite @teammate")
			break
		}
		if s.OnInvite == nil {
		...
		target := f[1]
		if len(target) > 320 || !(emailRE.MatchString(target) || handleRE.MatchString(target)) {
			s.Notice(p, "that doesn't look like an email or a handle — usage: /pinvite someone@example.com or /pinvite @teammate")
			break
		}
	...
	case "/phelp":
		s.Notice(p, "/pwho · /phud [on|off] · /pinvite <email|@handle> · /pgrant [name] (host) · /plock (host) · /pexit · in full-screen apps (vim/claude) use the ctrl-\\ prefix: w g r h l d q ?")
	}
	return true
}
