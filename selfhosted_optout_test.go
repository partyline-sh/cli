...
func TestOptOutsPointedAtProduction(t *testing.T) {
	...
	if telemetryDisabled() {
		t.Error("telemetryDisabled() = true against production; production behaviour must be unchanged")
	}
	if updateChecksDisabled() {
		t.Error("updateChecksDisabled() = true against production; production behaviour must be unchanged")
	}
}
...
func TestOptOutsWithNoBaseConfigured(t *testing.T) {
	...
	if telemetryDisabled() || updateChecksDisabled() {
		t.Error("an unconfigured install must behave exactly like production")
	}
}
