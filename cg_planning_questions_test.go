...
type planTestServer struct {
	...
	// The RECORD a filed plan leaves: the party it is readable in, and the document written into it.
	// Finalize creates this BEFORE the tree and refuses to file without it, so the fake plane has to
	// serve it or every finalize test refuses.
	originParty string
	partyDoc    string
}
...
func (p *planTestServer) start(t *testing.T) *api.Client {
	...
	mux.HandleFunc("/api/v1/parties", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"id":"party_rec","join_code":"abc","org":"team","link":"x"}`))
	})
	mux.HandleFunc("/api/v1/parties/party_rec/doc", func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "PUT" {
			var in struct {
	...
	mux.HandleFunc("/api/v1/work-items/tree", func(w http.ResponseWriter, r *http.Request) {
		var in struct {
			...
			OriginPartyID string           `json:"origin_party_id"`
		}
		_ = json.NewDecoder(r.Body).Decode(&in)
		p.filed, p.tree, p.originParty = true, in.Root, in.OriginPartyID
