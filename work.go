...
func workMain(args []string) {
	...
	out, err := runWorker(dir, task, model, thread, allowBash, timeout, nil, "")
...
// answer text, the run's token usage, and the engine's session id (the opaque resume handle).
type workerResult struct {
	Result    string      `json:"result"`
	Usage     workerUsage `json:"usage"`
	SessionID string      `json:"session_id"` // Slice 2: engine's opaque resume handle ("" = no resume)
}
...
func runWorker(dir, task, model, thread string, allowBash bool, timeout time.Duration, onLog func(string), resume string) (workerOutcome, error) {
	streaming := onLog != nil
	outputFormat := "json"
	if streaming {
...
// shape, so usage is treated as unknown and the handle is empty (restart-only).
func parseWorkerOutput(stdout []byte) (text string, tokens int, handle string, ok bool) {
	...
	return r.Result, r.Usage.total(), r.SessionID, true
}
