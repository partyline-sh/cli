...
//
// The four invariants are enforced HERE, at the atom:
...
func workerBashPosture(allowBash bool) string {
	...
	return "- You HAVE shell access. Before you finish, VERIFY your change by running whatever this project uses to check itself. FIND the commands rather than assuming them: look for a task runner or scripts file, a Makefile, a CI config, or the contributing notes — every ecosystem has its own, and this project's are the ones that count. Report exactly what you ran and what it said. If you could not verify, say so plainly rather than implying you did.\n"
}
...
func workerPrompt(task string, ctx string, thread, allowBash, hasStopRun bool) string {
	...
	if hasStopRun {
		// #794 T1b: the stop CHANNEL exists in this run — name it, or the rule below is unactionable.
		b.WriteString("- If your task's stop rule applies, a required tool is missing, or continuing would mean guessing: call the partyline-run `stop_run(reason)` tool, then end your turn. A deliberate stop is a first-class outcome — always better than a plausible wrong action.\n")
	} else {
...
func runWorker(dir, task, engineName, model, thread string, allowBash bool, timeout time.Duration, onLog func(string), resume string) (workerOutcome, error) {
	...
	// #794 T1b: a daemon-managed run gets the run-side MCP — stop_run(reason), the channel for a
	// DELIBERATE stop. Without it, an agent whose stop rule fires can only improvise (the 3am
	// wrong-write) or end its turn looking like success. Attached only when the run credentials
	// are actually in our env (crank under a daemon); a bare `ptln work` doesn't get a tool that
	// could never record anything.
	runMCPConfig := ""
	if os.Getenv("PARTYLINE_RUN_ID") != "" && os.Getenv("PARTYLINE_DAEMON_TOKEN") != "" {
		if exe, xerr := os.Executable(); xerr == nil && exe != "" {
			cfg, merr := json.Marshal(map[string]any{"mcpServers": map[string]any{
				"partyline-run": map[string]any{"command": exe, "args": []string{"run-mcp"}},
			}})
			if merr == nil {
				runMCPConfig = string(cfg)
				tools = append(tools, "mcp__partyline-run__stop_run")
			}
		}
	}
	...
	prompt := workerPrompt(task, workerContext(thread, task), thread != "", allowBash, runMCPConfig != "")
	if resume != "" {
	...
	if runMCPConfig != "" {
		cargs = append(cargs, "--mcp-config", runMCPConfig) // #794 T1b: stop_run
	}
...
func runWorkerAdapter(spec eng.Spec, dir, task, model, thread string, allowBash bool, timeout time.Duration, onLog func(string), resume string) (workerOutcome, error) {
	...
	argv, stdinPrompt, err := spec.OneShotArgs(workerPrompt(task, workerContext(thread, task), false, allowBash, false), model, eng.ToolsWrite(allowBash))
	if err != nil {
