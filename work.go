...
func workerBashPosture(allowBash bool) string {
	if !allowBash {
		return "- You do NOT have shell/Bash access in this run. Do NOT attempt to run tests, builds, linters, `npm`, `tsc`, `vitest`, `go`, `git`, or any shell command, and do NOT spawn sub-agents to run them — those tools are unavailable and every attempt is denied, so trying only wastes the run. Edit files directly; a separate review verifies your work. If the task genuinely cannot be done without running a command, STOP and say so.\n"
	}
...
// so a build never blocks on it. Bounded by workerContextCap (keep the tail — the freshest facts).
func workerContext(threadID string) string {
	...
	if len(facts) > workerContextCap {
		facts = "…(older context elided)…\n" + facts[len(facts)-workerContextCap:]
	}
	return facts
}
...
func runWorker(dir, task, engineName, model, thread string, allowBash bool, timeout time.Duration, onLog func(string), resume string) (workerOutcome, error) {
	...
	prompt := workerPrompt(task, workerContext(thread), thread != "", allowBash)
	if resume != "" {
...
func runWorkerAdapter(spec eng.Spec, dir, task, model, thread string, allowBash bool, timeout time.Duration, onLog func(string), resume string) (workerOutcome, error) {
	...
	argv, stdinPrompt, err := spec.OneShotArgs(workerPrompt(task, workerContext(thread), false, allowBash), model, eng.ToolsWrite(allowBash))
	if err != nil {
