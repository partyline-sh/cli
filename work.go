...
func workMain(args []string) {
	...
	out, _, err := runWorker(dir, task, model, thread, allowBash, timeout)
	fmt.Println(out)
	if err != nil {
...
// the worker recall/remember (Model A — the user's own tokens). It requests `--output-format json`
// so we can read the run's token usage for the crank token ceiling (O.5). Returns the final answer
// text and the total tokens the run reported — 0 means "no usage seen" (unknown, never fatal).
func runWorker(dir, task, model, thread string, allowBash bool, timeout time.Duration) (string, int, error) {
	cargs := []string{"-p", workerPrompt(task, thread != ""), "--output-format", "json", "--allowedTools", strings.Join(workerTools(allowBash), ",")}
	if model != "" {
