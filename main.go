...
func main() {
	...
	// The theme is a property of the INSTALL, not of the launcher. It used to be loaded only on
	// the launcher's boot path, so every other entry point — the board, the ctrl-\ menu, the
	// per-session menus — rendered with the default palette however the user had set it. The
	// board even called themed() on every frame, against a table that had never been loaded.
	//
	// Reading one small file is cheap enough to do unconditionally, including for the stdio
	// subcommands: it touches no protocol stream and cannot fail loudly.
	loadTheme()
	...
	if len(os.Args) > 1 {
		...
		case "login":
			...
			return
		case "connect":
			// Connect a machine with a pasted enrolment token instead of a device flow — the
			// no-browser path for servers, and the way to add the Nth machine without proving
			// who you are for the Nth time. (`join` is a different thing: joining a shared
			// terminal session.)
			connectMachineMain(os.Args[2:])
			return
		case "invite-machine":
			// Mint the token `ptln connect` takes, and print the command to paste.
			inviteMachineMain(os.Args[2:])
			return
		case "setup":
		...
		case "man":
			...
			manMain(os.Args[2:])
			return
		case "memory-capture-hook":
			// Hidden: the Stop hook that records what a session learned. Never blocks the stop.
			memoryCaptureHookMain(os.Args[2:])
			return
		case "keepgoing-hook":
		...
		case "doctor":
			...
			return
		case "feed":
			// The activity pane: what everyone else on the project is doing. A reader — see feed.go.
			feedMain(os.Args[2:])
			return
		case "bus":
			// The live channel between this machine and the rest of the project. Diagnostic:
			// the watcher and the agents are the real callers. See bus_client.go.
			busMain(os.Args[2:])
			return
		case "board":
		...
		case "llms":
			...
			return
		case "memory-hook":
			// UserPromptSubmit hook: print what the team recorded since this session last
			// looked. Hidden — it is wired into sessions, never typed.
			memoryHookMain()
			return
		case "memory":
			// Shared project memory: what the team has learned, as files in the project's
			// memory repo. See memory_store.go.
			memoryMain(os.Args[2:])
			return
		case "tmux":
		...
		fmt.Fprintln(os.Stderr, clispec.WrapNames("  ", 92))
