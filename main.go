...
func main() {
	...
	if len(os.Args) > 1 {
		...
		case "work":
			...
			workMain(os.Args[2:])
			return
		case "shape", "describe": // "shape" is the name (Shape it · Build it · Ship it); "describe" stays a compat alias
			// Requirements agent — conversational, SCORED task authoring that runs your local
			// claude and enqueues a well-specified Backlog card. `ptln shape` in a project dir.
		...
		fmt.Fprintln(os.Stderr, "commands: start, new, join, party, llms, thread, project, work, shape, crank, wt, daemon, org, skill, state, tray, sessions, login, logout, whoami, man, upgrade, version, help")
