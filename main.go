...
func main() {
	...
	if len(os.Args) > 1 {
		...
		// Not a known command. A leading flag or "--" used to mean "host a shared shell
		// with these args"; that's now `ptln start …` (bare `ptln` opens the session
		// manager), so point the user there instead of silently hosting a shell.
