...
func main() {
	...
	if len(os.Args) > 1 {
		...
		fmt.Fprintln(os.Stderr, "commands: start, new, join, party, chat, llms, thread, project, trigger, work, plan, crank, wt, daemon, peer, org, skill, settings, server, state, tray, sessions, login, logout, whoami, man, upgrade, version, help")
