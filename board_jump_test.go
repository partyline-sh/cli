...
func TestBellCountsMultipleArrivals(t *testing.T) {
	before := pboard(&api.Board{Building: []api.BoardCard{card("a"), card("b"), card("c")}})
	after := pboard(&api.Board{Review: []api.BoardCard{card("a"), card("b"), card("c")}})
	ring, note := boardBell(before, after)
	if !ring {
...
func TestBellSilentOnFirstLoad(t *testing.T) {
	if ring, _ := boardBell(nil, pboard(fullBoard())); ring {
		t.Error("the first board load must not ring")
	}
	if ring, _ := boardBell(pboard(fullBoard()), nil); ring {
...
func TestBellIgnoresAcceptedColumn(t *testing.T) {
	before := pboard(&api.Board{Review: []api.BoardCard{card("a", withStatus("done"))}})
	after := pboard(&api.Board{Accepted: []api.BoardCard{card("a", withStatus("done"))}})
