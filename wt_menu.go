...
func wtForkMenu(mx *ptymux.Mux) {
	...
	fmt.Printf("  carry your uncommitted work into it? %s(y/N)%s › ", cgDim, cgOff)
	ws, _ := in.ReadString('\n')
	withState := strings.HasPrefix(strings.ToLower(strings.TrimSpace(ws)), "y")
