...
func tmuxMenuItems() []tmuxMenuItem {
	...
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		...
		items = append(items, tmuxMenuItem{label: label, key: key, paneID: id})
	}
	items = append(items,
		...
		tmuxMenuItem{label: "merge the highlighted session in beside this one", key: "+", merge: true},
		tmuxMenuItem{label: "move the highlighted session back to its own window", key: "-", brk: true},
