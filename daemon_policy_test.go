...
func TestEveryGateDefaultsToAutoForARegisteredProject(t *testing.T) {
	...
	// THE ONE THAT WAS WRONG. Without this the run is announced to a console nobody is reading.
	if !autoRunProject("proj") {
		t.Error("autoRunProject = false for a registered project — runs will wait forever for an " +
			"approve-run prompt that the always-on service never renders")
	}
}
...
func TestAskStillOptsAProjectBackIn(t *testing.T) {
	...
	if autoRunProject("proj") {
		t.Error("autoRunProject = true despite policy:ask — the opt-out is gone")
	}
}

// An UNREGISTERED label must NOT auto-run: registration is the consent, so with no registration
// there is no consent. This is the one case where the answer is legitimately false, and it is what
// keeps "default auto" from meaning "run anything anyone names".
func TestAnUnregisteredProjectNeverAutoRuns(t *testing.T) {
	withRegistry(t, `{"projects":[{"label":"proj","path":"/tmp/proj"}]}`)

	if autoRunProject("some-other-label") {
		t.Error("autoRunProject = true for a label this machine never registered")
	}
}
