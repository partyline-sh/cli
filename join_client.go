...
func joinE2EE(relayAddr, code string, key []byte, identFn func() string) error {
	...
	//
	// churn guards against a host that refuses us on every attempt (invite-only and not signed
	// in, session locked) while the relay itself is perfectly healthy. Each re-dial then
	// SUCCEEDS instantly, so the backoff below never engages and the 3-minute deadline never
	// applies — it bounds only the inner redial loop, which exits on the first good dial. A
	// host on a version that sends FrameClose with its refusal ends this cleanly; this is the
	// floor for every other case, including older hosts.
	churn, lastUp := 0, time.Now()
	for {
		...
		// A connection that lasted only a moment was almost certainly refused, not dropped.
		// Five of those in a row is a verdict — stop, and say something the joiner can act on.
		if time.Since(lastUp) < 5*time.Second {
			if churn++; churn >= 5 {
				fmt.Print("\r\n☎ the host keeps ending this connection right after it opens.\r\n" +
					"  Usually that means the session is invite-only and this machine isn't signed in —\r\n" +
					"  run `ptln login`, then open the link again. If you are signed in, ask the host\r\n" +
					"  to check that the session isn't locked (`/plock`) and that you're on the invite.\r\n")
				return nil
			}
		} else {
			churn = 0
